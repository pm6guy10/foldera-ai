...
function mockAuthenticatedMicrosoftSettingsRequests(page: Page) {
  const future = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
...
test.describe("Settings manual brief trigger", () => {
  test("shows the sign-in prompt for signed-out visitors", async ({ page }) => {
    ...
    await expect(
      page.getByText("Please sign in to view settings"),
    ).toBeVisible();
  });
  ...
  test("renders owner trigger button and success state", async ({ page }) => {
    ...
    await expect(
      page.getByText("Generated briefs for 1 eligible user."),
    ).toBeVisible();
    await expect(
      page.getByText("Sent briefs for 1 eligible user."),
    ).toBeVisible();
  });
  ...
  test("renders failure state when manual trigger fails", async ({ page }) => {
    ...
    await page.route("**/api/settings/run-brief", async (route) => {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          error: "Could not run today's brief right now.",
        }),
      });
    });
    ...
    await expect(
      page.getByText("Could not run today's brief right now."),
    ).toBeVisible();
  });

  test("renders Microsoft sync coverage separately from incremental inserts", async ({
    page,
  }) => {
    await mockAuthenticatedMicrosoftSettingsRequests(page);
    await page.route("**/api/microsoft/sync-now", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          ...
          file_signals: 0,
          task_signals: 0,
          mail_total_signals: 278,
          calendar_total_signals: 45,
    ...
    await expect(
      page.getByText("323 Microsoft signals available"),
    ).toBeVisible();
    await expect(
      page.getByText("278 mail · 45 calendar · 0 files · 0 tasks"),
    ).toBeVisible();
    await expect(
      page.getByText(
        "27 new this run: 27 mail · 0 calendar · 0 files · 0 tasks",
      ),
    ).toBeVisible();
  });
});
