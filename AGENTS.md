...
**Reality check:** The Vercel project may **also** have GitHub integration enabled. That means **two production deploys per push** unless the post-CI workflow skips the CLI path. [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) calls the **Vercel REST API** (`/v6/deployments`) after checkout; if a **READY** **`target=production`** deployment already has **`meta.githubCommitSha`** equal to the CI commit, it **skips** `vercel build` + `vercel deploy` — avoids burning **Hobby `api-deployments-free-per-day`** (~100/24h) and pointless failures when www is already correct. If the CLI still runs and stderr shows **`api-deployments-free-per-day`**, retries are skipped; wait for the rolling window or upgrade **Pro**. **Source of truth for www:** `GET /api/health` → `revision.git_sha`, not the GitHub “Deploy to Vercel” email alone. Runbook: [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md) (**Deploy workflow red emails**). The deploy job uses **`concurrency: vercel-prod-cli-foldera`** + conditional **retries** when CLI deploy runs.
