...
**“Deploy to Vercel” workflow email failed:** The post-CI [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) job can fail at **`vercel deploy --prebuilt`** while **`vercel build`** succeeded — often a **race** with Vercel’s GitHub integration or overlapping runs. **Do not** treat that email alone as “prod is down”; confirm **Vercel READY** for the commit and **`/api/health` `revision`**. Mitigations: workflow **`concurrency`** + **retries** (see `deploy.yml`); operator note in [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md).
