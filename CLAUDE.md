...
**“Deploy to Vercel” workflow email failed:** The post-CI [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) job can fail at **`vercel deploy --prebuilt`** while **`vercel build`** succeeded — often a **race** with Vercel’s GitHub integration, overlapping runs, or **Hobby `api-deployments-free-per-day`**. **Do not** treat that email alone as “prod is down”; confirm **`/api/health` `revision`**. Mitigations: [scripts/ci/vercel-deploy-preflight.py](scripts/ci/vercel-deploy-preflight.py) **preflight** (skip / wait for Git / **check-ready** after quota); **`concurrency`**; CLI **retries** when CLI runs. Operator: [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md).
