...
2. Read `WHATS_NEXT.md` first, before all other reference files.
...
10. If recent changes or repo state conflict with the task, report that before editing.
...
- **Pre-launch pipeline spend (Vercel production):** `PROD_DEFAULT_PIPELINE_DRY_RUN=true` — `POST /api/settings/run-brief` defaults to **`pipelineDryRun`** (no Anthropic on the directive path) unless the client sends **`use_llm=true`** **and** **`ALLOW_PROD_PAID_LLM=true`** is set. Settings UI primary is **Run pipeline (dry run)** (`?dry_run=true`); secondary **Generate with AI** uses `use_llm=true`. Response includes **`spend_policy`** (`pipeline_dry_run`, `paid_llm_requested`, `paid_llm_effective`). Manual run-brief always passes **`skipSpendCap: false`** and **`skipManualCallLimit: false`** so daily spend and manual-call limits apply on paid runs. **`CRON_DAILY_BRIEF_PIPELINE_DRY_RUN=true`** — scheduled `/api/cron/daily-brief` passes **`pipelineDryRun: true`** (zero-LLM scheduled exercise; health alert still runs in `finally`). **`FOLDERA_ANOMALY_USE_HAIKU=true`** — generator anomaly pass uses Haiku + shorter prompt cap instead of Sonnet + 14k chars (cheaper when paid LLM is on). **Ops — `api_usage` last 24h by endpoint:** `SELECT endpoint, SUM(estimated_cost)::float, COUNT(*) FROM api_usage WHERE created_at > now() - interval '1 day' GROUP BY 1 ORDER BY 2 DESC NULLS LAST;`
...
**GitHub Actions / pre-push gate:** Before pushing, run the same checks CI runs: `npm run lint`, `npm run build`, `npx vitest run --exclude ".claude/worktrees/**"`, and **`npm run test:ci:e2e`**. Do not push if **`build-and-test` on `main` would fail** (e.g. missing-env 500s). `/api/health` must return **200** even when Supabase env is absent (degraded body) so CI E2E stays DB-free. **Deploy correlation:** production JSON includes **`revision.git_sha`** (Vercel) and **`build`** (7-char prefix); headers **`x-foldera-git-sha`** / **`x-foldera-deployment-id`** — compare to `git rev-parse HEAD` / Vercel dashboard after push to confirm the fix is live (`lib/config/deploy-revision.ts`). **Source of truth for “what’s live”:** always **`GET https://www.foldera.ai/api/health`** (or your prod origin) — **`revision.git_sha`** beats assumptions from git alone. **Docs-only** follow-up commits still deploy; www may show a **newer** SHA than the feature commit (e.g. `759ca8a`, `8964ad8` after `a12db1d`) — that is expected until you re-check health.

**“Deploy to Vercel” workflow email failed:** The post-CI [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) job can fail at **`vercel deploy --prebuilt`** while **`vercel build`** succeeded — often a **race** with Vercel’s GitHub integration, overlapping runs, or **Hobby `api-deployments-free-per-day`** (stderr: try again in 24 hours / more than 100). **Do not** treat that email alone as “prod is down”; confirm **Vercel READY** for the commit and **`/api/health` `revision`**. Mitigations: workflow **`concurrency`**, **API probe** to **skip** CLI deploy when production already matches the commit (saves double deploys vs Git integration), **retries** only when CLI runs, **no retries** on quota errors; operator note in [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md).

**Production vs `main` drift:** If health **`revision`** is missing or SHA lags **`origin/main`**, see [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md) **Production vs `main` (alias race + source of truth)**.
