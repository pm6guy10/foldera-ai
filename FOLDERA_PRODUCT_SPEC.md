...
| Directive generation uses Sonnet | BUILT | April 1: `lib/briefing/generator.ts` `GENERATION_MODEL` = `claude-sonnet-4-20250514`. `SYSTEM_PROMPT` = FOLDERA CONVICTION ENGINE (analyst-grade finished-artifact bar, causal diagnosis, evidence rules, JSON-only contract; **MANDATORY EMAIL PATH** forbids `do_nothing`/`wait_rationale` except **DECAY_RECONNECTION EXCEPTION** when the user prompt includes `DECAY_RECONNECTION_RULE` and context is ungrounded). `lib/briefing/__tests__/pipeline-receipt.test.ts` Anthropic mock branches on `FOLDERA CONVICTION ENGINE`. **Decay isolation (April 1):** for `discrepancyClass === 'decay'`, generator skips `runConvictionEngine` (avoids financial-runway goal bleed) and `researchWinner`; `fetchWinnerSignalEvidence` skips title-keyword expansion and filters snippets to entity email/name anchors; `buildDecayRecipientBriefFromEntity` fills `recipient_brief` when the standard parser fails but the entity row has `<email>`; relationship timeline uses `entityName` when present. Unit: `multi-user-safety.test.ts` decay short-path assertion. |
