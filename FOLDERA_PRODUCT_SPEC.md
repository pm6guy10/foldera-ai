...
| Unified marketing navigation | BUILT | March 31: `components/nav/NavPublic.tsx` on `/`, `/pricing`, `/blog`, `/blog/[slug]`, `app/not-found.tsx` — Platform (`#product` or `/#product`), Pricing `/pricing`, Blog `/blog`, Sign in, Get started free or Dashboard from session. **April 1 FLOW:** Below `sm`, full-screen overlay menu (`fixed inset-0 z-[70]`, `bg-[#07070c]/95 backdrop-blur-xl`), centered `text-lg font-bold` links, X close, backdrop + Escape close; hamburger for signed-in and signed-out; `data-testid="nav-mobile-menu-toggle"`. |
