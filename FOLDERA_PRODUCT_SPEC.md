...
Last Updated: April 1, 2026 (OPS mobile visual QA: `html`/`body` `#07070c`; footer/blog/pricing link 44px targets; scenario Prev/Next + tab 44px hit areas; pricing card `w-full` gutters; mobile menu `pointer-events-none` shell + `pointer-events-auto` nav so backdrop closes; solid overlay `bg-[#07070c]`; `tests/e2e/mobile-visual-qa.spec.ts` — scrollWidth ≤375, hamburger X + backdrop dismiss, dual PNGs under `tests/screenshots/mobile{,-after}/`; mobile-first pass: no code-local owner subscription/pinned-constraint bypass; `resolveUserPromptNames` + structured `user_full_name` / `user_first_name` in generator; scorer static duplicate stopwords exported + `full_name` in auth stop tokens; acceptance alerts at brief@foldera.ai; owner `user_subscriptions` row ensured in prod DB)
