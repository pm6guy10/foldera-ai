...
| Cron fires at 4am PT ingest + 4:10am PT brief | BUILT | vercel.json: `nightly-ops` `0 11 * * *` (sync/signals/maintenance only); `daily-brief` `10 11 * * *` (generate+send). Split April 1, 2026 so Sonnet gets a full serverless window. Live PROVEN receipt after next deploy + cron. | — |
