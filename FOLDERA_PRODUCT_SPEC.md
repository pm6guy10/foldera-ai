...
| Discrepancy detection | YELLOW | commit `fab67f4` (2026-03-28). `lib/briefing/discrepancy-detector.ts` with 5 extractors (decay, exposure, drift, avoidance, risk). Wired into scorer after emergent patterns. 29 unit tests. Production confirmed drift + exposure + risk candidates outranked all open-loop candidates. Blocked at generator: freshness gate and entity suppression false positive. | Generator exemptions — see AUTOMATION_BACKLOG P0 |
...
| Causal diagnosis layer (winner → mechanism → artifact) | BUILT | March 29 causal pass: generator now requires `causal_diagnosis` (`why_exists_now`, `mechanism`) in its output contract, derives a required diagnosis in structured context, and rejects artifacts that do not target the mechanism (`getCausalDiagnosisIssues(...)`). Regression coverage: `causal-diagnosis.test.ts` + `generator-runtime.test.ts` additions. Real owner production receipt after deployment: `POST /api/settings/run-brief` (`2026-03-29T15:55:42.099Z`) returned `200/ok=true`, reused action `2e3a92ac-f93e-42b4-a978-bedd3dcee4d6` (`pending_approval`, `send_message`, confidence `76`), and preserved top-5 candidate discovery in `generation_log`. |
