...
| Directive generation uses Sonnet | BUILT | April 1: `lib/briefing/generator.ts` `GENERATION_MODEL` = `claude-sonnet-4-20250514`. `SYSTEM_PROMPT` = FOLDERA CONVICTION ENGINE (analyst-grade finished-artifact bar, causal diagnosis, evidence rules, JSON-only contract; **MANDATORY EMAIL PATH** forbids `do_nothing`/`wait_rationale` except **DECAY_RECONNECTION EXCEPTION** when the user prompt includes `DECAY_RECONNECTION_RULE` and context is ungrounded). `lib/briefing/__tests__/pipeline-receipt.test.ts` Anthropic mock branches on `FOLDERA CONVICTION ENGINE`. **Decay isolation (April 1):** for `discrepancyClass === 'decay'`, generator skips `runConvictionEngine` (avoids financial-runway goal bleed) and `researchWinner`; `fetchWinnerSignalEvidence` skips title-keyword expansion, runs a **deep entity-targeted scan** for decay (up to 500 processed signals over `SIGNAL_RETENTION_DAYS`, matching entity emails from `relationshipContext` and/or name tokens), then filters snippets to entity anchors; non-decay still uses the shallow 30-row entity fallback when `snippets.length < 8`. `buildDecayRecipientBriefFromEntity` fills `recipient_brief` when the standard parser fails but the entity row has `<email>`; relationship timeline uses `entityName` when present. **Prompt wiring (April 1):** decay recipient-short path now includes `CANDIDATE_ANALYSIS` (scorer breakdown), `ENTITY_ANALYSIS` (bx_stats when present), full `CANDIDATE_TITLE`/`CANDIDATE_EVIDENCE`, `CONTEXT_ENRICHMENT`, `AVOIDANCE_SIGNALS`, `BEHAVIORAL_MIRROR`, and full `SUPPORTING_SIGNALS`; decay evidence fetch adds entity-scoped `response_pattern` signals; `buildTriggerContextBlock` emits `timeframe` plus formatted `discrepancyEvidence` delta metrics; scorer copies `Discrepancy.evidence` onto `ScoredLoop.discrepancyEvidence`; `relatedSignals` contributes up to 5 lines to `candidate_reason`; `enrichWinnerWithEntityBxStats` backfills bx_stats when hydration short-circuited; decay runs log full user prompt with `[generator] FULL_PROMPT_DECAY`. Unit: `multi-user-safety.test.ts`, `trigger-action-lock.test.ts`. |
