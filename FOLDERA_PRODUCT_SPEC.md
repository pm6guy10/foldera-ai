...
| Real directive email (not just wait_rationale) | YELLOW | Directives generate and send. March 24: generator oriented around behavioral gap analysis. **April 1, 2026:** `lib/briefing/generator.ts` `SYSTEM_PROMPT` reframed as **strategic partner** (name the cross-signal pattern, explain why-now with user history, deliver finished work; voice rules + quality examples; explicit lead for `behavioral_pattern` winners). JSON output schema and validation unchanged. March 28: discrepancy detection live — scorer_ev 4.37, all top 3 candidates are discrepancies. Generator freshness gate blocks them (stale evidence rejection). Fix in progress. | Generator freshness exemption for discrepancy type |
...
| Directive JSON validation retries (Sonnet) | BUILT | April 2, 2026: `generatePayload()` in `lib/briefing/generator.ts` caps validation retries at **2** after the first attempt (`MAX_DIRECTIVE_VALIDATION_RETRIES = 2` → 3 Sonnet calls max per candidate). No fourth attempt; final failure logs `generation_validation_exhausted` and returns `payload: null` (empty directive path). `directive_retry` `api_usage` rows are bounded per invocation accordingly. |
