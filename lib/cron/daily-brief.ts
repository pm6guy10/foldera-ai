...
export async function runDailySend(
  ...
  for (const userId of eligibleUserIds) {
    try {
      ...
      // Guard: if any action today was already emailed (daily_brief_sent_at set), do not send again.
      // reconcilePendingApprovalQueue suppresses do_nothing rows to 'skipped', destroying the sent-at
      // proof on the original row. We must search across all statuses to catch this case.
