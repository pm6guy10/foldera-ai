...
export function buildDeterministicWorkPacket(input: {
  ...
  return {
    ...
    triggering_reason: `Multiple consented fixture signals indicate "${input.workday_state.current_focus}" needs one reviewable next move.`,
    ...
    prepared_work: `Review "${input.workday_state.current_focus}" using ${signals.length} source-backed signals, then decide whether to move forward with: ${input.workday_state.next_move}`,
