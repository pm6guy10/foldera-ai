  ...
  it('scorer says write_document, LLM returns send_message first → retry with write_document → persisted write_document', async () => {
    ...
    anthropicResponses = [
      ...
      },
      {
        ...
        artifact: {
          document_purpose: `FOIL appeal admin action packet before ${FOIL_DEADLINE_LABEL}`,
          target_reader: 'Brandon Kapp',
          title: 'FOIL appeal filing action packet',
          content: `Source Email: Steven Goulden requested the FOIL-2025-025-00440 appeal response before ${FOIL_DEADLINE_LABEL}. Admin action: confirm the appeal acceptance path, name the legal response owner, and attach the signed filing packet. Deadline: 4 PM PT today, ahead of ${FOIL_FILING_LABEL}. Risk: if unresolved, the statutory appeal window can slip before filing. Exact message: I am confirming the appeal packet, review owner, and filing path for FOIL-2025-025-00440. Next action: save the signed packet, send the confirmation, and mark the appeal filing owner closed.`,
        },
...
describe('TEST B — Hostile false-positive render (blocked payload)', () => {
  ...
  it('stale evidence + polished LLM artifact → generation fails closed', async () => {
    ...
    // Positive winner contract must block stale evidence before any LLM/model-backed
    // render attempt can turn it into a polished false-positive artifact.
    expect(mockLogStructuredEvent).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    anthropicResponses = [
      {
        ...
        artifact_type: 'write_document',
        artifact: {
          title: `FOIL appeal filing action packet — FOIL-2025-025-00440`,
          document_purpose: 'FOIL appeal admin action packet for filing deadline',
          target_reader: 'Brandon Kapp',
          content: `Source Email: Steven Goulden sent the FOIL-2025-025-00440 denial and appeal deadline. Admin action: submit the appeal acceptance confirmation and assign the accountable review owner. Deadline: 4 PM PT today, before ${FOIL_DEADLINE_LABEL}. Risk: missing the deadline can pause or forfeit the appeal review path. Exact message: I am confirming the appeal packet, review owner, and filing path for FOIL-2025-025-00440. Next action: save the packet, send the confirmation, and mark the FOIL filing owner closed.`,
        },
