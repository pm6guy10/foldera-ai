...
**Calibration shipped this branch (`claude/verify-516-verdict-fix-3jqye5`, grounded not loosened):** made `missing_schedule_resolution_context` (`lib/briefing/artifact-taste-pack.ts`) grounding-aware — it now only fires when `currentnessDays == null`, mirroring the sibling `missing_current_artifact_anchor` gate. A #516-grounded "commitment due, nothing scheduled" card can now ship; undated/hollow ones stay blocked. Proof: `positive-winner-contract.test.ts` before/after (undated blocked, dated clears) + winner-selection + daily-utility-slate + free-artifact-allowance + hunt-grounding-516 green; typecheck + gate:continuity clean. **Deliberately NOT touched** (would be blind loosening, the #452 mistake): goal-drift `missing_current_artifact_anchor` (already grounding-aware; live candidate is hollow) and `discrepancy-card-frame.ts` `weak_risk`/`reminder_without_risk` (`WINNER_TRACE_ROOT_CAUSE.md` says it is correct).
...
2. **Next gate (separate, with proof):** the freshest live blocker is the generator emitting multi-sentence directives (`directive must be exactly one sentence`) on commitment-exposure candidates — a generator/prompt fix, not a taste-gate change. Needs a deterministic generator-validation repro (no paid loop) before touching.
