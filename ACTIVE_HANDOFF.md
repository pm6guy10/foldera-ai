...
- **#567 PARADIGM SWAPPED — stored goal inference was the WRONG primitive. Do NOT rebuild it.** A live head-to-head (2026-06-29, PR #584) proved ranking against a STATED stable objective + live evidence beats the stored `tkg_goals` model — which had rotted to 82d-frozen job-hunting goals + n-gram garbage (engine went SAFE_SILENCE ×5 / its #1 pick was `$346.61` homework over the Foldera infra item).
- **The fix is shipped, reversible, flag-gated:** `lib/briefing/stable-objective.ts` + `lib/briefing/scorer-goal-source.ts`, wired into the scorer chokepoint behind `FOLDERA_GOAL_SOURCE=stated` (default `database` = no behavior change). NEXT = a live smoke run on the owner, then flip `FOLDERA_GOAL_SOURCE=stated`. Ceiling proven on live rows: 14d of inbox = 100% newsletters/gig-spam/bills, zero human/sales mail → even the right anchor plays defense until offline/repo work becomes signal.
- **Deprecation is recorded, not yet executed:** `tkg_goals` is load-bearing across the scorer + generator; the goal-refresh cron (`lib/cron/goal-refresh.ts`) still writes it. Ripping it is a later scoped seam, not a chat edit.
