...
- **PR #593 (out-of-band, unrelated to #567) — `app/globals.css` token-system consolidation, OPEN as draft.** Ad hoc owner request, not part of the #567 seam below; deleted the broken `--foldera-*` block (duplicate/mislabeled colors) and the fully-dead `--ld-*` system, leaving only the live `.fd` token system `components/foldera/LandingPage.tsx` renders; collapsed the value-identical `border-radius`/`font-size` literals onto existing unused tokens. Build/typecheck/landing-spec-passing + before/after screenshots are the receipt on issue #594. Does not touch #567/workday-presence code at all — listed here only to satisfy write-back; #567 remains the active foundation seam below.
