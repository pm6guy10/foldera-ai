...
Active implementation seam is issue #391 — REVERTING the daily-cron approach (it contradicted #369, which closed the daily `morning-pipeline` guardian floor as WRONG PATH: a daily cron is a daily brief, not an as-needed guardian). PR #392 wired it; this branch reverts the 4th stage. **Runtime proof WAS obtained a better way:** a one-time owner-go live fire of the already-deployed `/api/cron/workday-presence-trigger-runner` produced the **first-ever grounded `workday_presence_trigger` receipt** (`9695a5c6`, `ev_count=1` — real commitment evidence; status `pending_approval`) + a **live owner Slack ping** (`ok, mode:live`). The grounded path works end-to-end to the receipt. **Deeper gap now located:** the card the guardian actually sent read *"Nothing prepared to act on yet — staying quiet"* with no action — the brain grounds the trigger but does NOT prepare a reviewable move/draft. Also commitment-data hygiene: selected title `SYNC:payment_financial:Pay_Supabase_invoice` is malformed. Cadence fix = the 15-min EXTERNAL cron (owner setup), never a Vercel daily cron ([[feedback_interrupt_as_needed_not_daily]]).
**LIVE (production `bf5d203`):** #382 frontend (PRs #383–#389) + #390 owner self-review system (`docs/BRANDON.md` + `docs/EXPERT_PANEL.md` + Bible Part IV) + #392 (being reverted). #382 stays OPEN for optional polish.
**Next after #391:** the real next seam — make the guardian PREPARE a reviewable move (draft) from a grounded lapsing commitment, so the card/ping is actionable instead of "nothing to act on yet"; clean commitment titles; owner configures the external 15-min cron for as-needed cadence. Plus: recolor legacy cyan favicons; triage 44 stale branches + 24 skipped tests.
