...
Issue #481 is the active exec-function go/no-go seam.

**#481 hail-mary (this session):** built the single end-to-end swoop on real owner data (delivered as a local file) → answer = **DON'T GIVE UP**. The grounding blocker — which *refines* the `positive_winner_contract` lever below — is two concrete layers, neither intelligence: (1) full-body source hydration is interview-class-only → other high-consequence winners ground on ~160-char summaries; (2) **~35% of signals / 96% of `uploaded_document` fail to decrypt and are silently dropped** from grounding (`usedFallback -> continue`), source/format-skewed not a date key-rotation. Forensic tooling shipped (`scripts/swoop-481-*.ts`); the hydration generalization was built + unit-proven then reverted (flips a 12-case runtime contract; quality proof needs a paid gen run). Owner-gated next: (a) verify the decrypt gap in prod — highest leverage; (b) one paid gen cycle on the generalized-hydration change. See `[[project_grounding_rootcause_481]]` + `#481` comment.
