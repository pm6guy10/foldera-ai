...
5. **Stage 4 (same seam, next):** phone-first delivery of the Stage 3 proposals (reuse Slack/email rails + Twilio SMS deep link; PWA + Web Push later). `TWILIO_*` secrets are owner-gated.

Full staged design: GitHub issue #486 and the approved session plan. Stages 4–5 (phone-first delivery, hardening) follow on the same seam.
