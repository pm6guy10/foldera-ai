...
6. **Stage 5 — BUILT (this branch), inert behind flags, zero behavior change.** Owner decision (2026-06-20): delivery is **Slack-first, NO texts** — the Twilio/SMS rail (`lib/scout/sms.ts`) is removed; `delivery.ts` now fans out Slack (full review-gated card) + email (opt-in fallback), each self-skipping when unset. **Proof contract tightened** (AGENTS.md Proof Doctrine + `.foldera-contract.json`): every Scout phase needs **real product proof — a live Slack card — or NO PASS**; hygiene alone never passes. **Free real proof posted** this phase: the actual `buildScoutSlackMessage` output rendered to a real card in `#foldera-self-loop` (no paid loop). Proof: scout suite 46 + typecheck + lint(0) + gate:continuity + build. Live app-adapter path stays owner-gated (`SCOUT_DELIVERY_ENABLED` + `FOLDERA_SLACK_SELF_CHANNEL_ID` already in prod + Stage 1-3 flags, then trigger the deliver route).
7. **Stage 6 (same seam, next):** hardening — budgets, dedup, delta re-index, fixture tests. PWA + Web Push later.

Full staged design: GitHub issue #486 and the approved session plan. Stage 6 (hardening) follows on the same seam.
