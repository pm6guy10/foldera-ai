...
**GEM-CARD HYGIENE:** the lapsing-trigger directive (`lib/workday-presence/triggers.ts`) leaked the raw `SYNC:<category>:<slug>` dedup key (live row showed "Commitment lapsing: SYNC:payment_financial:Pay_Supabase_invoice") — #399/#400 cleaned the brief card via commitment-bridge but not this path. Fix prefers the clean `humanCommitmentTitle` when `summary` is a raw SYNC key. **MERGED via PR #457 (main `c686812`).**

**OVERDUE-ADMISSION WINDOW (this slice):** the scorer dropped commitments overdue by >30d before scoring — silently killing a real overdue money/legal/work gem (the job-offer response lapsed ~5 weeks). `overdueAdmissionWindowDays` (lib/briefing/scorer.ts) widens the window to 60d for high-consequence rows (risk_score ≥ 60); low/moderate keep 30d. Fail-safe: only widens ADMISSION — the soft staleness/timing penalty still discounts the very stale at ranking. 816/816 briefing suite + typecheck green.
...
3. Code follow-ups: the >30d-overdue drop is now FIXED for high-consequence rows (this slice). Remaining harness-only seam: the **two-way test** (a relationship gem requires the user actually replied ≥1×) — coupled to #454's admission decision. NO paid API calls from here — prove in the harness.
