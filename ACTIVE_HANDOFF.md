...
- **Phase A shipped on PR #585 (draft, CI pending).** Root cause: the `goal_primacy_gate` blanket-exempts ALL `discrepancy` type, and the detector emits `exposure`/`behavioral_pattern` discrepancies off any personal commitment (e.g. *"make a payment to bring account under credit limit"*) → homework rode the exemption past the objective. Fix (`discrepancyBypassesGoalPrimacy` in `lib/briefing/scorer.ts`): in `stated` mode a no-goal discrepancy keeps its bypass only if it's a relationship/outcome class; personal/admin classes must match the objective or go SAFE_SILENCE. Default mode byte-for-byte unchanged.
- **Phase B shipped on the same PR #585 (same push).** (1) `lib/workday-presence/proactive-delivery.ts` (new) — a heartbeat-seeded, draft-backed scored_winner now proactively posts to Slack via the SAME `buildRightNowMessagePayload`→`buildSlackRightNowMessage`→`postMessage` pipeline the manual "Post to Slack" button already used (#394 finished-work gate enforced for free; dedup by winner content, not timestamp). Wired into `deliverWorkdayPresence`, heartbeat-only. (2) Cron de-hinge: `morning-pipeline` reordered so `nightly_ops` runs LAST instead of first — card-delivery stages always get the full 300s budget now; fixes the live-observed failure where `nightly_ops` alone hit >300s and the scorer never ran for a whole day. NEXT = owner reviews #585 diff + one live heartbeat run to confirm a real objective card lands in `#foldera-self-loop` end-to-end (owner separately confirmed canvas is the accepted doc-attachment shape via a manual Slack test).
