...
**Shipped this branch (`claude/continue-work-bo69h2`) — #518 next-gate item #2, generator fix not a gate change:** deterministic **directive one-sentence salvage** (`lib/briefing/generator.ts` → `applyDirectiveOneSentenceSalvage`, wired into `validateGeneratedArtifact` before the one-sentence check). The model appends an explanatory 2nd sentence to commitment-exposure directives → `directive must be exactly one sentence` fails the generation AND persistence gates, burns every paid LLM retry, ships nothing (dark verdict). Salvage collapses to the leading imperative sentence (pure truncation of the model's own text; declines when the lead is a weak demonstrative so it never promotes context into the move). Proof (no paid loop): `directive-one-sentence-salvage.test.ts` (repro 2-sentence fails `countSentences!==1`, fix→`==1`, email dots preserved, weak-lead declined) + full `lib/briefing` suite 836 green + `gate:continuity`. Prompt and taste-gates untouched.
