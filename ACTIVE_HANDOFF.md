...
- **But the swap alone still served homework — fix in flight (PR #585, draft).** That live run's winner was *"make a payment to bring account under credit limit"* (personal finance). Root cause: the `goal_primacy_gate` blanket-exempts ALL `discrepancy` type, and the detector emits `exposure`/`behavioral_pattern` discrepancies off any personal commitment → homework rode the exemption past the objective. Fix (`discrepancyBypassesGoalPrimacy` in `lib/briefing/scorer.ts`): in `stated` mode a no-goal discrepancy keeps its bypass only if it's a relationship/outcome class; personal/admin classes must match the objective or go SAFE_SILENCE. Default mode byte-for-byte unchanged. NEXT on #585 = owner reviews diff + one live `seed-from-scorer` run to confirm the homework drops at `goal_primacy_gate`.
- **Phase B (named, not started) — the card still isn't IN Slack.** A heartbeat-seeded proactive winner never posts: `trigger-runner` only fires on a fresh inbound trigger or hidden-op, so a draft-backed objective winner sits in state (last real Slack card 06-29). Phase B = proactive delivery for a draft-backed objective winner (respect #394 finished-work gate) + make provider-push the hinge and fix the `morning-pipeline` `nightly_ops` 300s timeout (it 504'd today before the scorer ran — cron must be backup, not the hinge).
