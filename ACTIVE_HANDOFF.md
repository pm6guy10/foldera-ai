...
7. **Stage 5 — OPEN as draft PR #491** (Slack-first delivery + Twilio removed + Scout retargeted to the Guardian frame + proof-contract tightened). Hygiene green; live Slack-card proof owner-gated. Owner's call whether it merges.
8. **Compass — issue #492 opened (DO NOT RUN).** The 2026-06-20 session re-grounded the lane: the Scout's outside-in opportunity-hunting (grants/jobs/customers) was tone-deaf. #492 captures what Foldera actually is vs. should be and the one money-move the math says — **stop adding brain/breadth; turn the hands on for ONE real loop on REAL data until "can't-live-without-it 3×/week."** It is a compass, not a seam; promote it explicitly before any work.

Full staged design: GitHub issue #486. The real next direction (when promoted) is #492's §3 — one real act, one real source, one real loop — not Stage 6 breadth.
