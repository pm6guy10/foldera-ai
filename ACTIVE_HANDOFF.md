...
Between rungs — no active implementation seam. `active_issue: none` is the valid control-plane form; the owner names the next seam. The live product thread is the GUARDIAN (workday-presence): it now fires, grounds a real commitment, and pings Slack ONLY when it has a reviewable move — see `FOLDERA_MASTER_BIBLE.md` Part II-B + the heartbeat memory for the full architecture map.
...
None — between rungs after #394 (guardian pings only finished work, PR #395 `d235cde`). Awaiting the owner's next chosen seam.
