...
**LIVE (production `05ba0df`):** PRs #383–#388 — amber cohesion + Bricolage/Inter/JetBrains type + tokens; premium shared footer; premium `/pricing`; landing + content show-don't-tell; landing "show the chaos" connector cards.
**STAGED (branch `feat/382-pixel-polish`, not yet pushed):** owner pixel-polish pass — killed residual cyan (the brand glyph `public/foldera-glyph.svg` was still cyan → now amber; +globals.css/colors.ts stragglers); demo sidebar logo now a real `<FolderaMark>` linked home (was a non-clickable generic Folder icon); reduced amber overload (doctrine/trust/enterprise icons neutralized, only "One move" stays amber) + killed the Sparkles AI-cliché; de-blurred the "filing clerk" float (removed card rotation + `will-change`); wider 1240 layout + bigger hero lead (now states the Slack/finished-work mechanism); connector logos in chips at full opacity; tightened section rhythm; legible hero text-fade. Build + lint + tsc(app) + gate:continuity green.
**Next:** push branch + open PR for the polish pass; recolor raster favicons (`app/favicon.ico`, `public/favicon.png`, `public/brand/icon-watermark.png` may still be cyan — binary recolor); Phase 4 content/legal → Phase 5 product; collapse landing header + `NavPublic` into one shared `SiteHeader`.
