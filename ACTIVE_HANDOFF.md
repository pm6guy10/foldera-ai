...
**#481 (exec-function go/no-go) cracked + merged (PR #482):** built the single end-to-end swoop on real owner data (delivered as a local file) → **DON'T GIVE UP**. The grounding blocker (refines the `positive_winner_contract` lever) is now MEASURED into two layers, neither intelligence: (1) full-body source hydration is interview-class-only → other high-consequence winners ground on ~160-char summaries; (2) **35% of signals fail to decrypt and are silently dropped** from grounding (`usedFallback -> continue`), split by the classifier into a **KEY GAP** (24% — valid AES-GCM the local key can't auth → verify prod `ENCRYPTION_KEY_LEGACY`) and a **FORMAT GAP** (11%, incl. **357 `uploaded_document`** rows stored PLAINTEXT and dropped though readable — prod-real, **code-fixable**). The richest source (uploaded docs) is the worst hit. Owner-gated next (value order): (b) code-fix the format gap → (a) prod key verification → (c) generalize hydration → (d) ONE paid gen cycle AFTER inputs un-starved. Tools: `scripts/swoop-481-*.ts`. See `[[project_grounding_rootcause_481]]` + `#481` comment.
