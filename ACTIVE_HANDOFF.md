...
2. The gem-ranking stakes-floor (above) is staged on `claude/gem-ranking-trust-aware` (814/814 briefing suite green, typecheck clean). Owner validation: run one generation cycle and confirm a real gem now clears the bar instead of "nothing cleared the bar."
3. Known remaining limit (follow-up, not done): commitments >30d overdue are dropped at scorer admission (line ~5077), so a 35-day-stale job-offer gem still dies there — a high-consequence overdue window extension is a separate seam. The robust noise follow-up to #454 stays the **two-way test** (a relationship gem requires the user actually replied ≥1×). NO paid API calls from here — prove in the harness.
