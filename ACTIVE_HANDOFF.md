...
Active implementation seam is issue #394 — guardian pings only finished work (NO new paid LLM). Two no-paid fixes: (1) gate the Slack send in `runWorkdayPresenceTriggerRunner` — when the intervention payload `mode` is `silent`/`dismissed`, do NOT send Slack (record quiet); stops the "nothing prepared to act on yet" noise ping (the live fire sent one). (2) seed a real presence `draft` from the latest already-generated daily-brief artifact in `tkg_actions` (keys title/content/type → `WorkdayPresenceDraft`, pure mapping; the brief already paid for it) attached to a draftless `scored_winner` before evaluate. Owner directive: prove with deterministic fixtures only — NO paid generation, NO live fire. Background: #391 live fire proved the grounded `workday_presence_trigger` receipt (`9695a5c6`, ev_count=1) + live Slack ping; the gap is draft-prep (`seed-from-scorer`/`generateArtifact`) only runs on dashboard load, not the event path; the card-mechanism (`directiveToPresenceState`→`draftFromArtifact`→`buildRightNowCard`) already works when a draft exists.
**LIVE (production `7b573eb`):** #382 frontend (PRs #383–#389) + #390 self-review system + #391/#393 guardian fires-grounded-and-pings (daily-cron reverted, honoring #369). #382 stays OPEN.
**Next after #394:** owner configures the external 15-min cron for as-needed cadence (the ongoing firing mechanism — not a Vercel daily cron); clean malformed commitment titles (`SYNC:payment_financial:…`); recolor legacy cyan favicons; triage 44 stale branches + 24 skipped tests. gitlab-handbook moved out of repo → `C:/Users/b-kap/foldera-reference/`.
