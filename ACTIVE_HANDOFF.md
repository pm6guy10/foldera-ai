...
4. **Stage 3 (same seam, next):** the proactive scout loop in `lib/scout/scout-loop.ts` (infer goal → web_search → retrieve Drive materials → reuse the generator writer for finished, review-gated artifacts).
