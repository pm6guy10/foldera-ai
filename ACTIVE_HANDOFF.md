...
Last updated: 2026-06-19 UTC (active seam: #445 Master Audit — passes 0/1/2/3 + D-3 + F-1 + gem-surfacing revert #453 merged to main `27df0c2`; gem-ranking #456 + lapsing-card hygiene #457 merged to `c686812`; **Pass 4 (Backend/runtime) shipped (verdict CONCERN)**; gem-surfacing slice 3 = overdue-admission window PR #460; broadcast-sender suppression staged as DRAFT PR #454 awaiting owner live validation. NOTE: parallel-agent collision occurred — Pass 4 #458 + overdue #460 both rewrote the control plane; resolved by hand.)
...
**OVERDUE-ADMISSION WINDOW (gem-surfacing slice 3, PR #460):** the scorer dropped commitments overdue by >30d before scoring — silently killing a real overdue money/legal/work gem (the job-offer response lapsed ~5 weeks). `overdueAdmissionWindowDays` (lib/briefing/scorer.ts) widens the window to 60d for high-consequence rows (risk_score ≥ 60); low/moderate keep 30d. Fail-safe: only widens ADMISSION — the soft staleness/timing penalty still discounts the very stale at ranking. 816/816 briefing suite + typecheck green.
...
3. Code follow-ups: the >30d-overdue drop is now FIXED for high-consequence rows (PR #460). Remaining harness-only seam: the **two-way test** (a relationship gem requires the user actually replied ≥1×) — coupled to #454's admission decision. NO paid API calls from here — prove in the harness.
