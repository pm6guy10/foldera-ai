...
Issue #481 is the active grounding format-gap seam.

**#481 FORMAT GAP — FIXED (this seam).** Grounding silently dropped readable source: `decryptWithStatus` returns `usedFallback=true` for BOTH undecryptable ciphertext AND legacy **plaintext** rows, and every evidence loader dropped on `usedFallback` alone — discarding **633 readable rows (11%), incl. 357 of 390 `uploaded_document`** (resumes/packets/filings — the richest source). Fix: shared `looksLikeEncryptedPayload` discriminator (`lib/encryption.ts`) gates the drop so only true ciphertext is skipped; plaintext flows into grounding. Applied at `fetchWinnerSignalEvidence` + full-body hydration (`generator.ts`) + `entity-trust-repair.ts`. **Additive** (can only add real source). Proof: `lib/__tests__/encryption.test.ts` + 989 briefing/signals/autopsy tests + typecheck + build green. NO paid run, NO prod access needed.
**Prior #481:** swoop landed → **DON'T GIVE UP** (PR #482); classifier (PR #483) split the 35% decrypt drop into this FORMAT GAP + a **KEY GAP** (24%, valid AES-GCM the local key can't auth → verify prod `ENCRYPTION_KEY_LEGACY`). Remaining seams: (c) generalize full-body hydration beyond interview-class (deferred — incoherent runtime fixtures + needs paid validation); (a) prod key check; (d) one paid gen cycle on un-starved inputs. See `[[project_grounding_rootcause_481]]`.
**Merged + LIVE — #478 commitment date anchoring (PR #479):** `insertCommitment` set `made_at = now()` (extraction time) not the source `occurred_at`, so a year-old "send filings to counsel" got a fabricated fresh deadline (74-82). Fixed + backfilled live: recent real obligations lead (declined payment $346.61 @ 2026-06-17 = 88 #1); year-old legal filings collapse to 17-25.
