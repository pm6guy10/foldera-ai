...
- **#567 vision-gap slice MERGED+LIVE (PR #610, `770580c`): conviction footer + judgment ratchet + all-clear closure.** Owner shipped two vision docs ("borrowed executive function"); exploration proved the gap was intelligence computed then DISCARDED before the user surface. Three commits: (A) card footer now says what it was ranked against and what it beat (`conviction_line` from scorer diagnostics, replaces bare coverage count, UUID-guarded); (B) Dismiss (Slack + dashboard) now skips the winner's REAL backing row via `executeAction` (`feedback_weight -0.5`, `skip_reason slack_dismiss/dashboard_dismiss`) so the scorer's behavioral rate finally learns — the old `presence_action` receipts were type-inert (1,204 skips taught nothing, issue #592); (C) quiet dashboard backs safe_silence with `pipeline_runs` proof: "All clear. Checked N open loops at <time> — nothing needs you." No migrations, no gate loosened, silence still never pushed. **NOT YET live-verified** — next real pipeline run + a real Slack dismiss + a real quiet day should confirm all three surfaces.
- **KEYSTONE (#584 MERGED+LIVE): rank against the STATED objective + live evidence — do NOT rebuild stored goal inference** (`FOLDERA_GOAL_SOURCE=stated` in prod; dead alternative, SETTLED #9).
- **#606 MERGED+LIVE (PR #607) but NOT YET live-verified:** bare-UUID false positive in the pre-LLM candidate gate silently killed real R1 candidates ~30h; next real pipeline run must confirm the two stuck commitments (due 07-05, 07-07) now clear.
- **All #567 follow-ons MERGED+LIVE on main** (PRs #584-#588, #591, #607): Phase A goal-primacy, Phase B proactive Slack delivery, stale-state freshness guard, budget segmentation ×2 (cron + push), decision-lock card collapse. History: `docs/archive/SESSION_HISTORY.md`.
- **Deprecation recorded, not executed:** `tkg_goals` still load-bearing; goal-refresh cron still writes it. Pool hygiene (88 commitments overdue 30d+) is a smaller parallel seam.
- **Skip-signal hygiene backfill (issue #592, live-verified 2026-07-02):** live DB check corrected the record — the "1,204 type-inert `presence_action` skips" claim was wrong (only 15 such rows exist; the real 1,205 owner-scoped `skipped` rows already had real action_types and mostly already fed `computeCandidateScore`'s `behavioral_rate`). The actual waste: ~1,052 of those rows were pipeline self-log/automation noise (`do_nothing` "Nothing cleared the bar today"/`__GENERATION_FAILED__` placeholders, auto-suppression, dev/test artifacts) counted as if the user rejected that action type. Quarantined (`feedback_weight=0`); ~30 genuine explicit-reason rows + real-directive null-reason rows left untouched. `scripts/backfill-skip-signal-hygiene.sql` has the exact rule set.
...
**Vision-gap seam MERGED (PR #610, `770580c`) — make the shipped card match the vision docs.** The scorer already computed the conviction case (selection margin, runner-up kill reasons, matched objective) and dropped it; the dismissal loop was one-way (type-inert `presence_action` rows); safe_silence rendered as literal absence on the pull surface.

- What shipped (3 commits, squashed into `770580c`): `conviction_line` (scorer diag → `workday_presence_state` → Slack footer + dashboard, replaces coverage count when provable, falls back otherwise); `recordDismissJudgment` (`lib/workday-presence/judgment-feedback.ts`) wired into the Slack interaction + dashboard message-action dismiss paths; `loadAllClearEvidence` (`lib/workday-presence/all-clear.ts`) + `all_clear` response key + dashboard "All clear." render.
- Proof: 314 tests green across touched areas (39 files); lint, build, large-file-splits, gate:continuity green. Pre-existing failure (also on clean main, env-shaped): `app/api/workday-presence/seed-from-scorer/__tests__/route.test.ts` (8 tests, 500-vs-200). Branch `claude/closing-the-gap-95r3z9` deleted post-merge.
- **Follow-up (this PR, 2026-07-02): historical skip signal backfilled.** Live DB check (owner directive: "go retroactive, past skips need to teach the scorer") found the prior "1,204 type-inert `presence_action`" characterization was wrong — see TL;DR/DON'T-FORGET corrections above. Ran `scripts/backfill-skip-signal-hygiene.sql` against production: quarantined ~1,052 automation/self-log rows (`feedback_weight=0`), reclassified the 1 recoverable `presence_action` row. Live schema check also confirmed the `skip_reason`/`action_type` CHECK constraints already accept `slack_dismiss`/`dashboard_dismiss`/`presence_action` on the live DB (local migration files are stale — no fix needed there).

## Next exact move

1. **Live-prove PR #610's three surfaces (schema confirmed unblocked, no real dismiss event yet):** one real seeded card must render the conviction footer; one real Dismiss must flip its backing `tkg_actions` row to `skipped` with `skip_reason='slack_dismiss'`; one quiet day must render the all-clear on the dashboard.
2. **Live-verify #606:** trigger the next real pipeline run and re-query `pipeline_runs.raw_extras` + `workday_presence_suppression_trace` for `2cbc1bab-8e0e-43b0-bf4a-9a0cd6b5d91f` — confirm the two previously-stuck commitment candidates (due 2026-07-05, 2026-07-07) now produce a real card.
3. **Then #567 owner sign-offs** (tkg_goals re-ground + Gmail connector 1→967) and **#597** live dismissal-tap verification — note the judgment ratchet above gives #597 real teeth now. PR #593 (CSS cleanup) still open, low priority.
