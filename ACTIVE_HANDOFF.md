...
- **INCIDENT (2026-06-30, same day, fixed same day, PR in flight) — proactive-delivery's first live run posted a STALE pre-fix homework card.** `seedFromScorerForUser` only overwrites `workday_presence_state` on a SUCCESSFUL seed; the first post-merge heartbeat hit the daily manual-call-limit (an existing, intentional per-day brake on `skipSpendCap` calls — separate from the spend cap, resets daily) and never reached the scorer, so `workday_presence_state` stayed whatever it was at 15:03 UTC — the pre-Phase-A "make a payment to credit limit" winner. Proactive-delivery had no dedup history yet (first run ever) and delivered that stale state to the real `#foldera-self-loop` channel as if it were fresh. **Fix:** `evaluateProactiveDelivery` now requires `state.updated_at` to be within `STATE_FRESHNESS_WINDOW_MS` (10 min) of `nowIso` — stale leftover state is never treated as a fresh winner, regardless of cursor history. See LESSONS_LEARNED #34. NEXT = merge the freshness-fix PR, then a live heartbeat AFTER today's manual-call-limit resets (tomorrow, or `skipManualCallLimit`) to actually exercise Phase A+B end to end (owner separately confirmed canvas is the accepted doc-attachment shape via a manual Slack test).
