...
- **ISSUE #589 FIXED (PR pending) — "Decision lock" card collapsed to one `Source:` line + one paragraph.** Implemented the issue's drafted diff in `buildDecisionEnforcedFallbackPayload`'s `write_document` branch (`lib/briefing/generator.ts`); also found + fixed a gap in that draft — 3 of the 4 causal-mechanism asks didn't naturally carry the forcing-function/pressure/owner-assignment language the removed boilerplate lines guaranteed, so added narrow conditional guards. Tests updated; lib/briefing 912/912 green. NOT YET DONE (from the issue's own checklist): the `behavioral_pattern` write_document branch (5 paragraph-length lines) and LLM-generated multi-step cards (e.g. "Nathaniel's Birthday") weren't touched — same brevity review may apply there.
