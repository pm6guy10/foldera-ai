...
**What commit is live on www:** **`GET https://www.foldera.ai/api/health`** (prod origin) — field **`revision.git_sha`** / **`revision.git_sha_short`** — is the **source of truth**, not `git log -1` alone. **Docs-only** commits still deploy; the live SHA may be **newer** than the commit that shipped a feature (e.g. follow-up `SESSION_HISTORY` / punchlist pushes). Re-check health after every deploy you care about. Alias-race recovery: [docs/MASTER_PUNCHLIST.md](docs/MASTER_PUNCHLIST.md) **Production vs `main`**.
...
- **Fix (round 3, same day):** Run **#217** still failed — preflight ran **before** Git finished **BUILDING**, so CLI uploaded into quota. **`scripts/ci/vercel-deploy-preflight.py`**: wait **35s** when no row yet; if **in-flight** for this SHA, poll up to **~10m** for **READY**; **`check-ready`** after quota error → workflow **exit 0** if Git became **READY** during CLI attempt.
