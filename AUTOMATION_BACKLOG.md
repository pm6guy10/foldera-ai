...
| AB16 | All real goals have `current_priority=false`. Only 3 suppression goals (priority 1) have `current_priority=true`. Generator `buildUserIdentityContext()` may produce empty identity context. | NEW | AUTO_FIXABLE | WARN_IDENTITY_CONTEXT_STARVED | `tkg_goals`, `lib/briefing/generator.ts` | March 22 audit: 14 goals total. 11 real goals (priority 2-5) all `current_priority=false`. 3 suppressions (priority 1) are `true`. If generator only queries `current_priority=true`, identity context is suppression-only. | yes | Can fix via SQL update. | Set `current_priority=true` on priority >= 3 goals. |
