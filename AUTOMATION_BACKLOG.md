...
- **AZ-24 (2026-04-04, slice 2 — shipped)** — **`has_recent_evidence`** and DecisionPayload **`freshness_state` / stale age** use **`getNewestEvidenceTimestampMs`**: max past timestamp over **hydrated `supporting_signals`** ∪ **`winner.sourceSignals`** (same union family as thread gate). **Tests:** `thread-evidence-for-payload.test.ts`. **Left:** post-deploy re-query `scripts/az05-action-type-distribution.sql`; next levers — legacy **`research`** row drain, **`suggestedActionType: research`** in `scorer.ts` signal_velocity.
...
| 4 | **AZ-24** | Pipeline: actionable share vs `do_nothing` / `research` | Agent | §1.1 / matrix G | **Receipt:** 2026-04-03 MCP counts + CI `8739494` success + `test:prod` 61 (see DONE bullet). **Slice 1–2 shipped:** thread gate + freshness union (`getNewestEvidenceTimestampMs`). **Next:** post-push SQL needle check; research row drain / scorer `signal_velocity` | Re-query SQL after deploys; ensure Anthropic credits healthy for non-`research` paths |
