...
| AB4 | `make_decision` action type dominates scorer output. | OPEN (CLOSE-CANDIDATE) | MANUAL_REVIEW | WARN_MAKE_DECISION_DOMINANCE | `lib/briefing/scorer.ts` | March 23 nightly: 37/77 = 48% (down from 49%). Slow decline trend continues. Today's winner was `schedule` type. | no | Scorer excluded from auto-fix. | Close alongside AB1 if share drops below 40% by March 25. |
