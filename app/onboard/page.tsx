...
function OnboardContent() {
  ...
  const submit = async () => {
    ...
    setSubmitError(null);
    try {
      const buckets = isEdit ? Array.from(selected) : [];
      const response = await fetch('/api/onboard/set-goals', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ buckets, freeText: null, skipped: false }),
      });
      if (!response.ok) {
      ...
      <main id="main" className="mx-auto flex min-h-[100dvh] max-w-6xl flex-col justify-center px-4 py-10 sm:px-6 sm:py-14">
        <div className="mx-auto w-full max-w-3xl overflow-hidden rounded-[28px] border border-cyan-300/14 bg-[#050c14]/94 px-6 py-8 shadow-[0_24px_90px_rgba(0,0,0,0.44)] sm:px-10 sm:py-10">
          ...
          <h1 className="mt-6 text-4xl font-semibold tracking-[-0.055em] text-white sm:text-5xl">
            {isEdit ? 'Edit your focus' : 'Connect one source'}
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-text-secondary">
            {isEdit
              ? 'Update focus areas anytime.'
              : 'Connect Google or Microsoft so Foldera can check what is current, then the dashboard can show one useful move or an honest waiting state.'}
          </p>

          {!isEdit && (
            <section className="mt-8 border-t border-border-subtle pt-6">
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-text-secondary">Source check</p>
              <h2 className="mt-3 text-2xl font-black tracking-tight">Source connection</h2>
              ...
              {connectorReady ? (
                <p className="mt-4 border-l-2 border-accent-dim pl-3 text-xs font-bold text-accent-hover">
                  Source connected. Foldera can check it now.
                </p>
              ) : (
                <>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              type="button"
              onClick={() => submit()}
              disabled={saving || (isEdit ? selected.size === 0 : !connectorReady)}
