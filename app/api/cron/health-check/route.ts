...
export async function GET(request: Request) {
  ...
  try {
    ...
    const lines = [
      ...
      `Microsoft token: ${healthData.microsoft || 'unknown'}`,
      `Google token: ${healthData.google || 'unknown'}`,
      `Last directive: ${healthData.last_directive || 'unknown'}`,
      `Last directive age: ${healthData.last_directive_age_hours ?? 'unknown'} hours`,
      '',
      'Full response:',
