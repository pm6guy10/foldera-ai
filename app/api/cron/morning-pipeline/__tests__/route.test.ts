...
describe('morning-pipeline cron route', () => {
  ...
  it('runs all five cron stages in order and forwards cron auth', async () => {
    ...
    // Verify order: nightly → seed → trigger → maintenance → renew_graph
  ...
  it('isolates a thrown stage so downstream stages still run (one throw cannot drop the whole pipeline)', async () => {
    ...
    // trigger_runner, daily_maintenance, and renew_graph_subscriptions must still run.
