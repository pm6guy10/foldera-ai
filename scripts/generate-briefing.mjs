...
// Post-generation validation: stale context + placeholders
...
const PLACEHOLDER_RE_CHECK = /\$\[.*?\]|\$\{.*?\}|\[AMOUNT\]|\[NAME\]|\[DATE\]|\[TODO\]|INSERT\s+\w+\s+HERE|\[email@|\[Company\]/i;

const fullJson = JSON.stringify(parsed);
const hasStale = STALE_TERMS.test(fullJson);
const hasPlaceholderInOutput = PLACEHOLDER_RE_CHECK.test(fullJson);

if (hasStale || hasPlaceholderInOutput) {
  const violations = [];
  if (hasStale) violations.push('stale context references');
  if (hasPlaceholderInOutput) violations.push('unfilled placeholders');
  console.warn('[VALIDATION FAILED] ' + violations.join(', ') + ' — falling back to safe do_nothing');
  parsed = {
