...
**Rule:** when a DB/identity migration is executed, the same change usually has a **code half** (constants, hardcoded IDs, fixtures) that must move in lockstep — grep the literal across the whole repo (`rg <uuid>`), not just the env. And when something is parked as "owner-only / sandbox-cannot-do," sanity-check that it actually requires owner privileges; a surprising amount of "owner work" is a code edit the harness can do and prove. Two genuinely-distinct halves remain owner-only here: the **DB mutation** (done, #509) and the **Vercel env** (`FOLDERA_SELF_USER_ID`, hidden value, drives the self-loop cron) — but the constant was neither. Fixed 2026-06-23: `constants.ts` + `owner-user-id-boundary.test.ts` + the 4 `.mjs` debug scripts repointed to `2cbc1bab`. Corollary for #518: the dark verdict is **not** data-starvation (the account is rich) — it is gate calibration, which keeps the focus where the value is. Closed 2026-06-23: constant fix merged (#534) and `FOLDERA_SELF_USER_ID` set + redeployed — identity is now consolidated code + env.

## 26. Make "Where Are We" a Read, Not a Re-Derivation — Surface Live State at the Seam

The single biggest friction sink across sessions is **rehashing context**: which account is live, is the env set, did the PR merge, is the verdict still dark. Each re-derivation costs a Supabase query / Vercel check / git archaeology that we already did last session. The session that mis-filed `OWNER_USER_ID` (Lesson 25) lost *days* because the live truth wasn't surfaced where the next agent would look — it lived in a human's head as "owner-only."

**Rule — automate the answer at the seam, don't re-investigate it:**
- **Encode live facts as enforced state, not prose.** `ACTIVE_SEAM_STATE.json` + `npm run roll` + the SessionStart `cron:` line already do this for issue/branch/PR/cron-outcome. Extend the same pattern to any fact you find yourself re-deriving: if you queried it twice across sessions, it belongs in the rolled state or a boot line.
- **A divergence that's invisible until it produces a dark verdict deserves a parity assertion.** Code-constant vs env (`OWNER_USER_ID` vs `FOLDERA_SELF_USER_ID`) silently disagreed; the cheap permanent fix is a test/boot check that fails loud when they drift — so it can never become "spend a day finding it" again.
- **Lead the session with the rolled TL;DR; don't reconstruct it.** The boot order (handoff → active issue) plus an ≤8-line enforced TL;DR means the next agent *reads* state in under a minute instead of re-proving it. Keep that TL;DR honest at write-back (which half is done, which is owner-only, what's the one next score) — a stale TL;DR forces the re-derivation it was meant to kill.

Net: every fact you had to dig for is a candidate to roll forward. The score is forward momentum on **value (#518)**; context reconstruction is pure friction, and friction is automatable away at the gate/script/boot line.
