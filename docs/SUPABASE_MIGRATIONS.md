...
- **2026-05-05:** **`drop_safe_unused_advisor_indexes`** — applied repo migration [`supabase/migrations/20260505083000_drop_safe_unused_advisor_indexes.sql`](../supabase/migrations/20260505083000_drop_safe_unused_advisor_indexes.sql) to production via `npx supabase db query --linked -f` with the exact file contents. Dropped zero-scan, non-protective advisor indexes **`idx_system_health_failure`**, **`idx_ml_snapshots_outcome`**, **`idx_ml_priors_bucket`**, **`idx_user_brief_cycle_gates_last_cycle`**, and **`idx_tkg_signals_outcome_label`**. Preserved **`idx_ml_snapshots_user`** and **`idx_tkg_goals_entity_id`** despite unused-index INFO rows because they protect FK paths; dropping them would trade cosmetic INFO cleanup for possible unindexed-foreign-key regression. Linked performance advisor now reports only those two intentional DB exceptions plus the external Auth connection-strategy setting.
