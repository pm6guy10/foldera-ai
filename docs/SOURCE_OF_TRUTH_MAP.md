...
- `FOLDERA_MASTER_BIBLE.md` is the canonical plain-English build bible and reference authority for product understanding.
- `FOLDERA_MASTER_SYNTHESIS_DRAFT.md` remains `REFERENCE_DRAFT` source material and is superseded for master-bible reading.
...
| `REFERENCE_AUTHORITY` | Canonical reference authority that answers product/build questions but does not activate execution. |
...
| `FOLDERA_MASTER_BIBLE.md` | `REFERENCE_AUTHORITY` | Canonical plain-English master bible for product understanding, money path, proof model, and forbidden work. | Future source-truth/build-definition issues and operator sessions needing a repo-backed survival document. | Active seam selection, queue mutation, product/runtime changes, schema migrations, deployment changes, or unsupported claims. | Issue #181 and source-truth closeout / PR receipt. |
