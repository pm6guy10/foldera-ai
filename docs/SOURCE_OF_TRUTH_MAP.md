...
| GitHub issue #216 | `REFERENCE_ONLY` | Completed trust/privacy/no-send rail Product MVP seam (rung 4 COMPLETE via PR #218). | Agents and reviewers tracing the rung 4 closeout. | Active seam selection, product/runtime implementation, or queue activation by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #216 as COMPLETE. |
| GitHub issue #220 | `CURRENT_CONTROL` | Add bounded self-serve early-access payment path (rung 5 IN_PROGRESS). | Agents and reviewers checking the current product seam. | Product/runtime implementation, queue activation, or unrelated product widening by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #220 as the active Product MVP seam. |
