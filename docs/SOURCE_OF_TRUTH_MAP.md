...
- GitHub issue #173 `Promote first executable MVP rung from Master Synthesis` controls the active source-truth build-sequence promotion seam.
...
| GitHub issue #173 `Promote first executable MVP rung from Master Synthesis` | `CURRENT_CONTROL` | Defines the active source-truth build-sequence promotion seam that selects the first executable MVP rung from the Master Synthesis build bible. | The current assignee, reviewer, PR author, and source-truth gates for issue #173. | Product/runtime/frontend/backend work, schema/source-lane audit, Supabase migrations, Vercel changes, Slack / PR #142, Stripe, connectors, landing/dashboard/auth/backend, or treating the draft as implementation authority. | `npm run gate:command`, `npm run gate:continuity`, focused source-truth tests, PR changed-file review, and GitHub receipts. |
...
| `FOLDERA_MASTER_SYNTHESIS_DRAFT.md` | `REFERENCE_DRAFT` | Preserves the Master Synthesis build bible in repo as reference-only source material. | Future source-truth/build-definition issues when explicitly assigned. | Current implementation authority, active seam selection, product/runtime changes, schema migrations, deployment changes, or customer/enterprise claims. | Issue #173 source-truth gate checks `REFERENCE_DRAFT`, `READINESS VERDICT`, `build-bible ready as a reference draft`, and explicit `not implementation authority` language. |
