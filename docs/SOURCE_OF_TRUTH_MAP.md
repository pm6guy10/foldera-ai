...
- GitHub issue #220 is completed — rung 5 live proof passed (all 5 checks proven via Supabase MCP + Vercel MCP). Retained for receipt history.
- GitHub issue #231 is the active work-state purity seam (owner directive): earned entity trust, nightly demotion sweep for the contaminated graph, and acquisition-agent quarantine. Issue #226 resumes as the next seam after #231 is proven.
- GitHub issue #226 is paused — owner-path readiness: Brandon must reliably sign in and complete one Slack self-loop before non-owner paid loop proof (rung 7). It resumes after issue #231 is proven. Rung 7 non-owner loop is forbidden until #226 is proven.
...
| GitHub issue #231 | `CURRENT_CONTROL` | Active work-state purity seam — earned trust, clean graph, agent quarantine (owner directive). |
| GitHub issue #226 | `REFERENCE_ONLY` | Paused owner-path readiness seam — resumes after issue #231 is proven. Must not authorize non-owner proof or rung 7 work until proven. |
...
| GitHub issue #208 | `REFERENCE_ONLY` | Completed first user journey shell (rung 3) by PR #215. | Agents and reviewers tracing the rung 3 closeout. | Active seam selection, product/runtime implementation, or queue activation by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #208 as COMPLETE. |
| GitHub issue #216 | `REFERENCE_ONLY` | Completed trust/privacy/no-send rail (rung 4) by PR #218. | Agents and reviewers tracing the rung 4 closeout. | Active seam selection, product/runtime implementation, or queue activation by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #216 as COMPLETE. |
| GitHub issue #220 | `REFERENCE_ONLY` | Bounded self-serve payment path (rung 5 COMPLETE — live proof passed). | Agents and reviewers tracing the rung 5 closeout. | Active seam selection, product/runtime implementation, or queue activation by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #220 as completed. |
| GitHub issue #231 | `CURRENT_CONTROL` | Active work-state purity seam — earned trust (outbound evidence), nightly demotion sweep, agent quarantine (owner directive). | The current assignee, reviewers, and PR authors. | Non-owner proof, rung 7 work, Slack implementation, auth fixes, Stripe changes, schema migrations, new agents, new connectors, or broad cleanup. | ACTIVE_HANDOFF.md and FOLDERA_BUILD_ORDER.yaml active_issue = 231; launch_ladder rung 6 is IN_PROGRESS. |
| GitHub issue #226 | `REFERENCE_ONLY` | Paused owner-path readiness seam (sign-in + Slack self-loop); resumes after issue #231 is proven. Rung 7 non-owner paid loop is forbidden until #226 is proven. | Agents and reviewers planning the next seam. | Active seam selection while paused, non-owner proof, or rung 7 work. | FOLDERA_BUILD_ORDER.yaml paused_issues lists issue 226; launch_ladder rung 6b is PAUSED. |
...
| GitHub issue #168 | `REFERENCE_ONLY` | Completed automatic Open Threads capture seam retained for receipt history after PR #205. | Agents and reviewers tracing the command-os closeout. | Active seam selection, product/runtime implementation, or queue activation by itself. | FOLDERA_BUILD_ORDER.yaml marks issue #168 as closed/completed. |
