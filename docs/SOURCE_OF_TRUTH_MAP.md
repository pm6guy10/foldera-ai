...
| GitHub issue #194 | `CURRENT_CONTROL` | The first money-loop verdict-loop issue that follows the cleanup closeout. | The current assignee, reviewer, and PR author for the first money-loop seam. | Queue activation, product/runtime implementation outside the verdict loop, or unrelated cleanup. | Manual boot-sequence read plus PR scope review. |
