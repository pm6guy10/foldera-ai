...
- GitHub issue #170 `Foldera Master Synthesis Lock Pass - customer, deliverable, build spec, and issue ladder` controls the active source-truth build-definition seam.
- `FOLDERA_MASTER_SYNTHESIS_DRAFT.md` is `REFERENCE_DRAFT`: repo-contained source material only, not build-ready and not implementation authority.
...
| GitHub issue #170 `Foldera Master Synthesis Lock Pass - customer, deliverable, build spec, and issue ladder` | `CURRENT_CONTROL` | Defines the active source-truth build-definition seam for putting the Master Synthesis draft under repo control as a reference draft. | The current assignee, reviewer, PR author, and source-truth gates for issue #170. | Product/runtime work, Supabase migrations, Vercel changes, Slack / PR #142, Stripe, connectors, landing/dashboard/auth/backend, or treating the draft as build-ready. | `npm run gate:command`, `npm run gate:continuity`, focused source-truth tests, PR changed-file review, and GitHub receipts. |
| `FOLDERA_MASTER_SYNTHESIS_DRAFT.md` | `REFERENCE_DRAFT` | Preserves the current Master Synthesis manuscript in repo and defines the required next pass acceptance standard. | Future source-truth/build-definition issues when explicitly assigned. | Current implementation authority, build-ready spec status, active seam selection, product/runtime changes, schema migrations, deployment changes, or customer/enterprise claims. | Header verdict says not build-ready; issue #170 source-truth gate checks `REFERENCE_DRAFT`, `READINESS VERDICT - NOT BUILD-READY YET`, and `REQUIRED NEXT PASS`. |
