...
| `FOLDERA_MASTER_BIBLE.md` | `REFERENCE_ONLY` | Canonical plain-English master bible for Foldera's product, money path, build order, forbidden work, and Codex loop. | Future source-truth/build-definition issues when explicitly assigned. | Active seam selection, product/runtime implementation, queue advancement, or unsupported customer claims. | Source-truth gates and PR receipts must treat it as reference authority, not live control. |
