...
| `npx vitest run --exclude ".claude/worktrees/**"` | Pass — 55 files, **601** tests |
| `npm run build` | Pass |
| `npm run test:ci:e2e` | Pass — **39** tests after briefings E2E (`PLAYWRIGHT_WEB_PORT=3011`, `NEXTAUTH_URL=http://127.0.0.1:3011`) |
...
| D | Data / DB / migrations | **Yellow** | Migrations in repo; some production applies historically manual | `supabase/migrations/` | Agent |
...
| G | Generator / pipeline | **Yellow** | Large modules; quality gates layered; CE-1–CE-6 backlog | `generator.ts`, `scorer.ts`, AUTOMATION_BACKLOG CE | Agent |
...
| L | Logging / observability | **Yellow** | Sentry wired; correlation IDs still open | AUTOMATION_BACKLOG | Agent |
...
| Q | Quality gates / tests | **Green** | Lint + 599 vitest + CI e2e + 61 prod | This document § snapshot | Agent |
...
| U | UX polish / a11y | **Yellow** | Mobile prod layout tests; blog typography backlog | `mobile-prod-layout.spec.ts` | Agent |
