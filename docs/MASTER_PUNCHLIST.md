...
**Production vs `main` (alias race + source of truth):** **`GET https://www.foldera.ai/api/health`** is the **canonical** record of which commit is **actually running** on www — not `git log -1` alone, not “I just pushed,” and not an older session note. Use **`revision.git_sha`** / **`revision.git_sha_short`** (and optional **`x-foldera-git-sha`**) as proof. **Docs-only** commits (e.g. `SESSION_HISTORY`, `WHATS_NEXT`, punchlist edits) still go through CI → deploy; after they land, www may legitimately show a **newer** SHA than the commit that introduced a feature — that is normal. **Drift:** if `revision` is missing or `git_sha_short` ≠ first 7 of `git rev-parse origin/main`, multiple production deploys may have finished **out of order** and the wrong one took the alias. **Fix:** push an empty commit to `main` (CI → [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml)) or **Promote** the deployment whose SHA matches `main` in the Vercel dashboard.

**“Deploy to Vercel” GitHub email failed but www looks fine:** The [deploy workflow](../.github/workflows/deploy.yml) can fail at **`vercel deploy --prebuilt`** (step 8) even when **`vercel build`** succeeded. Common cause: **race** — Vercel’s **GitHub integration** already deployed the same commit while the CLI job was uploading, or two `workflow_run` deliveries queued overlapping CLI deploys. **Truth:** compare **`GET /api/health` → `revision.git_sha`** to your commit; check [Vercel deployments](https://vercel.com/dashboard) for that SHA **READY**. **Mitigation (in repo):** workflow uses a **`concurrency` group** (`vercel-prod-cli-foldera`); **probes Vercel’s API** first and **skips** `vercel pull` / `vercel build` / `vercel deploy` when a **READY** production deployment already matches **`githubCommitSha`** (saves Hobby **~100 deployments / 24h** when Git + CLI would otherwise double-deploy); **3 deploy attempts** with backoff only when CLI runs; **no retries** if stderr contains **`api-deployments-free-per-day`** (quota exhausted — wait ~24h rolling window, turn off **either** Git integration **or** CLI path if you only need one, or **upgrade Pro**). If emails still show red, open the failed job log on GitHub Actions for the real stderr (annotations often only say exit code 1).
