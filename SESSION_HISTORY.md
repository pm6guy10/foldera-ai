...
## 2026-05-11 - Pipeline cron heartbeat reruns now validate the real daily window
- MODE: CI reliability seam only.
...
- Verification: `npm run health` returned `RESULT: 0 FAILING`; `npx vitest run scripts/__tests__/pipeline-cron-heartbeat-check.test.ts --reporter=verbose` passed (`4/4`); `GITHUB_EVENT_NAME=schedule`, `GITHUB_RUN_ATTEMPT=2`, `npm run check:pipeline-heartbeat` passed with `Checking daily_brief cron_complete window: 2026-05-11T11:00:00.000Z -> ... UTC` and `OK: daily_brief cron_complete count=1 in UTC window ...`; `npm run build` passed; public GitHub Actions API confirmed the still-red latest run is `Pipeline cron heartbeat` `#34` attempt `2` on old commit `5186327...`, so no fresh run has exercised the new logic yet.
- Unresolved issues: A fresh GitHub run on the new head commit is still needed for hosted proof because rerunning `#34` would stay pinned to `5186327`. This seam does not change daily-brief generation behavior or any paid model path.
