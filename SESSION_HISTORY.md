...
- What changed: Traced the no-winner output to two separate rules. The real current winner rejection is `lib/briefing/discrepancy-card-frame.ts`, where the closest deadline-shaped candidate fails as `weak_risk; reminder_without_risk`. The confusing graph blocker came from `auditBehavioralGraphConsistency()` reporting natural rolling-window count aging as drift, after which `winner-truth.ts` promoted any drift row to a current blocker. Narrow fix: `isRollingWindowBoundaryDrift()` now ignores monotonic count decreases when no signal is newer than the graph computation, while still reporting newer-signal mismatches.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` still stops at `GATE_9_REAL_NON_OWNER_BETA`; `npm run gate:quality`, `npm run gate:visual`, and `npm run gate:decision-trace` passed; focused behavioral graph test passed (`6/6`); post-fix `npm run winner:autopsy` passed with `graph_drift: []`, `action_needed: []`, `future_findings: []`, and `no_safe_artifact_today` explained by `weak_risk; reminder_without_risk`; `npm run build` passed; production `/api/health` reported `c5fda9eb608deb674e03e5c002eea88b381346f2` from deployment `dpl_FAGL3fyyBT17aiHEUr4VfQB4W3iC`.
- Unresolved issues: No trustworthy current winner exists today. The clean next owner-private product seam is improving source evidence/risk framing or input hydration for the closest deadline candidate without weakening the discrepancy-card quality bar. Real beta proof remains externally blocked on one connected non-owner account.

## 2026-05-13 - Winner proof packet
- MODE: Winner proof packet only; no UI polish, frontend redesign, paid generation, outbound email, Stripe, schema, fake users, beta-readiness claim, or hidden model reasoning exposure.
- Files changed: `docs/WINNER_PROOF_PACKET.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added a Brandon-readable winner proof packet that answers the full selected-move trust chain in plain English: sources read, freshness, candidates considered/rejected, no-safe-artifact verdict, why-now, source evidence, produced/current move, send-nothing conditions, and separation of production proof, owner-private proof, mock proof, and missing proof. The packet does not claim beta readiness and does not present the prior owner-private WorkSourceWA persisted artifact as today's current winner.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` stopped at `GATE_9_REAL_NON_OWNER_BETA` with `BLOCKED_EXTERNAL`; `npm run gate:quality` passed; `npm run gate:visual` passed; `npm run gate:decision-trace` passed; `npm run winner:autopsy` ran and returned the current blocker truth, `no_safe_artifact_today`, with weak-risk/reminder-only quality failure plus behavioral graph drift; docs source-of-truth test passed; `npm run build` passed; production `/api/health` reported winner proof packet commit `71986ddf7cb8c296645d2203618034a06ea73ada`; Vercel deployment `dpl_i1P6M7tYdVxhvLfVEoGEibADs9Pj` is `READY`.
- Unresolved issues: Real beta proof remains externally blocked on one real connected non-owner account. Current owner-private winner proof should not produce a new artifact until behavioral graph drift is repaired or proven non-blocking.
...
- What changed: Added the executable release gate program and `npm run gate:status`, which reports the current gate, first failing gate, proof found, proof missing, exact next move, and do-not-touch list. The command passes GATE_0 through GATE_8 from live SHA truth plus existing mock harness proof and stops at `GATE_9_REAL_NON_OWNER_BETA` as `BLOCKED_EXTERNAL`. Added a narrow preflight allowance so this release-gate controller file set can ship without a product contract.
...
- Verification: `npm run health` passed at session start (`RESULT: 0 FAILING`, warnings only); `npm run build` passed; focused dashboard Playwright slice passed (`4` tests); dashboard navigation + live artifact pixel-lock specs passed (`14` tests); local production-route screenshot proof captured desktop `1920x1080` and mobile `390x844` with mocked authenticated `/dashboard` data, no console errors, and horizontal overflow `0`; Vercel served production commit `b917f7c76a4891d319793bd41148a66f7db5036e` as Ready; authenticated production dashboard smoke passed (`3` tests).
- Unresolved issues: Browser/IAB plugin proof could not run because the local Node runtime is `v22.19.0` and the Browser plugin requires `>=22.22.0`; Playwright fallback was used. No paid generation was run and no outbound email was sent.
...
- Verification: `npm run health` passed (`RESULT: 0 FAILING`, Outlook disconnected and last-generation `do_nothing` warnings only); reproduced the live scheduled failure with `npm run test:prod -- --grep "pricing CTA"`; `npm run build` passed; local built-page browser proof at `http://127.0.0.1:3015/` confirmed `copyVisible=true`, `trialVisible=false`, and CTA `href="/start"`; focused landing Playwright slice passed (`npx playwright test tests/e2e/public-routes.spec.ts --grep "Landing page /" --reporter=list`, 11 tests); pushed commit `075a4da`; Vercel production `/api/health` served `075a4da` from deployment `dpl_9ZHXvHRay2x84GL8a9tpaUtfjMno`; live `npm run test:prod -- --grep "pricing CTA"` passed; full public Production E2E lane passed (`37 passed`, `24 skipped`); GitHub Actions reported Production E2E success for run `25500156448`.
- Unresolved issues: None for this seam. The GitHub Actions Node 20 deprecation warning on `actions/upload-artifact@v4` remains a separate workflow-maintenance warning, not the Production E2E failure.
...
- What changed: Added robots and manifest public assets, completed sitemap coverage for `/about`, `/status`, and `/security`, removed audited vestigial/test-only API routes while preserving live system draft queue routes, pruned unused UI component files/exports, replaced raw TSX `rounded-button` / `min-h-[48px]` primitives with semantic global classes, moved script owner UUID literals behind `OWNER_USER_ID`, and clarified source-of-truth ownership so `FULL_AUDIT_RESULTS.md` remains evidence rather than a mutable checklist.
- Verification: `npm run health` (`RESULT: 0 FAILING`, Gmail stale / no Microsoft mailbox / last generation `do_nothing` warnings); focused config Vitest suite passed (`8 files`, `11 tests`); `npm run lint` passed; `npm run build` passed; `npx playwright test tests/e2e/public-routes.spec.ts --reporter=list` passed (`48` tests); `npm run test:ci:unit` passed.
- Unresolved issues: Supabase migration reconciliation remains blocked by linked DB credentials / pooler circuit breaker per `MIGRATION_RECONCILIATION_REPORT.md`. P2-8 large-file splitting remains intentionally unsolved by this cleanup seam because the approved seam list did not authorize broad product-file refactors.
...
## 2026-05-06 — Daily Utility Slate added below strict artifact gate
- MODE: Daily Utility Slate Plan.
- Files changed: `lib/briefing/daily-utility-slate.ts`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `components/foldera/DailyUtilitySlateCard.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/dashboard/DashboardChromeExtras.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added a deterministic Daily Utility Slate contract for the no-finished-artifact path, built from winner-truth evidence rather than model prose. `/api/conviction/latest` now returns `finished_artifact_verdict` plus `daily_utility_slate` when no strict artifact survives, while valid artifacts still require the discrepancy-card gate. `/dashboard` renders the slate in the main card instead of generic silence, and blocked candidates are surfaced only as awareness, not finished artifacts. The slate selector prefers concrete command-center families over vague goal-drift candidates.
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only: Gmail fresh 8h, Outlook fresh 5h, last generation `do_nothing`); focused Vitest passed for slate/latest/large-file guard (`15` tests); focused discrepancy/artifact/winner-contract Vitest passed (`28` tests); `npx playwright test tests/e2e/authenticated-routes.spec.ts -g "daily utility slate|loads and shows directive card" --reporter=list` passed (`2` tests); `npx playwright test tests/e2e/cpu-reduction.spec.ts -g "owner system generate uses run-brief" --reporter=list` passed (`1` test); `npm run build` passed; live no-paid owner slate probe returned `no_finished_artifact` with `blocked_but_real = Commitment due 2026-05-14 with no matching calendar block`.
- Unresolved issues: `npm run winner:autopsy` and `npm run winner:replay:3day` still fail the deeper dependability proof because 2026-05-05 remains a historical garbage-regression write_document row. No paid generation was run and no outbound email was sent.

## 2026-05-06 — Daily Utility Slate internal-output correction
- MODE: Product proof correction after live dashboard showed operator trace.
- Files changed: `lib/briefing/daily-utility-slate.ts`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `components/foldera/DailyUtilitySlateCard.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `SESSION_HISTORY.md`.
- What changed: Stopped the slate from rendering internal blocker codes, candidate family/tier labels, and speculative calendar-gap candidates with missing schedule-resolution context. The dashboard no-safe state now says `Today's read` and `No safe finished action today`, with a plain-language reason and source-health context instead of `positive_winner_contract`, `weak_*`, or `missing_*` strings.
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only); focused Vitest passed for slate/latest/large-file guard (`16` tests); fresh-server Playwright passed for the Daily Utility Slate route (`1` test); `npm run build` passed; live no-paid owner slate probe now returns `blocked_but_real = null` and a watch item explaining that sources are current but no safe finished action was proven.
- Unresolved issues: This correction improves honesty and removes internal sludge, but it still proves that today has no useful finished action. The stronger-winner problem remains open; no paid generation was run and no outbound email was sent.

## 2026-05-06 — Dashboard slate moved to bounded persisted receipts
- MODE: Controller + Daily Utility Slate implementation correction.
- Files changed: `AGENTS.md`, `lib/briefing/daily-utility-slate.ts`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `tests/config/__tests__/docs-source-of-truth.test.ts`, `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added the active source-of-truth loading hierarchy to `AGENTS.md` and guarded it with a config test. Reworked `/api/conviction/latest` so the normal dashboard no-artifact slate reads only bounded persisted `tkg_actions` receipts instead of running the owner winner-truth diagnostic path. The slate remains conservative: it surfaces a plain-language no-safe watch item from persisted no-send receipts and does not expose candidate ids, scoring, or internal blocker codes.
...
## 2026-05-02 — Security seam hardened for dev access, verdict errors, and CSP
- MODE: FOLDERA SECURITY HARDENING SEAM — dev route, verdict route, and deploy headers only.
- Files changed: `app/api/dev/stress-test/route.ts`, `app/api/dev/stress-test/__tests__/route.test.ts`, `app/api/health/verdict/route.ts`, `app/api/health/verdict/__tests__/route.test.ts`, `tests/config/__tests__/vercel-security-headers.test.ts`, `vercel.json`, `SESSION_HISTORY.md`.
- What changed: Brought `/api/dev/stress-test` onto the same `resolveUser` + `OWNER_USER_ID` authorization path as the other owner-only dev endpoints while preserving `ALLOW_DEV_ROUTES`; routed `/api/health/verdict` database failures through `apiErrorForRoute` so raw DB messages no longer reach the client; added a conservative global `Content-Security-Policy` to `vercel.json` alongside the existing security headers.
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only); `git status --short --branch`; `git log --oneline -10`; `npx vitest run app/api/dev/stress-test/__tests__/route.test.ts app/api/health/verdict/__tests__/route.test.ts tests/config/__tests__/vercel-security-headers.test.ts` passed (`9` tests); `npm run build` passed; `npx playwright test tests/e2e/public-routes.spec.ts --reporter=list` passed (`45` tests).
- Unresolved issues: No live Vercel deployment/header fetch was run in this local seam proof. No paid calls were made and no outbound email was sent.

## 2026-05-02 — Morning cron orchestration shipped; schema reconciliation stopped at read-only report
- MODE: FOLDERA OPERATIONS SEAM — cron contract + read-only schema truth.
- Files changed: `app/api/cron/morning-pipeline/route.ts`, `app/api/cron/morning-pipeline/__tests__/route.test.ts`, `app/api/cron/nightly-ops/route.ts`, `app/api/cron/daily-brief/route.ts`, `app/api/cron/daily-maintenance/route.ts`, `tests/config/__tests__/vercel-cron-config.test.ts`, `vercel.json`, `MIGRATION_RECONCILIATION_REPORT.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Added a single scheduled `/api/cron/morning-pipeline` entrypoint that runs the existing `nightly-ops`, `daily-brief`, and `daily-maintenance` routes sequentially, then reduced `vercel.json` to one scheduled morning cron entry. Wrote a read-only migration reconciliation report capturing the current local/remote divergence, the duplicate local timestamp version, and the exact `supabase db lint --linked` auth/circuit-breaker blocker.
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only); `git status --short --branch`; `git log --oneline -10`; `npx vitest run app/api/cron/morning-pipeline/__tests__/route.test.ts app/api/cron/nightly-ops/__tests__/route.test.ts app/api/cron/daily-brief/__tests__/route.test.ts app/api/cron/daily-maintenance/__tests__/route.test.ts tests/config/__tests__/vercel-cron-config.test.ts` passed (`12` tests); `npm run build` passed; `npx playwright test tests/e2e/public-routes.spec.ts --reporter=list` passed (`45` tests); `npx supabase migration list --linked` returned a divergent local/remote history before the linked pooler fully opened its auth circuit breaker.
- Unresolved issues: Schema reconciliation is still BLOCKED. `npx supabase db lint --linked` failed because the shell lacks a working `SUPABASE_DB_PASSWORD`, then the Supabase pooler returned `FATAL: Circuit breaker open: Too many authentication errors`. No DB mutation was attempted, and no reconciliation strategy was applied without separate approval.

## 2026-05-05 — Positive winner contract and taste pack enforced before model attempts
- MODE: Positive Winner Contract + Taste Pack Plan.
- Files changed: `lib/briefing/artifact-taste-pack.ts`, `lib/briefing/generator.ts`, `lib/briefing/types.ts`, `lib/briefing/__tests__/positive-winner-contract.test.ts`, `scripts/winner-autopsy.ts`, `package.json`, `SESSION_HISTORY.md`.
- What changed: Added a shared production-safe good/bad artifact taste pack, wired candidate artifactability and positive winner contract checks into ranked candidate selection before paid/model attempts, reserved attempt slots for viable Tier 1/Tier 2 command-center artifacts, added winner-quality metadata to generation logs, and added a read-only owner winner autopsy harness. Stale interview-status ghosts, transactional sender pressure, relationship-silence risk, missing recipients, missing source facts, generic prep, and stale evidence now block before consuming model attempts.
- Verification: `npm run health` at start and finish (`RESULT: 0 FAILING`, warnings only; Gmail fresh, Microsoft/Outlook stale warnings remain); `git log --oneline -10`; `git status --short --branch`; `npm test -- --run lib/briefing/__tests__/positive-winner-contract.test.ts lib/briefing/__tests__/artifact-quality-gate.test.ts` passed (`19` tests); `npm test -- --run lib/briefing/__tests__/generator-runtime.test.ts` passed; `npm run test:critical-ci-contracts` passed; `npm run winner:autopsy` passed and selected `Deadline closing: CWU interview #2` as a Tier 1 `interview_role_fit_packet` with `5` viable Tier 1/Tier 2 candidates, `0` decrypt fallback rows, and Microsoft staleness logged as an adjacent risk; `npm run build` passed.
- Unresolved issues: No paid live owner proof was run and no outbound email was sent. Microsoft provider freshness remains an adjacent risk, and the latest historical skipped artifact still reflects the older stale MACSC decision, but the current free autopsy no longer selects that stale candidate.

## 2026-05-05 — Dependability first truth surface, live graph repair, and honest no-safe-artifact fallback
- MODE: Dependability First: Winner Truth, Graph Repair, and Live Proof.
- Files changed: `app/api/cron/daily-maintenance/__tests__/route.test.ts`, `app/api/cron/daily-maintenance/route.ts`, `app/api/cron/nightly-ops/__tests__/route.test.ts`, `app/api/cron/nightly-ops/contract.ts`, `app/api/cron/nightly-ops/route.ts`, `app/api/system/winner-truth/route.ts`, `app/api/system/winner-truth/__tests__/route.test.ts`, `app/dashboard/system/SystemClient.tsx`, `lib/auth/self-identity.ts`, `lib/briefing/__tests__/positive-winner-contract.test.ts`, `lib/briefing/artifact-taste-pack.ts`, `lib/briefing/generator.ts`, `lib/briefing/scorer.ts`, `lib/signals/__tests__/behavioral-graph.test.ts`, `lib/signals/__tests__/entity-trust.test.ts`, `lib/signals/behavioral-graph.ts`, `lib/signals/entity-trust-repair.ts`, `lib/signals/entity-trust.ts`, `lib/signals/signal-processor.ts`, `lib/system/winner-truth.ts`, `lib/utils/signal-egress.ts`, `scripts/winner-autopsy.ts`, `scripts/winner-replay-3day.ts`, `tests/e2e/cpu-reduction.spec.ts`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`, `package.json`.
- What changed: Replaced JS-side/stale behavioral-graph reads with a metadata-only paginated aggregation path, moved graph refresh + polluted-entity trust repair into nightly ops before briefing, added a graph freshness guard so relationship-heavy candidates are downgraded when the graph is stale, added a shared owner-only winner-truth service/route and plain-language `/dashboard/system` proof cards, and tightened the positive winner contract so weak goal-drift memos, thin interview packets, and fake calendar conflicts no longer count as viable winners. Also repaired the live owner graph/trust state with no paid/model calls and surfaced the deeper preview-only-mail finding in the truth output/UI.
- Verification: `npm run health` at start and finish (`RESULT: 0 FAILING`; Outlook/Microsoft stale warnings remain); focused Vitest passed for graph/trust/nightly-ops/daily-maintenance/winner-truth (`22` tests across the final focused runs); `npm run build` passed multiple times after the schema-safe freshness fix and Playwright assertion update; `npx playwright test tests/e2e/cpu-reduction.spec.ts --reporter=list` passed (`2` tests), including owner `/dashboard/system` truth cards and dry-run behavior; no-paid live repair via inline `tsx` refreshed the owner behavioral graph (`373` entities updated, `3710` signals scanned, `0` stale rows) and downgraded polluted trusted entities; post-repair `npm run winner:autopsy` now reports `current_winner.verdict = no_safe_artifact_today`, `graph_drift = []`, `polluted_entities = []`, `decrypt_fallback_count = 0`, and explicit blockers for the thin CWU packet, fake calendar gap, stale follow-ups, and goal-drift memos.
- Unresolved issues: `npm run winner:replay:3day` still fails because the recent persisted owner days (`2026-05-03` to `2026-05-05`) are historical garbage-regression outputs written before these repairs; no paid run was executed to regenerate those days. Microsoft provider freshness remains stale (~3 days), and synced mail still stores preview text rather than full raw message bodies, which is now logged as a future backlog finding. No outbound email was sent.

## 2026-05-05 — CI winner-contract fixture repair after positive contract push
- MODE: CI unit failure seam only.
- Files changed: `lib/briefing/generator.ts`, `lib/briefing/__tests__/winner-selection.test.ts`, `lib/briefing/__tests__/pipeline-receipt.test.ts`, `lib/briefing/__tests__/decision-payload-adversarial.test.ts`, `lib/briefing/__tests__/scorer-metadata-egress.test.ts`, `SESSION_HISTORY.md`.
- What changed: Preserved the positive winner contract but moved its hard-block reason after existing deterministic disqualifiers so specific legacy blocker reasons remain visible. Hydrated stale winner-selection fixtures with current source facts, updated stale-evidence receipt expectations to prove pre-model positive-contract blocking, and aligned the metadata-egress assertion with the safe `created_at` signal metadata select.
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only; Outlook/Microsoft stale); reproduced the pasted CI failure locally with `npx vitest run lib/briefing/__tests__/winner-selection.test.ts lib/briefing/__tests__/pipeline-receipt.test.ts lib/briefing/__tests__/decision-payload-adversarial.test.ts`; after patch, the same focused run passed (`30` tests); `npm run test:ci:unit` passed; `npm run build` passed.
- Unresolved issues: No product/browser proof was run for this CI-only repair. `gh` is unavailable in this shell, so remote GitHub Actions confirmation must come from the web/API after push. No paid generation was run and no outbound email was sent.

## 2026-05-05 — Discrepancy frame gate enforced through dashboard and owner proof
- MODE: Discrepancy Engine Execution Plan.
- Files changed: `lib/briefing/discrepancy-card-frame.ts`, `lib/briefing/generator.ts`, `lib/conviction/execute-action.ts`, `lib/cron/daily-brief-generate.ts`, `app/api/conviction/latest/route.ts`, `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `app/dashboard/system/SystemClient.tsx`, `components/foldera/DailyBriefCard.tsx`, focused tests, `tests/e2e/authenticated-routes.spec.ts`, `tests/e2e/cpu-reduction.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added a shared discrepancy-card frame contract and deterministic quality judge, moved frame validation into pending-action persistence and `/api/conviction/latest`, hid dashboard artifacts without passing claim/contradiction/risk/evidence/next-action/source refs, stored discrepancy receipts in existing execution results and approve/skip feedback, derived pattern memory from existing action history, and reframed dashboard/system cards around discrepancy proof. Tightened the judge after owner proof exposed a title-shaped action, so weak next actions now return `No safe artifact today` instead of pretending to be winners.
- Verification: `npm run health` at start and finish (`RESULT: 0 FAILING`, Gmail stale warning and Outlook disconnected/fresh notice remain); focused Vitest passed for frame/judge/dashboard/latest/winner-selection/daily-brief/winner-truth (`84` tests); `npm run build` passed; `npx playwright test tests/e2e/authenticated-routes.spec.ts tests/e2e/cpu-reduction.spec.ts --reporter=list` passed (`37` tests); `npm run winner:autopsy` passed and now reports `current_winner.verdict = no_safe_artifact_today` with reason `Selected candidate failed discrepancy-card quality: weak_next_action`; `npm run winner:replay:3day` passed with three allowed days (`no_safe_artifact`, `no_safe_artifact`, `no_safe_artifact`).
- Unresolved issues: No paid generation was run and no outbound email was sent. Microsoft remains stale/disconnected and cannot support urgency or relationship-silence winners. Mail sync still stores preview text rather than full raw bodies; the owner truth surface logs that as future backlog, not a current blocker.

## 2026-05-06 — Dashboard Figma shell applied to live dashboard route
- MODE: Figma dashboard implementation seam only.
- Files changed: `app/dashboard/page.tsx`, `app/globals.css`, `components/dashboard/DashboardBriefWorkPanel.tsx`, `components/dashboard/DashboardStatsStrip.tsx`, `components/foldera/DailyBriefCard.tsx`, `components/foldera/DashboardSidebar.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `tests/dashboard/live-artifact-pixel-lock.spec.ts`, `SESSION_HISTORY.md`.
...
- Verification: `npm run health` (`RESULT: 0 FAILING`, warnings only); reproduced the GitHub failure locally with the two failing Playwright cases; after patch, the same two-test CI slice passed (`2` tests); the full authenticated route spec passed (`36` tests); `npm run test:ci:e2e:lint` passed; `npm run build` passed; pushed to `main` and confirmed GitHub checks green, Vercel production `Ready`, `/api/health` serving the pushed SHA, and live `/dashboard` rendering the Figma frame with the real no-safe-artifact slate.
- Unresolved issues: User visual approval is still separate from technical delivery proof. No paid generation was run and no outbound email was sent.

## 2026-05-06 — Public landing visual direction locked for live push
- MODE: Public landing visual lock-in seam.
- Files changed: `components/foldera/LandingPage.tsx`, `components/nav/NavPublic.tsx`, `app/globals.css`, `public/foldera-glyph.svg`, `SESSION_HISTORY.md`.
- What changed: Reworked `/` to the supplied Foldera visual direction with the daily brief as the first primary product object, stronger dark/cyan/green hierarchy, mobile proof/action/story sections, cleaned mobile context icons, and refreshed public nav/header treatment. Removed the giant hero-folder-first treatment before shipping.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Outlook disconnected and last-generation `do_nothing` warnings only; Playwright fallback browser proof loaded `http://localhost:3000/` at desktop and mobile widths, confirmed the desktop daily brief card is visible while giant folder/wide card are hidden, confirmed mobile context renders six cleaned rows, and captured screenshots; `npm run build` passed after the code changes; focused public-route regression for neutral approve copy and footer destinations passed (`3` tests).
- Unresolved issues: Browser/IAB plugin runtime remained blocked by local Node v22.19.0 requiring >=22.22.0, so Playwright fallback was used. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Mobile dashboard menu restored
- MODE: Dashboard mobile shell polish seam.
- Files changed: `app/dashboard/page.tsx`, `components/foldera/DashboardSidebar.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Replaced the mobile logo-only dashboard strip with an integrated mobile dashboard menu using the existing shell nav contract. Mobile users can open the menu, see Executive Briefing, Playbooks, Signals, Audit Log, Integrations, and Settings, switch panels in-place on `/dashboard`, and sign out from the account row. Desktop sidebar and dashboard action wiring were unchanged.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with freshness warnings only; `npm run build` passed after clearing stale generated `.next` output; focused Playwright passed for `dashboard-navigation` (`12/12`), authenticated dashboard slice (`19/19`), dashboard screenshot capture (`1/1`), and live-artifact pixel-lock (`3/3`). Screenshot proof captured mobile menu open/closed at `C:\Users\b-kap\AppData\Local\Temp\foldera-dashboard-mobile-menu-open.png` and `C:\Users\b-kap\AppData\Local\Temp\foldera-dashboard-mobile-menu-closed.png`.
- Unresolved issues: Browser/IAB proof was unavailable in this workspace, so Playwright CLI was used. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Dashboard large-file guard stabilized
- MODE: CI failure seam only.
- Files changed: `app/dashboard/page.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardStatsStrip.tsx`, `SESSION_HISTORY.md`.
- What changed: Extracted the mobile dashboard layout from `app/dashboard/page.tsx` into the existing `components/dashboard` split boundary and exported the shared dashboard stats type. The route now stays comfortably below the large-file guard instead of hovering at the 1000-line threshold, while the mobile menu, dashboard slate, panel switching, and desktop stage behavior remain on the same contracts.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with freshness warnings only; reproduced the CI failure with `npx vitest run tests/config/__tests__/large-file-splits.test.ts` (`expected 1002 to be less than or equal to 1000`); after extraction the same guard passed with `app/dashboard/page.tsx` at `926` lines; `npm run build` passed; `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`12/12`); `npx playwright test tests/e2e/authenticated-routes.spec.ts --grep "Dashboard /dashboard" --reporter=list` passed (`19/19`); `npm run test:ci:unit` passed.
- Unresolved issues: GitHub/Vercel post-push proof is pending in-session. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Dashboard reset to finished-work inbox
- MODE: Hard Reset Dashboard To Finished Work Inbox.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardSecondaryPanel.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/foldera/DailyUtilitySlateCard.tsx`, `components/foldera/DashboardSidebar.tsx`, `components/foldera/EmptyStateCard.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/dashboard/live-artifact-pixel-lock.spec.ts`, `tests/e2e/authenticated-routes.spec.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Reframed authenticated `/dashboard` as a three-tab finished-work inbox with Today, History, and Sources as the primary app model. Legacy `briefing`, `playbooks`, `audit-log`, `signals`, `integrations`, and `settings` panel params now remap safely into Today, History, or Sources; the old stats strip and operator taxonomy are removed from the primary surface; Today keeps finished artifact actions/paywall behavior and no-safe states now show safe missing-input copy when derivable from existing slate/source status.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Outlook/Microsoft freshness warnings only; `npm run build` passed after clearing stale generated `.next` output; `npx vitest run tests/config/__tests__/dashboard-inbox-model.test.ts tests/config/__tests__/large-file-splits.test.ts` passed (`4` tests); `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`13/13`); `npx playwright test tests/e2e/authenticated-routes.spec.ts --grep "Dashboard /dashboard" --reporter=list` passed (`19/19`); `npx playwright test tests/dashboard/live-artifact-pixel-lock.spec.ts --reporter=list` passed (`3/3`); mobile screenshot proof captured the open Today/History/Sources menu at `C:\Users\b-kap\AppData\Local\Temp\foldera-dashboard-mobile-inbox-proof.png`.
- Unresolved issues: GitHub CI, Vercel deployment, production `/api/health` SHA, and live production `/dashboard` mobile proof are pending until the verified commit is pushed. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Dashboard always-value workspace
- MODE: Dashboard always-value continuation seam.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardSecondaryPanel.tsx`, `components/dashboard/DashboardWorkspacePanels.tsx`, `components/dashboard/ProductShell.tsx`, `components/foldera/DailyUtilitySlateCard.tsx`, `components/foldera/DashboardSidebar.tsx`, `components/foldera/EmptyStateCard.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Made authenticated `/dashboard` one adaptive workspace instead of a panel jump model: Today, Recent Work, Sources, and Account render together on desktop and stack on mobile. The no-artifact state now still tells the user what Foldera checked, what it protected, and the smallest unlock without leaking internal blocker codes; source and account controls stay inside the dashboard instead of sending users to legacy settings pages.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with freshness warnings only; red-green proof covered the new dashboard helper and unified workspace Playwright assertions; `npx vitest run tests/config/__tests__/dashboard-inbox-model.test.ts app/dashboard/__tests__/dashboard-page-model.test.tsx` passed (`6` tests); `npx vitest run tests/config/__tests__/large-file-splits.test.ts` passed; `npm run build` passed; `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`14/14`); production-bundle screenshot proof captured desktop artifact and mobile no-artifact states at `.screenshots/dashboard-seam/always-value-desktop-current.png` and `.screenshots/dashboard-seam/always-value-mobile-current.png`.
- Unresolved issues: GitHub CI, Vercel deployment, production `/api/health` SHA, and live production `/dashboard` proof are pending until this commit is pushed. Browser/IAB auth proof was unavailable because the exposed Browser runner could not import the repo auth helper, so Playwright browser proof was used. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Authenticated dashboard CI assertions aligned
- MODE: CI failure seam only.
- Files changed: `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
...
## 2026-05-07 - Public promise aligned to product truth
- MODE: Cross-surface public alignment seam.
- Files changed: `app/layout.js`, `app/dashboard/layout.tsx`, `app/login/page.tsx`, `app/login/login-inner.tsx`, `app/pricing/page.tsx`, `app/start/page.tsx`, `app/try/page.tsx`, `components/foldera/LandingPage.tsx`, `tests/e2e/landing-dashboard-visual.spec.ts`, `tests/e2e/public-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Removed mature-platform overclaims from public acquisition and first-use surfaces: fake trusted-team logos, fake large outcome metrics, unshipped Slack/Notion/Drive storage integrations, SOC 2/enterprise claims, and unconditional "every morning" copy. Replaced them with the current truth: Gmail/Microsoft-first source-backed checks, finished work when safe, useful blocker when not, source trail, approval before anything moves, and no outbound by default.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Microsoft freshness warnings only; active-surface overclaim grep returned no matches; `npm run build` passed; focused public/first-use Playwright lane passed (`21/21`) on a fresh production bundle after an initial stale-server retry; screenshot proof captured homepage, pricing, and start surfaces under `.screenshots/public-alignment/`.
- Unresolved issues: GitHub CI, Vercel deployment, production `/api/health` SHA, and live public-route proof are pending until this alignment commit is pushed. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Legacy dashboard routes folded into the unified workspace
- MODE: Dashboard route alignment seam.
- Files changed: `app/dashboard/audit-log/page.tsx`, `app/dashboard/briefings/page.tsx`, `app/dashboard/integrations/page.tsx`, `app/dashboard/playbooks/page.tsx`, `app/dashboard/settings/SettingsClient.tsx`, `app/dashboard/signals/page.tsx`, `components/dashboard/foldera-dashboard-pixel-lock.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Removed the old dashboard contradictions that sent users from Sources/History into Settings, Briefings, or "executive briefing" copy. Legacy Integrations and Signals now hand off to the unified Sources panel; Audit Log and Playbooks hand off to Recent Work and Today; Settings points source summary users back to Sources; the dormant pixel-lock hotspots now use Today, Recent Work, Sources, and Account.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Outlook/Microsoft freshness warnings only; stale dashboard/settings route-copy grep returned no matches in active dashboard/test paths; `npm run build` passed; focused authenticated Settings/Signals/Briefings Playwright lane passed (`14/14`); `CI=1 npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`14/14`) after proving local non-CI failures were stale server reuse; authenticated browser screenshot proof captured `.screenshots/dashboard-seam/legacy-sources-route-auth.png`.
- Unresolved issues: GitHub CI, Vercel deployment, production `/api/health` SHA, and live production route proof are pending until this alignment commit is pushed. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Public homepage simplified to one paid-worthy promise
- MODE: Public product-value surface seam.
- Files changed: `components/foldera/LandingPage.tsx`, `app/globals.css`, `tests/e2e/public-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Collapsed the homepage from a busy multi-section explainer into one simple product promise: Foldera checks sources, produces the useful artifact when safe, and holds back honestly when not. Removed the giant trust-icon row, integration-card grid, mobile network preview, diagram panel, reason-tile maze, and gradient CTA clutter. The page now leads with the actual daily brief object and a three-part value loop.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Outlook/Microsoft freshness warnings only; `npm run build` passed; focused landing Playwright lane passed (`12/12`); stale landing overclaim/removed-section grep returned no matches; production-bundle screenshot proof captured `.screenshots/public-alignment/home-simplified-final-desktop.png` and `.screenshots/public-alignment/home-simplified-final-mobile.png`.
- Unresolved issues: GitHub CI, Vercel deployment, production `/api/health` SHA, and live homepage proof are pending until this simplification commit is pushed. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Outlook connector health truth hardened
- MODE: Outlook/Microsoft connector reliability seam.
- Files changed: `scripts/health.ts`, `scripts/health-connectors.ts`, `scripts/__tests__/health-connectors.test.ts`, `lib/auth/user-tokens.ts`, `lib/auth/__tests__/user-tokens.test.ts`, `SESSION_HISTORY.md`.
...
## 2026-05-07 - OAuth callbacks now fail closed without refresh capability
- MODE: OAuth connector durability seam.
- Files changed: `app/api/google/callback/route.ts`, `app/api/google/callback/__tests__/route.test.ts`, `app/api/microsoft/callback/route.ts`, `app/api/microsoft/callback/__tests__/route.test.ts`, `SESSION_HISTORY.md`.
- What changed: Hardened both Google and Microsoft connect callbacks so Foldera no longer persists a provider row as connected when the OAuth callback returns an access token but no refresh token. Those paths now redirect back to Settings with `missing_refresh_token` instead of saving a dead connector row that can never support background sync.
- Verification: `npx vitest run app/api/google/callback/__tests__/route.test.ts app/api/microsoft/callback/__tests__/route.test.ts` passed (`4/4`) and proved both the happy path and the new fail-closed path; `npx vitest run lib/auth/__tests__/user-tokens.test.ts lib/auth/__tests__/token-store.test.ts` passed (`15/15`); `npm run health` returned `RESULT: 0 FAILING` with existing Outlook reconnect + do_nothing warnings only; `npm run build` passed.
- Unresolved issues: Live OAuth/browser proof and production row verification remain unproven in this session because no real Google/Microsoft callback was exercised and no production DB truth tool was available here. No paid generation was run and no outbound email was sent.

## 2026-05-07 - Settings OAuth recovery copy polished
- MODE: OAuth connector recovery UX seam.
- Files changed: `app/dashboard/settings/SettingsClient.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
...
## 2026-05-07 - Behavioral graph boundary drift unblocked
- MODE: Winner-truth diagnostic seam.
- Files changed: `lib/signals/behavioral-graph.ts`, `lib/signals/__tests__/behavioral-graph.test.ts`, `SESSION_HISTORY.md`.
- What changed: The behavioral graph consistency audit now ignores one-count rolling-window drift when the graph was computed after the entity's latest signal. Normal time passing can no longer turn a fresh graph into a false `behavioral_graph_drift_against_db_truth` current blocker.
- Verification: Reproduced the live Keri Nopens shape as a failing test (`stored.signal_count_90d=17`, `actual.signal_count_90d=16`, latest signal older than graph timestamp); focused graph/winner-truth Vitest passed (`8/8`); live read-only winner-truth replay now reports `graph_drift: []` and `current_blockers: []`; `npm run health` returned `RESULT: 0 FAILING` with Outlook reconnect and last-generation `do_nothing` warnings only; `npm run build` passed.
- Unresolved issues: Outlook remains disconnected and requires real OAuth reconnect; three-day consistency still includes the historical 2026-05-05 garbage-regression artifact. GitHub CI, Vercel deployment, and production `/api/health` SHA are pending until this commit is pushed. No paid generation was run and no outbound email was sent.

## 2026-05-08 - Zero-friction source recovery doctrine and Microsoft freshness repair
- MODE: Acting senior operator / source freshness money-loop seam.
- Files changed: `AGENTS.md`, `CLAUDE.md`, `ACCEPTANCE_GATE.md`, `FOLDERA_PRODUCT_SPEC.md`, `CURRENT_STATE.md`, `LESSONS_LEARNED.md`, `lib/config/constants.ts`, `scripts/health-connectors.ts`, `app/api/integrations/status/route.ts`, `app/api/integrations/status/__tests__/route.test.ts`, `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `components/dashboard/DashboardWorkspacePanels.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Codified the zero-friction agentic standard across active doctrine: Foldera should safely self-prepare, self-refresh, and self-recover before asking Brandon to diagnose anything. Aligned dashboard source readiness with the same 25-hour mail-cursor freshness bar used by health, added `needs_sync` to the integrations status payload, made no-artifact copy treat stale source cursors as a freshness blocker, and changed the dashboard Sources panel so stale connected Google/Microsoft sources auto-attempt safe sync while keeping manual sync/reconnect controls as fallback. Ran the real Microsoft sync path with existing owner tokens; it refreshed Microsoft tokens and pulled 85 mail signals plus 19 calendar signals.
- Verification: `npm run health` before work reported `Outlook stale`, `Mail cursors stale`, and `RESULT: 0 FAILING`; focused red tests failed before implementation; after patch `npx vitest run app/api/integrations/status/__tests__/route.test.ts tests/config/__tests__/dashboard-inbox-model.test.ts tests/config/__tests__/large-file-splits.test.ts` passed (`15/15`); `npm run build` passed; `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`15/15`) including stale Microsoft auto-recovery and 390px no-overflow; focused `/start` CI smoke passed (`1/1`); real owner `syncMicrosoft()` returned `mail_signals=85`, `calendar_signals=19`; post-sync `npm run health` reported `Outlook fresh`, `Mail cursors current`, and `RESULT: 0 FAILING`; authenticated browser proof captured desktop and mobile Sources with no console errors and `overflow=0`.
- Unresolved issues: Last generation is still `do_nothing`; this slice did not run paid generation, outbound email, Stripe, schema migration, or destructive actions. The next autonomous move is to make the daily value/artifact path use the newly fresh sources to produce useful work or one irreducible blocker without requiring Brandon to operate the system.

## 2026-05-08 - Foldera operating constitution locked into repo
- MODE: Repo-driven alignment / operating constitution seam.
- Files changed: `FOLDERA_OPERATING_SYSTEM.md`, `CODEX_START.md`, `AGENTS.md`, `CLAUDE.md`, `ACCEPTANCE_GATE.md`, `FOLDERA_PRODUCT_SPEC.md`, `LESSONS_LEARNED.md`, `SYSTEM_RUNBOOK.md`, `tests/config/__tests__/docs-source-of-truth.test.ts`, `SESSION_HISTORY.md`.
- What changed: Added `FOLDERA_OPERATING_SYSTEM.md` as the canonical product doctrine and `CODEX_START.md` as the session boot contract, with `BRANDON.md` preserved as the how-to-think/taste file. Updated `AGENTS.md`, `CLAUDE.md`, `ACCEPTANCE_GATE.md`, `SYSTEM_RUNBOOK.md`, product spec, and lessons to point at the constitution instead of duplicating the same zero-friction doctrine in multiple places. Added a config test that fails if the operating constitution, boot contract, or source-of-truth hierarchy stop naming those files.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh, Outlook fresh, mail cursors current, and `Last generation do_nothing` warning; `npx vitest run tests/config/__tests__/docs-source-of-truth.test.ts tests/config/__tests__/large-file-splits.test.ts` passed (`4/4`); `npm run lint` passed; `npm run build` passed.
- Unresolved issues: This is a docs/guardrail lock-in, not a product artifact proof. Last generation is still `do_nothing`; no paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Dashboard brand link returns to landing page
- MODE: Dashboard navigation polish seam.
- Files changed: `components/foldera/DashboardSidebar.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: The Foldera brand mark in the dashboard sidebar, mobile chrome, and stage shell now links to `/` instead of looping back to `/dashboard`. Added a Playwright regression that clicks the visible dashboard brand link and proves it lands on the public landing page.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh, Outlook fresh, mail cursors current, and `Last generation do_nothing` warning; the new Playwright test failed first against the stale `/dashboard` href, then passed after the change; `npm run build` passed; `npx playwright test tests/e2e/dashboard-navigation.spec.ts --grep "dashboard brand mark" --reporter=list` passed (`1/1`); `$env:PLAYWRIGHT_WEB_PORT='3018'; npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`16/16`); `npm run lint` passed; `git diff --check` passed.
- Unresolved issues: This was a navigation polish fix only. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Daily winner repair stops throwing away viable work
- MODE: Money-loop artifact recovery seam.
- Files changed: `lib/briefing/generator.ts`, `lib/briefing/__tests__/decision-enforced-fallback.test.ts`, `lib/briefing/__tests__/generator-runtime.test.ts`, `lib/system/winner-truth.ts`, `lib/system/__tests__/winner-truth.test.ts`, `SESSION_HISTORY.md`.
- What changed: The generator now treats one-sentence directive render failures as repairable for already-validated `send_message` and `write_document` candidates, letting the deterministic DecisionPayload fallback produce finished work instead of falling through to `do_nothing`. Winner truth now reads the correct `artifactable` flag, reports top viable candidates, and builds a concrete next action for deadline document candidates instead of echoing the candidate title as a weak action.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh, Outlook fresh, mail cursors current, and the existing `Last generation do_nothing` warning; focused Vitest passed for generator repair and winner truth (`47/47` across the selected files); `npm run winner:replay:3day` now classifies 2026-05-08 as a useful `write_document` artifact candidate; `npm run winner:autopsy` now reports `current_winner.verdict=selected`, two top viable tier-1 candidates, and no `no_safe_artifact_reason`; `npm run build` passed.
- Unresolved issues: No paid generation or outbound email was run, so production has not yet persisted a fresh live artifact row. The last stored production generation remains historical `do_nothing` until the next approved/live generation path runs.

## 2026-05-08 - Auth refresh no longer burns source connectors
- MODE: Connector durability / source friction seam.
- Files changed: `lib/auth/auth-options.ts`, `lib/auth/__tests__/auth-options.test.ts`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Removed the destructive coupling where a NextAuth session JWT refresh failure could call `softDisconnectAfterFatalOAuthRefresh()` and clear Google/Microsoft `user_tokens`. App-session refresh can still fail the session token, but connector disconnect authority stays in the background token-store/sync path that reads the persisted source row directly.
- Verification: Red test first proved `auth-options.refreshMicrosoftToken` and `auth-options.refreshGoogleToken` soft-disconnected connectors on JWT refresh `invalid_grant`; after patch, the auth regression plus token-store/user-token suites passed (`17/17`), and adjacent integrations/status, health connector, and Microsoft sync-now suites passed (`19/19`). `npm run health` still reports current Outlook reconnect required because the live Microsoft secrets were already cleared before this patch.
- Unresolved issues: Existing Microsoft row cannot be restored without real OAuth reconnect. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Mail sync stores long body text instead of preview-only evidence
- MODE: Evidence quality / source usefulness seam.
- Files changed: `lib/config/constants.ts`, `lib/sync/google-sync.ts`, `lib/sync/microsoft-sync.ts`, `lib/sync/__tests__/google-sync.test.ts`, `lib/sync/__tests__/microsoft-sync.test.ts`, `lib/system/winner-truth.ts`, `lib/system/__tests__/winner-truth.test.ts`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Added a shared 12k encrypted mail body capture cap and applied it to new Gmail and Microsoft mail signals. New synced mail rows now use `Body text:` instead of `Body preview:` so extraction/generation can see source facts below the first short preview. Winner truth now only emits `mail_sync_stores_preview_not_full_raw_body` when the configured body capture cap is still preview-sized.
- Verification: Red tests first proved the Gmail/Microsoft body helpers and winner-truth flag were missing; after patch, focused sync/winner-truth Vitest passed (`10/10`). `npm run winner:autopsy` no longer reports `mail_sync_stores_preview_not_full_raw_body`; it only reports current Microsoft reconnect/freshness as the remaining source action-needed item. `npm run health` returned `RESULT: 0 FAILING` with the expected Outlook reconnect and historical `do_nothing` warnings.
- Unresolved issues: Existing already-synced old rows are still whatever they were when ingested; richer body capture applies to new or re-synced mail. Existing Microsoft row still needs real OAuth reconnect. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Dashboard shows current best move when latest artifact is empty
- MODE: Dashboard daily-value / no-paid operator seam.
- Files changed: `app/api/conviction/daily-value/route.ts`, `app/api/conviction/daily-value/__tests__/route.test.ts`, `app/dashboard/page.tsx`, `app/dashboard/dashboard-page-model.tsx`, `lib/briefing/daily-utility-slate.ts`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `tests/e2e/authenticated-routes.spec.ts`, `tests/e2e/flow-routes.spec.ts`, `tests/dashboard/empty-first-read-pixel-lock.spec.ts`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Added a separate `/api/conviction/daily-value` projection that uses deterministic winner truth to produce a sanitized daily utility slate without importing winner-truth into the normal `/api/conviction/latest` route. The dashboard now asks for that projection only when no visible pending artifact exists, so an empty latest response can still show the current best move, evidence, why it matters, and the next safe action without pretending the work was sent, saved, or approved.
- Verification: Focused daily-value/latest/dashboard-model Vitest passed (`26/26`); `npm run build` passed and shows `/api/conviction/daily-value`; `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`17/17`) including the empty-latest current-best-move mobile proof; `npx playwright test tests/e2e/authenticated-routes.spec.ts --reporter=list` passed (`37/37`); `npm run winner:autopsy` reports fresh Google/Microsoft, current winner `selected`, `action_needed: []`, and `future_findings: []`; `npm run lint` passed.
- Unresolved issues: This is a no-paid useful-read projection, not a persisted finished artifact. Approval/save/send still require a real `pending_approval` action from the existing generation path. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Dashboard current best move became copyable value
- MODE: Dashboard daily-value actionability seam.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `components/foldera/DailyUtilitySlateCard.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: The no-paid current-best-move state now produces sanitized clipboard text and shows a `Copy brief` action. The daily-value source trail copy now describes evidence behind the move instead of saying Foldera stopped, while no-safe states still explain why Foldera held back.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh, Outlook fresh, mail cursors current, and historical `Last generation do_nothing`; `npx vitest run tests/config/__tests__/dashboard-inbox-model.test.ts --reporter=verbose` passed (`7/7`); `npm run build` passed; focused current-best-move Playwright proof passed; full `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`17/17`); `npm run lint` passed.
- Unresolved issues: This does not persist a fresh pending artifact or run a paid generation. Approval/save/send still require the existing generation path to create a real `pending_approval` action. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Recent Work hides internal no-send receipts
- MODE: History trust / source-trail receipt seam.
- Files changed: `app/api/conviction/history/route.ts`, `app/api/conviction/history/__tests__/route.test.ts`, `SESSION_HISTORY.md`.
- What changed: `/api/conviction/history` now returns only user-facing Recent Work rows (`pending_approval`, `approved`, `executed`, or real skipped work), filters internal `do_nothing` / no-send / failed validation tombstones, and builds a short artifact preview from the persisted artifact column plus `execution_result.artifact`.
- Verification: Red API tests first proved artifact previews were blank and internal no-send rows leaked; after patch `npx vitest run app/api/conviction/history/__tests__/route.test.ts --reporter=verbose` passed (`3/3`); focused dashboard History Playwright passed; legacy `/dashboard/briefings` history Playwright passed; `npm run health` reported `RESULT: 0 FAILING`; `npm run lint` passed; `npm run build` passed; full `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`17/17`).
- Unresolved issues: This is a history projection fix only. It does not create a fresh pending artifact or run paid generation. No outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Current best move no longer looks like a blocker
- MODE: Dashboard daily-value mixed-state copy seam.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: The missing-input helper now returns no blocker prompt when the daily-value slate already has a `primary_move`. This keeps the current-best-move dashboard state framed as useful value today instead of showing a contradictory `One thing Foldera needs` badge.
- Verification: Red dashboard model test first proved a primary move was being classified as an outcome blocker; after patch the focused model/history Vitest passed (`10/10`), `npm run build` passed, focused current-best-move Playwright passed, full `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`17/17`), `npm run health` reported `RESULT: 0 FAILING`, and `npm run lint` passed.
- Unresolved issues: This does not persist a fresh pending artifact or run paid generation. No outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Current best move source trail uses customer-readable labels
- MODE: Dashboard daily-value trust seam.
- Files changed: `lib/briefing/source-ref-labels.ts`, `lib/briefing/daily-utility-slate.ts`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `app/api/conviction/daily-value/__tests__/route.test.ts`, `app/dashboard/dashboard-page-model.tsx`, `tests/config/__tests__/dashboard-inbox-model.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
...
## 2026-05-08 - Controller contract enforcement becomes machine-readable
- MODE: Controller/preflight enforcement seam.
- Files changed: `.gitignore`, `.husky/pre-commit`, `.husky/pre-push`, `scripts/controller-autopilot.ts`, `scripts/preflight.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `scripts/__tests__/preflight-contract.test.ts`, `SESSION_HISTORY.md`.
- What changed: `controller:autopilot` now writes an ignored `.foldera-contract.json` on `GO` only, including base commit, normalized allowed file patterns, raw allow/deny text, proof fields, anti-regression notes, and a paste-ready next command as the final stdout line. `preflight` now validates that contract in manual, pre-commit, and pre-push modes; pre-commit checks staged files, while pre-push checks committed files since the contract base commit.
- Verification: Focused contract tests failed first against the partial stdout-only/exact-HEAD behavior, then passed after implementation (`22/22` across `controller-autopilot`, `preflight-contract`, and `preflight-core`). `npm run lint`, `npm run build`, and `git diff --check` passed.
- Unresolved issues: This is local controller guard hardening only. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Controller loop stops depending on agent judgment
- MODE: Controller/backlog contract hardening seam.
- Files changed: `CODEX_START.md`, `SYSTEM_RUNBOOK.md`, `FOLDERA_PRODUCTION_BACKLOG.md`, `scripts/controller-autopilot.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `scripts/__tests__/preflight-contract.test.ts`, `tests/config/__tests__/docs-source-of-truth.test.ts`, `SESSION_HISTORY.md`.
- What changed: Added required backlog schema for `Money loop rung`, `Is user-facing`, and `Browser proof command`, wrote the new fields into `.foldera-contract.json`, and made the controller stop when the selected seam has no `Money loop rung` or no explicit `Is user-facing` value. `preflight-contract` now fails invalid user-facing contracts that omit `browser_proof_command`. Repo doctrine now hard-codes `max_seams: 5` in the boot/runbook prompt path and requires backlog/session-history updates before the final push instead of after it.
- Verification: Focused controller/preflight/docs tests passed in a clean worktree (`27/27` across `preflight-core`, `controller-autopilot`, `preflight-contract`, and `docs-source-of-truth`). `npm run controller:autopilot` returned the expected clean `STOP` with no actionable backlog item and surfaced the new contract fields in the seam report. `npm run lint` passed. `npm run build` passed. Contract validation against this seam passed in both `pre-commit` and `pre-push` modes before push.
- Unresolved issues: The main checkout still contains unrelated dirty dashboard/runtime files outside this seam, so final verification and shipping ran from an isolated clean worktree to avoid altering them. No paid generation, outbound email, schema migration, payment action, or destructive action was run.

## 2026-05-08 - Image-first landing page direction handoff
- MODE: Homepage handoff/spec seam only.
- Files changed: `LANDING_PAGE_VISUAL_HANDOFF.md`, `FOLDERA_PRODUCTION_BACKLOG.md`, `SESSION_HISTORY.md`.
- What changed: Added one production-safe homepage handoff that treats the high-fidelity desktop/mobile mockup images as visual direction rather than editable implementation source of truth. The handoff grounds the next homepage build in the existing Foldera landing references, preserves the dark neon Foldera look, keeps the Daily Brief as the primary proof object, defines the allowed homepage sections, and forbids fake logos, fake metrics, unsupported integrations, and overbuilt long-page claims.
- Verification: `npm run health` returned `RESULT: 0 FAILING`; `npm run controller:autopilot` printed a real `CONTROLLER RESULT`; no app code changed.
- Unresolved issues: This seam only creates the implementation handoff. The future homepage implementation still needs separate responsive route proof on the real `/` page before any product-behavior claim.

## 2026-05-08 - Homepage implementation from image-first handoff
- MODE: Homepage implementation seam only.
- Files changed: `components/foldera/LandingPage.tsx`, `tests/e2e/public-routes.spec.ts`, `LANDING_PAGE_VISUAL_HANDOFF.md`, `FOLDERA_PRODUCTION_BACKLOG.md`, `SESSION_HISTORY.md`.
- What changed: Replaced the old public landing body on `/` with the production-safe homepage from `LANDING_PAGE_VISUAL_HANDOFF.md`. The real route now uses a dark neon Foldera layout with one finished move / Daily Brief-first hero, a prominent proof card, a three-part checks-sources / ships-artifact / stops-safely explanation, a protected trust line, and a short CTA/footer while keeping public nav/footer destinations and truthful claims intact. The existing handoff spec ships alongside the implementation unchanged so future homepage work keeps the same source of truth.
...
## 2026-05-09 - Low-value platform invites no longer survive as fallback daily winners
- MODE: Winner-selection seam plus delivery hardening only.
- Files changed: `lib/briefing/low-value-event-invite.ts`, `lib/briefing/scorer.ts`, `lib/briefing/artifact-taste-pack.ts`, `lib/briefing/generator.ts`, `lib/briefing/__tests__/winner-selection.test.ts`, `lib/briefing/__tests__/positive-winner-contract.test.ts`, `package.json`, `scripts/run-local-tsx.cjs`, `SESSION_HISTORY.md`.
- What changed: Added a shared low-value event invite detector so generic platform/webinar/event invites default to weak candidates unless source facts prove a direct current dependency, explicit attend intent, or accepted/protected consequence. Ranking now penalizes those invites, artifactability blocks them, and the positive winner contract treats the blocker as terminal so the pipeline can fall back to `do_nothing` instead of shipping a low-value decision packet. Also replaced the hook's `preflight` entrypoint with a repo-local `tsx` launcher so commit-time validation no longer depends on fragile Windows `npx tsx` resolution.
- Verification: `npm run build` passed; `npm run health` returned `RESULT: 0 FAILING`; focused Vitest passed across `lib/conviction/__tests__/artifact-generator.test.ts`, `lib/briefing/__tests__/artifact-quality-gate.test.ts`, `lib/cron/__tests__/daily-brief.test.ts`, `lib/briefing/__tests__/winner-selection.test.ts`, `lib/briefing/__tests__/positive-winner-contract.test.ts`, and `lib/briefing/__tests__/scorer-noise-filter.test.ts`; the unpaid deterministic replay persisted `do_nothing` / `skipped`; winner peek showed the generic Notion/platform invite no longer survived as the winner path.
- Unresolved issues: This seam is locally and deterministically proven only. No paid generation, outbound email, schema migration, or Supabase schema change was run.
...
- What changed: Replaced the homepage proof-card buttons' raw `min-h-[48px]` usage with the existing `foldera-touch-height` utility so the landing page stays inside the design-system primitive guard. Extracted the desktop dashboard shell into `components/dashboard/DashboardDesktopStage.tsx`, bringing `app/dashboard/page.tsx` back under the line-count threshold without changing dashboard data wiring or UX. Updated the dashboard brand-link browser assertion to point at the shipped BL-020 homepage contract (`landing-hero-heading` / `One finished move`) instead of the retired landing headline.
- Verification: `node node_modules/vitest/vitest.mjs run tests/config/__tests__/large-file-splits.test.ts tests/config/__tests__/design-system-buttons.test.ts --reporter=verbose` passed (`2/2`); `npm run build` passed; `node scripts/wait-for-next-build-settle.mjs` reported `Next build output settled`; focused landing Playwright passed (`12/12`); focused dashboard navigation Playwright passed (`17/17`); `npm run health` remained `RESULT: 0 FAILING`.
- Unresolved issues: Unrelated local dirt remains outside this seam in `package.json`, `scripts/run-local-tsx.cjs`, `scripts/__tests__/npm-wrapper.test.ts`, and generated log/screenshot files. No auth, connector, mail, controller, or health-script behavior changed.

## 2026-05-09 - Cost Firewall Phase 1 summary-only dashboard reads
- MODE: Supabase egress reduction seam only.
- Files changed: `app/api/conviction/latest/route.ts`, `app/api/conviction/history/route.ts`, `app/api/conviction/actions/[id]/route.ts`, `app/dashboard/page.tsx`, `app/dashboard/dashboard-page-model.tsx`, `components/foldera/DailyBriefCard.tsx`, `lib/conviction/action-read-shapes.ts`, `lib/briefing/daily-utility-slate.ts`, `lib/ml/priors.ts`, `lib/cron/aggregate-ml-global-priors.ts`, `supabase/migrations/20260509110000_create_tkg_action_summaries_view.sql`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `app/api/conviction/history/__tests__/route.test.ts`, `app/api/conviction/actions/[id]/__tests__/route.test.ts`, `app/dashboard/__tests__/dashboard-page-model.test.tsx`, `lib/briefing/__tests__/daily-utility-slate.test.ts`, `lib/ml/__tests__/priors.test.ts`, `lib/cron/__tests__/aggregate-ml-global-priors.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added the `public.tkg_action_summaries` view and switched dashboard latest/history reads to summary-only fields so list surfaces no longer pull full `artifact` or `execution_result`. Added `/api/conviction/actions/[id]` as the explicit full-detail path, kept the dashboard on summary data until the user opens finished work, and extended the dashboard card test hooks so the compact layout proves the detail handoff too. Also fixed the ML priors read/write fallback to use the live production `approve_rate` columns first while preserving older local schema compatibility.
- Verification: `npm run build` passed after the priors type fix; focused unit/route suite passed (`33/33`) across latest/history/detail/dashboard model/slate/priors/aggregate tests; focused Playwright dashboard navigation passed (`18/18`), including the new proof that initial dashboard load makes zero detail requests and only fetches full artifact detail after explicit user open. Supabase migration `create_tkg_action_summaries_view` was applied to project `neydszeamsflpghtrhue`, verified via migration list and `information_schema.columns`, and post-DDL advisor checks showed only pre-existing auth/index warnings.
- Unresolved issues: `/api/drafts/pending` still reads `execution_result` for the system draft queue and remains a separate Phase 2 list-read candidate outside this dashboard/latest/history seam. No generation logic, Anthropic call flow, budget governor, cost ledger, or RLS policy was weakened or changed.

## 2026-05-09 - Read-only user endpoints now use short private cache policy
- MODE: Read-only route cache seam only.
- Files changed: `lib/utils/read-only-user-cache.ts`, `app/api/conviction/latest/route.ts`, `app/api/conviction/daily-value/route.ts`, `app/api/conviction/history/route.ts`, `app/api/conviction/actions/[id]/route.ts`, `app/api/graph/stats/route.ts`, `app/api/integrations/status/route.ts`, `app/api/subscription/status/route.ts`, `app/api/onboard/check/route.ts`, `app/api/onboard/set-goals/route.ts`, `next.config.mjs`, `app/dashboard/page.tsx`, `app/dashboard/settings/SettingsClient.tsx`, `app/onboard/page.tsx`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `app/api/conviction/daily-value/__tests__/route.test.ts`, `app/api/conviction/history/__tests__/route.test.ts`, `app/api/conviction/actions/[id]/__tests__/route.test.ts`, `app/api/integrations/status/__tests__/route.test.ts`, `tests/config/__tests__/read-only-route-cache-policy.test.ts`, `tests/e2e/backend-safety-gates.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added one shared `private, max-age=20, stale-while-revalidate=40` helper for read-only user GETs and wired it into the dashboard, settings, history, detail, onboarding, graph, subscription, and daily-value routes. Removed steady-state dashboard/settings `no-store` fetches so normal repeat reads can reuse private browser cache, while keeping freshness-sensitive reloads explicit after approve/skip and OAuth reconnect. Also updated the `/api/integrations/:path*` header override in `next.config.mjs` so it no longer forced `no-store` over the route response.
- Verification: Focused Vitest cache-route suite passed (`31/31`); `npm run build` passed after clearing stale `.next`; targeted Playwright passed for `tests/e2e/dashboard-navigation.spec.ts` and `tests/e2e/backend-safety-gates.spec.ts` (`31 passed`, `4 skipped`), including the new authenticated runtime assertion that `/api/onboard/check`, `/api/onboard/set-goals`, `/api/integrations/status`, `/api/subscription/status`, `/api/graph/stats`, `/api/conviction/latest`, `/api/conviction/history`, and `/api/conviction/daily-value` all return the short private cache headers; `npm run health` returned `RESULT: 0 FAILING`.
- Unresolved issues: This seam changes only GET cache policy and client fetch freshness rules. It does not touch model calls, generation logic, RLS, or write paths. Local `next build` still occasionally needs a fresh `.next` wipe in this workspace before reruns.

## 2026-05-09 - Lovable demo transplanted into /demo
- MODE: Demo-only route transplant seam.
- Files changed: `app/demo/**`, `components/demo/**`, `lib/demo/**`, `tests/e2e/demo-route.spec.ts`, `tailwind.config.js`, `package.json`, `package-lock.json`, `SESSION_HISTORY.md`.
- What changed: Copied the Lovable `foldera-demo-flow` executive briefing demo into a new isolated `/demo` route tree with its own demo data store, demo components, copied Radix/button/input/textarea primitives, and demo-prefixed Tailwind theme tokens. Adapted only the router/runtime seams required for Next App Router (`next/link`, `next/navigation`, client markers, page metadata) and kept the landing page plus production dashboard untouched.
- Verification: `npm run health` returned `RESULT: 0 FAILING` with only the historical `Last generation do_nothing` warning; red-first Playwright proof for `/demo` failed before implementation and then passed after the transplant (`2/2` in `tests/e2e/demo-route.spec.ts`); landing route regression stayed green in `tests/e2e/public-routes.spec.ts --grep "landing demo uses neutral public copy and default approve language"`; `npm run build` passed after the final metadata correction and enumerated `/demo`, `/demo/audit`, and `/demo/briefs/[briefId]`; `npm run lint` passed; browser proof on a local `next start -p 3019` instance confirmed `/demo` title `Executive Briefing — Foldera`, visible executive briefing shell, visible `Audit Log` nav entry, and clean warn/error console logs.
- Unresolved issues: The in-app browser could read DOM/title/console state but timed out on screenshot capture and one live click attempt, so final interaction proof came from the passing Playwright route test instead of a browser-captured image. No paid generation, outbound email, schema migration, payment action, or destructive data action was run.


## 2026-05-09 - Anthropic budget governor blocks independent call sites before paid model spend
- MODE: Anthropic budget governor seam only.
- Files changed: `lib/llm/anthropic-budget-governor.ts`, `lib/briefing/generator.ts`, `lib/briefing/insight-scan.ts`, `lib/signals/signal-processor.ts`, `lib/signals/summarizer.ts`, `lib/cron/goal-refresh.ts`, `lib/agents/anthropic-runner.ts`, `app/api/try/analyze/route.ts`, `lib/extraction/conversation-extractor.ts`, focused tests in `lib/llm/__tests__/anthropic-budget-governor.test.ts`, `lib/briefing/__tests__/generator-runtime.test.ts`, `lib/briefing/__tests__/insight-scan.test.ts`, `lib/signals/__tests__/signal-processor.test.ts`, `lib/signals/__tests__/summarizer.test.ts`, `lib/cron/__tests__/goal-decay-signal.test.ts`, `lib/agents/__tests__/anthropic-runner.test.ts`, `app/api/try/analyze/__tests__/route.test.ts`, `lib/extraction/__tests__/conversation-extractor.test.ts`, and `SESSION_HISTORY.md`.
- What changed: Added a shared Anthropic budget governor on top of the existing monthly reserve RPC and wired it into independent Anthropic entrypoints so they fail closed before model calls when the monthly budget is exhausted. `generateDirective` keeps the existing do-nothing sentinel path, `/api/try/analyze` returns 503, agent runner returns an explicit budget error, insight scan returns an empty result, signal extraction reports budget exhaustion, weekly summarization falls back to a deterministic digest, goal refresh stops further enrichment, and conversation ingest blocks before writing the raw signal.
- Verification: `npm run health` returned `RESULT: 0 FAILING`; focused Vitest seam coverage passed across the helper, generator, insight scan, signal processor, summarizer, goal refresh, agent runner, try-analyze route, and conversation-ingest paths; `npm run build` passed.
- Unresolved issues: This seam is locally proven only. No paid Anthropic run, production deploy proof, outbound email, or Supabase query refactor was performed.

## 2026-05-10 - Cost Firewall production-proof seam added back to the actionable backlog
- MODE: Backlog/controller seam only.
- Files changed: `FOLDERA_PRODUCTION_BACKLOG.md`, `SESSION_HISTORY.md`.
- What changed: Added `BL-022` as the next rung-0 `OPEN` item for Cost Firewall production proof. The new seam builds on the shipped owner-only `/api/dev/cost-summary` route and `public.cost_events` ledger, keeps the scope inside the allowed dev/admin cost surface files, requires owner-authenticated production route proof against real ledger totals, and updates the backlog top-item pointer from waiting `BL-015` to actionable `BL-022`.
- Verification: In the clean `origin/main` worktree, `npm run health` returned `RESULT: 0 FAILING` with Gmail/mail-cursor stale warnings only, `npm run build` passed, and `git status --short --branch` showed only `FOLDERA_PRODUCTION_BACKLOG.md` and `SESSION_HISTORY.md` modified. Before the update, `npm run controller:autopilot` returned `CONTROLLER RESULT: STOP` with `Selected backlog ID: UNKNOWN` and `No actionable backlog item found`. After the update, `npm run controller:autopilot` still returned `CONTROLLER RESULT: STOP`, but it selected `BL-022` and named only the two source-of-truth files as the hard stop, proving the backlog/controller seam is actionable again without unrelated dirty files.
- Unresolved issues: The clean worktree includes only `FOLDERA_PRODUCTION_BACKLOG.md` and `SESSION_HISTORY.md` for this seam, so there are no unrelated dirty files in scope. This session adds the actionable seam only; it does not execute the cost-proof implementation or production owner-authenticated ledger check.

## 2026-05-10 - Connector freshness proof and reauth surfacing
- MODE: Connector freshness seam only.
- Files changed: `lib/integrations/connector-health.ts`, `app/api/integrations/status/route.ts`, `app/api/settings/run-brief/route.ts`, `app/dashboard/settings/SettingsClient.tsx`, `scripts/health.ts`, `scripts/health-connectors.ts`, `lib/integrations/__tests__/connector-health.test.ts`, `app/api/integrations/status/__tests__/route.test.ts`, `app/api/settings/run-brief/__tests__/route.test.ts`, `scripts/__tests__/health-connectors.test.ts`, `tests/e2e/authenticated-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Added one shared connector-health classifier that turns `user_tokens` rows into `fresh`, `stale`, `disconnected`, `reauth_required`, and `never_synced` statuses plus deterministic reauth/reconnect instructions. Reused that truth in `/api/integrations/status`, the settings page stale/reconnect UI, `npm run health`, and the pre-generation `settings/run-brief` guard so Google stale vs Microsoft fresh is surfaced consistently before any generation attempt.
- Verification: Focused Vitest seam suite passed (`37/37`) across connector-health, integrations status, run-brief, and health-script tests; `npm run build` passed; focused Playwright settings proof passed for `tests/e2e/authenticated-routes.spec.ts --grep "shows stale Google clearly while Microsoft stays fresh"`; live `npm run health` returned `⚠ Gmail stale 43h ago - Refresh Google before the next generation if newer mail or calendar updates should be present.`, `✓ Outlook fresh 3h ago · b-kapp@outlook.com`, and `RESULT: 0 FAILING`.
- Unresolved issues: This seam does not change artifact-quality selection, paid generation logic, sync workers, or Supabase schema. `.foldera-contract.json` remained dirty outside this seam and was left untouched.

## 2026-05-10 - Low-authority event invites no longer win on context-smart framing alone
- MODE: Artifact winner-selection seam only.
- Files changed: `.foldera-contract.json`, `lib/briefing/low-value-event-invite.ts`, `lib/briefing/artifact-taste-pack.ts`, `lib/briefing/generator.ts`, `lib/briefing/__tests__/low-value-event-invite.test.ts`, `lib/briefing/__tests__/positive-winner-contract.test.ts`, `lib/briefing/__tests__/winner-selection.test.ts`, `SESSION_HISTORY.md`.
- What changed: Extended the shared low-value event invite assessor so it now records `contextSmart`, `changesNextMove`, `sourceAuthority`, and an internal silent-suppression receipt for spam/promotional webinar/platform invites. Wired that into candidate artifactability and the positive winner contract so low-authority event invites are blocked before generation, require direct next-move impact to compete, and surface as silent suppression receipts instead of user-facing “skip/decide whether to attend” artifacts. Preserved the direct-dependency escape hatch for real current product/customer/revenue blockers, and updated `.foldera-contract.json` so the repo preflight gate matches this exact seam instead of the prior docs-only contract.
- Verification: Focused Vitest seam suite passed (`37/37`) across `low-value-event-invite`, `positive-winner-contract`, and `winner-selection`; adjacent `lib/system/__tests__/winner-truth.test.ts` passed (`5/5`); `npm run build` passed; `npm run health` returned `RESULT: 0 FAILING` with Gmail stale `43h ago`, Outlook fresh `4h ago`, mail cursors stale for Google, and last generation `do_nothing`.
- Unresolved issues: This seam does not change paid generation, LLM artifact-quality output logic, sync freshness, or dashboard/browser surfaces. Silent suppression is currently carried in internal winner/artifactability receipts, not a new user-facing product surface.

## 2026-05-10 - Homepage front door upgraded around the Daily Brief proof
- MODE: Public homepage seam only.
- Files changed: `app/page.tsx`, `components/foldera/LandingPage.tsx`, `components/nav/NavPublic.tsx`, `tests/e2e/public-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Rebuilt `/` into a tighter premium front door that centers Foldera’s real product contract in the first viewport: connected sources in, one finished move out, source trail visible, and approval before anything sends. The nav now points visitors to Product / Security / Pricing with a primary `See demo` CTA, the hero drives directly into `/demo`, the Daily Brief proof object is the dominant visual instead of generic SaaS filler, the middle of the page explains `Signals in` vs `Finished move out`, and the safety section keeps the trust line truthful without inventing logos, metrics, or unsupported claims.
- Verification: `npm run health` returned `RESULT: 0 FAILING`; `npm run lint` passed; `npm run build` passed twice, including after the final copy adjustment; focused Playwright landing proof passed in `tests/e2e/public-routes.spec.ts --grep "Landing page /"` (`12/12`); focused mobile homepage proof passed in `tests/e2e/mobile-visual-qa.spec.ts --grep "scrollWidth ≤ viewport: home|opens full-screen overlay, closes on X"` (`2/2`); local in-app browser proof loaded `/` with title `Foldera` at desktop and mobile sizes and loaded `/demo` with title `Executive Briefing — Foldera`; fresh screenshots were captured to `artifacts/verification/homepage-desktop-1440.png` and `artifacts/verification/homepage-mobile-390.png`.
- Unresolved issues: This seam intentionally does not change `/demo` internals, auth flows, pricing logic, backend behavior, or any dashboard/product execution path beyond the public homepage entry experience.

## 2026-05-10 - BL-022 Cost Firewall production proof closed without product changes
- MODE: Backlog/proof seam only.
- Files changed: `.foldera-contract.json`, `FOLDERA_PRODUCTION_BACKLOG.md`, `SESSION_HISTORY.md`.
- What changed: Closed BL-022 after proving the existing owner-only `/api/dev/cost-summary` route already exposes real production `cost_events` totals without widening the seam into UI or generator work. Kept the controller-generated BL-022 contract file in sync so preflight and commit-time scope enforcement still match the executed seam. Updated the backlog top-item pointer to reflect that no actionable `OPEN` item remains and the remaining backlog is waiting passive or external proof.
- Verification: `npm run controller:autopilot` selected BL-022 with `money_loop_rung: prove_unit_economics`; `npm run preflight` passed contract validation and reported infrastructure degraded only because local paid LLM is unset; unauthenticated production `GET https://www.foldera.ai/api/dev/cost-summary` returned `401 {"error":"Unauthorized"}`; owner-authenticated production `GET https://www.foldera.ai/api/dev/cost-summary` returned `200` with `last_24h = last_7d = last_30d = $0.023245`, `event_count = 5`, `input_tokens = 18955`, `output_tokens = 2020`, endpoint breakdown `insight_scan $0.017973 (1)`, `goal_refresh $0.003556 (3)`, `signal_extraction $0.001716 (1)`, and model breakdown `claude-haiku-4-5-20251001 $0.023245 (5)`; an independent production `cost_events` rollup via the service-role DB path matched the route output exactly (`row_count_30d = 5`); `npx vitest run app/api/dev/cost-summary/__tests__/route.test.ts --reporter=verbose` passed (`3/3`); `npm run build` passed; `npm run health` returned `RESULT: 0 FAILING`; `npm run preflight` passed.
- Unresolved issues: No new product blocker was found in BL-022. After this closeout, the remaining backlog items are waiting passive proof or external blockers rather than an actionable code seam.

## 2026-05-11 - Scheduled production smoke CTA contract restored and heartbeat checkout hardened
- MODE: CI reliability seam only.
- Files changed: `.github/workflows/pipeline-cron-heartbeat.yml`, `components/foldera/LandingPage.tsx`, `tests/e2e/public-routes.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Restored the locked public `/start` CTA wording on the homepage from `Start free` back to `Get started free` in both visible CTA slots so the scheduled production smoke contract matches the shipped page again. Added a focused local landing assertion that both `/start` links keep that exact copy. Hardened the scheduled heartbeat workflow to match the working production-e2e checkout path by upgrading to `actions/checkout@v5` and `actions/setup-node@v5`, pinning checkout to `ref: main`, and moving the lane to Node 22 so it no longer depends on the failing exact-SHA fetch pattern or Node 20 deprecation.
- Verification: `npm run health` returned `RESULT: 0 FAILING`; the new landing CTA assertion failed first in `tests/e2e/public-routes.spec.ts` because both `/start` links rendered `Start free`; after the copy fix, `npx playwright test tests/e2e/public-routes.spec.ts --grep "Landing page /"` passed (`13/13`); `npm run build` passed after clearing stale `.next` / `.turbo` output and stopping lingering Windows `next build` processes; live production pre-push proof showed `https://www.foldera.ai/` title `Foldera`, `hasGetStartedFree=false`, `hasStartFree=true`, and both `/start` links still rendering `Start free`; GitHub Actions run `Production E2E` #1094 failed only on `tests/production/smoke.spec.ts` `pricing CTA says "Get started free"`, while `Pipeline cron heartbeat` #34 failed inside `actions/checkout@v4` before repo code ran after three `git fetch` attempts for exact SHA `5186327...` returned GitHub HTTP 500.
- Unresolved issues: Live production proof still requires push + deploy so the restored CTA copy can be re-checked on `https://www.foldera.ai/`. The heartbeat workflow hardening is locally reasoned from the failing fetch path and still needs a post-push GitHub Actions run to prove the lane now clears checkout.

## 2026-05-11 - Pipeline cron heartbeat now uses a strict same-day UTC service window
- MODE: CI reliability seam only.
- Files changed: `scripts/pipeline-cron-heartbeat-check.ts`, `scripts/__tests__/pipeline-cron-heartbeat-check.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Root-caused the remaining heartbeat failure to same-day query semantics, not a missing daily brief. Replaced the rolling `Date.now() - 3h` check with a strict UTC service window that starts at today's expected daily-brief lower bound (`11:00 UTC` by default, env-configurable) and ends at `now`, then queries `pipeline_runs` for `phase = cron_complete`, `invocation_source = daily_brief`, `created_at >= lower_bound`, and `created_at <= now`. Also removed import-time execution from the heartbeat script so the new helper can be tested without firing a live query on module import.
- Verification: `npm run health` returned `RESULT: 0 FAILING`; `npx vitest run scripts/__tests__/pipeline-cron-heartbeat-check.test.ts --reporter=verbose` passed (`4/4`); `GITHUB_EVENT_NAME=schedule`, `GITHUB_RUN_ATTEMPT=2`, `npm run check:pipeline-heartbeat` passed with `Checking daily_brief cron_complete window: 2026-05-11T11:00:00.000Z -> ... UTC` and `OK: daily_brief cron_complete count=1 in UTC window ...`; `npm run build` passed; GitHub `workflow_dispatch` run `Pipeline cron heartbeat` `#35` on `main` commit `4dafbb3d45e16a8c75171bfd5104ce8cf0928182` completed `success` at `https://github.com/pm6guy10/foldera-ai/actions/runs/25693902390`.
- Unresolved issues: The old failed rerun `#34` on commit `5186327` remains in GitHub history, but the current hosted replacement proof is green. This seam does not change daily-brief generation behavior or any paid model path.

## 2026-05-11 - Scheduled check reliability class narrowed to real failures and future-noise drift
- MODE: CI reliability seam only.
- Files changed: `.github/workflows/production-e2e.yml`, `.github/workflows/weekly-audit.yml`, `tests/production/smoke.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Classified current scheduled red history by root bucket instead of treating every old cron failure as current truth. `Pipeline cron heartbeat` is now a closed monitor-semantics/platform seam with fresh green replacement runs; `Production E2E` is a closed real product regression on the locked homepage CTA copy with fresh green replacement deployment-status runs. The remaining scheduled-class drift was environment noise from `actions/upload-artifact@v4` in the cron Playwright/audit lanes, so both scheduled workflows now use `actions/upload-artifact@v6`, log what success means, and the production smoke CTA assertion is scoped directly to the visible landing `/start` link contract instead of generic page text.
- Verification: `npx playwright test tests/production/smoke.spec.ts --config playwright.prod.config.ts --grep 'locked /start CTA copy stays "Get started free"'` passed (`1/1`) against the live site; `npm run build` passed; `npm run health` returned `RESULT: 0 FAILING`; GitHub API truth before push showed current-main `Production E2E` deployment-status runs `#1096`, `#1097`, and `#1098` green, `Pipeline cron heartbeat` workflow-dispatch runs `#35` and `#36` green, and the only remaining scheduled-class issue in code was the Node-20 deprecation warning from `upload-artifact@v4`.
- Unresolved issues: Fresh hosted proof was pending at ship time. No product code changed in this seam.

## 2026-05-11 - Scheduled production monitor hardening hosted proof closed on current main
- MODE: CI reliability seam only.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Recorded the fresh hosted proof for the updated scheduled-monitor workflows after push. `Production E2E` now proves the locked landing `/start` CTA contract directly and no longer carries the targeted artifact-action deprecation risk; `Weekly Production Audit` also moved to the non-deprecated artifact runtime while keeping its report-only best-effort contract.
- Verification: GitHub `Production E2E` `#1099` (`workflow_dispatch`, commit `9a3b420142891babc859c673144be56eeabec7b5`) completed `success` at `https://github.com/pm6guy10/foldera-ai/actions/runs/25695446490`; GitHub `Weekly Production Audit` `#8` (`workflow_dispatch`, same commit) completed `success` at `https://github.com/pm6guy10/foldera-ai/actions/runs/25695447427`; direct job-log inspection confirmed `actions/upload-artifact@v6` was present and the targeted `Node.js 20 actions are deprecated` warning was absent in both runs.
- Unresolved issues: Old historical reds remain visible (`Production E2E` `#1094`, `Pipeline cron heartbeat` `#34`), but they now have fresh green current-main replacement runs and should not be treated as open scheduled failures.

## 2026-05-11 - Controller now generates app-owner fallback contracts from current truth
- MODE: Controller/autopilot seam only.
- Files changed: `.foldera-contract.json`, `scripts/controller-autopilot.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Replaced the old waiting-only stop rule in `controller:autopilot` with app-owner fallback generation. The controller now reads `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, latest `SESSION_HISTORY.md`, and fresh health output before deciding to stop. When backlog rows are all waiting/blocked/closed, it synthesizes exactly one generated contract from current product truth instead of returning `Selected backlog ID: UNKNOWN`, and it only hard-stops with an exact external blocker when every remaining money-loop rung is truly externally blocked. Machine-readable contracts now carry `generated_contract_id`, `user_system_path`, `required_product_proof`, `acceptance_condition`, and `stop_condition`.
- Verification: `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts --reporter=verbose` passed (`15/15`), covering generated fallback GO, exact external-blocker STOP, no normal `UNKNOWN` stop, and richer contract fields. `npm run controller:autopilot` now returns `CONTROLLER RESULT: GO` with `GENERATED-CANDIDATE-SELECTION-CONVERGENCE` and `money_loop_rung: candidate_selection` instead of `UNKNOWN`. `npm run health` returned `RESULT: 0 FAILING`. `npm run build` passed.
- Unresolved issues: This seam changes controller/runtime contract selection only; it does not execute the generated convergence contract or change product behavior. The generated fallback currently points at `GENERATED-CANDIDATE-SELECTION-CONVERGENCE`, while BL-015/003/005 remain paid/quota blocked, BL-006 remains blocked on a real non-owner account, and BL-011 remains blocked on passive next-window proof.

## 2026-05-11 - Convergence now counts known entity email aliases across calendar and drive
- MODE: Generated convergence seam only.
- Files changed: `lib/briefing/discrepancy-detector.ts`, `lib/briefing/__tests__/discrepancy-detector.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Executed only `GENERATED-CANDIDATE-SELECTION-CONVERGENCE`. Added a regression where the same entity appears across email, calendar, and drive, but the calendar/drive signals carry only the known entity email alias and a generic title rather than the full name. Narrowed `extractConvergence` so exact known-entity email matches count toward convergence buckets alongside full-name hits. This closes the under-match without widening broader candidate selection or touching paid generation, outbound email, Stripe, schema, or DB state.
- Verification: Red-first `node node_modules/vitest/vitest.mjs run lib/briefing/__tests__/discrepancy-detector.test.ts --reporter=verbose` failed on the new convergence alias case, then passed after the fix (`110/110`). `npm run winner:autopsy` passed after the change and remained grounded: current winner still reports `no_safe_artifact_today`, `weak_risk`, no graph drift, and `action_needed: []`. `npm run health` returned `RESULT: 0 FAILING`. `npm run build` passed.
- Unresolved issues: This seam proves the deterministic convergence under-match only. It does not yet prove that a live production winner changes because of this specific path, and it does not touch the remaining paid/quota/non-owner/passive blockers.

## 2026-05-11 - Generated contracts can now close their own source-truth findings
- MODE: Controller/autopilot seam only.
- Files changed: `.foldera-contract.json`, `scripts/controller-autopilot.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `scripts/__tests__/preflight-contract.test.ts`, `CURRENT_STATE.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Fixed the generated-contract closure rule so app-owner fallback contracts now carry `source_truth_file`, `source_truth_finding`, and `required_closure_update`, and they automatically allow the exact source-truth file needed to retire the triggering finding plus the receipt files. Added a preflight guard that rejects generated contracts whose `source_truth_file` is not allowed and rejects `SESSION_HISTORY.md` rewrites unless the contract explicitly requires a history correction. Updated `CURRENT_STATE.md` so the old convergence name-overlap finding now truthfully records that known entity email aliases were fixed in commit `20e4f12`, while broader unproven convergence risk remains open only at the weaker evidence boundary.
- Verification: Focused controller/preflight regression proof passed in `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts scripts/__tests__/preflight-contract.test.ts --reporter=verbose` (`26/26`). `npm run controller:autopilot` on a clean tree no longer emitted `GENERATED-CANDIDATE-SELECTION-CONVERGENCE`; it selected `GENERATED-USEFUL-CURRENT-MOVE-DAILY-VALUE` with closure metadata pointing at `CURRENT_STATE.md`. `npm run health` returned `RESULT: 0 FAILING`. `npm run build` passed.
- Unresolved issues: This seam changes controller/runtime closure behavior only. It does not execute the new useful-current-move generated contract, and the paid/quota/non-owner/passive blockers remain unchanged.

## 2026-05-11 - Useful current move fallback is proven when no pending artifact exists
- MODE: Generated daily-value seam only.
- Files changed: `.foldera-contract.json`, `CURRENT_STATE.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Executed only `GENERATED-USEFUL-CURRENT-MOVE-DAILY-VALUE`. The product code did not need further changes: the existing daily-value route, slate shaping, dashboard fallback model, and current-best-move dashboard path already satisfied the contract. Closed the exact `CURRENT_STATE.md` finding that treated this fallback as an open broken seam, and recorded that the dashboard path is now proven to show one grounded useful move with source trail and non-contradictory copy behavior when `/api/conviction/latest` has no visible pending artifact.
- Verification: `node node_modules/vitest/vitest.mjs run app/api/conviction/daily-value/__tests__/route.test.ts lib/briefing/__tests__/daily-utility-slate.test.ts tests/config/__tests__/dashboard-inbox-model.test.ts --reporter=verbose` passed (`16/16`). `node node_modules/@playwright/test/cli.js test tests/e2e/dashboard-navigation.spec.ts --grep "current best move" --reporter=list` passed (`1/1`).
- Unresolved issues: This seam proves the current-best-move fallback path only. It does not convert the latest persisted artifact path into a finished approval/send artifact, and it does not change paid/quota/non-owner/passive blockers.

## 2026-05-12 - Dashboard app shell split cleanly from unproven public-nav auth
- MODE: Dashboard shell seam only.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `app/globals.css`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/foldera/DashboardSidebar.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Rebuilt `/dashboard` so the authenticated route is the real full-screen app surface on desktop and mobile instead of a framed preview composition. Removed the fixed-canvas and scale wrapper, removed the fake mobile phone shell, kept panel switching inside the same dashboard route, replaced the dead loading box with a Daily Brief skeleton, and turned the empty/no-source states into real Daily Brief artifacts. Explicitly split `NavPublic.tsx` out of this seam because auth-aware landing proof remained unverified under a separate root-route hydration and static-asset issue.
- Verification: `npm run lint` passed. `npm run build` passed. `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`19/19`). Fresh local proof screenshots were captured to `artifacts/verification/dashboard-contract-desktop.png` and `artifacts/verification/dashboard-contract-mobile.png`, and both show the real `/dashboard` route filling the viewport without the old centered mock frame or fake phone shell. `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `1h ago`, Outlook fresh `1h ago`, mail cursors current, and last generation `do_nothing`.
- Unresolved issues: Public-nav auth awareness is intentionally deferred. Do not mix `NavPublic.tsx` back into the dashboard seam until `/` hydration is proven and browser proof confirms logged-out `Sign in + Start free` and logged-in `Dashboard` behavior.

## 2026-05-12 - Controller fallback contracts now require a live current finding
- MODE: Controller/autopilot seam only.
- Files changed: `scripts/controller-autopilot.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Fixed the fallback-contract generator so it only emits app-owner seams from live current truth. `source_freshness` is now derived from the current health line itself instead of broad stale keywords in old handoff text, generated contracts carry exact source-truth lines that are present in the current source file or health output, and the controller refuses any generated contract whose `source_truth_finding` is absent. This prevents closed or stale findings from being reselected, including the false `GENERATED-SOURCE-FRESHNESS-CONNECTOR-HEALTH` seam.
- Verification: `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts --reporter=verbose` passed (`19/19`) with new coverage for fresh Gmail/Outlook + current cursors, stale closed handoff truth, and live `source_truth_finding` validation. `npm run controller:autopilot` no longer emitted `GENERATED-SOURCE-FRESHNESS-CONNECTOR-HEALTH`; it now returned `CONTROLLER RESULT: STOP` with exact blocker `All remaining money-loop rungs are externally blocked by paid/model-backed proof.` `npm run health` returned `RESULT: 0 FAILING`. `npm run build` passed.
- Unresolved issues: The remaining queue state is still externally blocked by paid/quota proof (`BL-015`, `BL-003`, `BL-005`), a real non-owner account (`BL-006`), and passive next-window proof (`BL-011`). Unrelated dashboard shell work was quarantined in stash `quarantine-dashboard-shell-before-controller-live-finding-fix` and was not part of this seam.

## 2026-05-12 - Clean-tree post-commit controller proof build blocker corrected as stale local drift
- MODE: Verification receipt only.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Corrected the stale receipt that treated `app/dashboard/page.tsx` missing `stageTransform` as a current clean-tree build blocker. The failure came from local dashboard drift, not current tracked `main`; no source files changed, and only the untracked `.codex-dashboard-build.log` was removed during proof cleanup.
- Verification: `npm run build` passed. `npm run lint` passed. `git status --short` was clean. `origin/main` matched current head `fc46bae`.
- Unresolved issues: No clean-tree build blocker remains from the stale `stageTransform` report. The controller fix remains pushed, and the remaining queue state is still external paid/quota/account/passive proof.

## 2026-05-12 - Dashboard shell shipped as its own seam; public nav deferred cleanly
- MODE: Dashboard app-shell seam only.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `app/globals.css`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/foldera/DashboardSidebar.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Restored `/dashboard` as the real full-screen authenticated app surface on desktop and mobile. Removed the fixed-canvas desktop frame, removed the fake mobile phone shell, kept Today/Sources/Recent Work/Account panel switching inside one route, replaced the dead loading box with a Daily Brief skeleton, and upgraded the no-action and no-source fallbacks into real Daily Brief artifacts. Kept auth-aware public nav out of this change set because the root `/` hydration/static-asset proof remains a separate unresolved seam.
- Verification: `npm run lint` passed. `npm run build` passed. `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`19/19`). `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `2h ago`, Outlook fresh `2h ago`, `Mail cursors current`, and last generation `do_nothing`. Fresh proof screenshots were captured to `artifacts/verification/dashboard-contract-desktop-fresh.png` and `artifacts/verification/dashboard-contract-mobile-fresh.png`.
- Unresolved issues: Public-nav auth awareness is still intentionally deferred. Do not mix `NavPublic.tsx` or public-route auth tests into this seam until `/` hydration and logged-in/logged-out browser proof are working.

## 2026-05-12 - Dashboard app-fit shell and Today source trail
- MODE: Dashboard app-fit source-trail surface only.
- Files changed: `.foldera-contract.json`, `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `app/globals.css`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/foldera/DailyBriefCard.tsx`, `components/foldera/RightPanel.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Used the Lovable matrix only as a screen/state checklist and implemented one live Foldera dashboard surface. `/dashboard` now behaves as a fixed-height app shell on desktop and mobile: document/body do not scroll beyond the viewport, desktop chrome/card sizing adapts to available height, mobile keeps the bottom nav in-app, and overflow is contained in the active card/panel region instead of the page. The Today support rail also renders source labels and evidence from the active finished move's existing discrepancy/source trail while preserving the existing fallback explainer when no grounded trail is present.
- Verification: `npm run lint` passed. `npm run build` passed after clearing stale `.next` output. `PLAYWRIGHT_WEB_PORT=3018 npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`19/19`) with assertions for no document/body vertical scroll, source-trail rail labels/evidence, no raw `email:`/`calendar:` refs, mobile no fake frame/no horizontal overflow, in-shell panel navigation, and no-source fallback. Fresh proof screenshots were captured to `test-results/dashboard-fit-proof/dashboard-desktop-fit.png` and `test-results/dashboard-fit-proof/dashboard-mobile-fit.png`.
- Unresolved issues: Public-nav auth awareness remains intentionally deferred. Continue matrix coverage one real Foldera surface at a time rather than copying the compressed Lovable matrix visual style.

## 2026-05-12 - Dashboard visual seam CI proof ladder repaired
- MODE: Dashboard/UI CI repair only.
- Files changed: `.foldera-contract.json`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/page.tsx`, `app/globals.css`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardStateCards.tsx`, `components/foldera/DailyBriefCard.tsx`, `components/foldera/DashboardSidebar.tsx`, `components/foldera/RightPanel.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `tests/e2e/dashboard-navigation.spec.ts`.
- What changed: Repaired the CI failures for the dashboard visual seam without reverting the full-screen app shell. `app/dashboard/page.tsx` is below the 1000-line split threshold, the authenticated dashboard lane sees `dashboard-empty-state` again, notification affordances are visible, non-owner account copy is neutral, write-document support remains visible, and post-skip empty state remains testable. The old fake stats/search assertions were narrowed to the intended full-screen shell contract.
- Verification: `npm run build` passed. `npm run lint` passed. `npx vitest run tests/config/__tests__/large-file-splits.test.ts --reporter=verbose` passed. `npx playwright test tests/e2e/dashboard-navigation.spec.ts tests/e2e/authenticated-routes.spec.ts --reporter=list` passed (`57/57`). `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `4h ago`, Outlook fresh `4h ago`, `Mail cursors current`, and last generation `do_nothing`.
- Unresolved issues: GitHub CI green proof is pending until the repair commit is pushed. Public-nav auth awareness remains intentionally deferred.

## 2026-05-12 - Dashboard/UI proof now must match the affected CI lane
- MODE: Doctrine/proof-rule seam only.
- Files changed: `AGENTS.md`, `ACCEPTANCE_GATE.md`, `scripts/__tests__/brandon-doctrine.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Encoded the permanent CI-parity proof rule: for every seam, local proof must include the affected CI lane, and dashboard/UI work specifically requires build, lint, large-file-splits, dashboard-navigation, and authenticated-routes. Added doctrine regression coverage so this proof ladder cannot silently drift out of the active acceptance gate.
- Verification: `npx vitest run scripts/__tests__/brandon-doctrine.test.ts tests/config/__tests__/docs-source-of-truth.test.ts --reporter=verbose` passed (`6/6`). `npm run build` passed. `npm run lint` passed. `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `4h ago`, Outlook fresh `4h ago`, `Mail cursors current`, and last generation `do_nothing`.
- Unresolved issues: GitHub CI proof remains required after push. Stash `quarantine-dashboard-current-brief-mobile-css` preserves an unrelated uncommitted mobile current-brief stylesheet tweak outside this doctrine seam.

## 2026-05-12 - Dashboard visual CI repair follow-up closes clean-tree authenticated lane
- MODE: Dashboard/UI CI repair only.
- Files changed: `.foldera-contract.json`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `app/globals.css`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/foldera/DashboardSidebar.tsx`, `components/foldera/RightPanel.tsx`, `tests/e2e/dashboard-navigation.spec.ts`.
- What changed: Fixed the remaining clean-tree dashboard lane failures after the visual shell shipment. Restored visible desktop/mobile notification affordances, removed the remaining `Workspace Owner` copy from the non-owner shell, kept the account menu reachable by both account/open-menu labels, and added the app-fit classes/CSS/coverage needed for the mobile card/footer to stay above the in-shell bottom nav without a fake phone wrapper.
- Verification: `npm run build` passed. `npm run lint` passed. `npx vitest run tests/config/__tests__/large-file-splits.test.ts --reporter=verbose` passed. `npx playwright test tests/e2e/dashboard-navigation.spec.ts tests/e2e/authenticated-routes.spec.ts --reporter=list` passed (`57/57`). `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `5h ago`, Outlook fresh `5h ago`, `Mail cursors current`, and last generation `do_nothing`.
- Unresolved issues: GitHub CI green proof remains required after push. Public-nav auth awareness remains intentionally deferred.

## 2026-05-12 - Dashboard dynamic viewport app-fit tightened
- MODE: Dashboard app-fit seam only, with A-Z route proof readout.
- Files changed: `app/globals.css`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Tightened the existing full-screen dashboard shell so `/dashboard` behaves more like a screen-fitted app instead of a locked/clipped page. The Daily Brief stage now uses denser draft spacing at desktop heights, and mobile keeps the finished draft visible in the fixed app shell while the bottom nav remains in-flow rather than behaving like a fake phone frame. No backend/API, source freshness, paid generation, outbound email, Stripe, schema, or public-nav auth behavior changed.
- Verification: `npm run build` passed. `npm run lint` passed. `npx vitest run tests/config/__tests__/large-file-splits.test.ts --reporter=verbose` passed. `npx playwright test tests/e2e/dashboard-navigation.spec.ts --reporter=list` passed (`19/19`). `npx playwright test tests/e2e/authenticated-routes.spec.ts --reporter=list` passed (`38/38`). `npx playwright test tests/e2e/public-routes.spec.ts --reporter=list` passed (`50/50`). `npx playwright test tests/e2e/mobile-visual-qa.spec.ts --reporter=list` passed (`11/11`). Fresh `/dashboard` screenshots were captured outside the repo at 1440x900 and 390x844 with html/body scroll dimensions equal to viewport dimensions. `npm run health` returned `RESULT: 0 FAILING` with Gmail fresh `5h ago`, Outlook fresh `5h ago`, mail cursors current, and last generation `do_nothing`.
- Unresolved issues: GitHub CI green proof remains required after push. Public-nav auth awareness is still a separate unproven seam; continue A-Z route-fit work one surface at a time and do not mix it with source_freshness.

## 2026-05-12 - Frontend surfaces share one Foldera app contract
- MODE: Frontend surface contract A-Z, excluding already-shipped dashboard app-fit internals.
- Files changed: `app/page.tsx`, `app/globals.css`, `app/login/login-inner.tsx`, `app/start/page.tsx`, `app/onboard/page.tsx`, `components/nav/NavPublic.tsx`, `components/foldera/LandingPage.tsx`, `components/dashboard/ProductShell.tsx`, `tests/e2e/public-routes.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Made the landing nav auth-aware from the server so logged-out users see `Sign in` + `Start free`, while logged-in users see `Dashboard` instead of `Sign in` without adding a client session poll. Unified legacy app routes behind a wider matte Foldera `ProductShell` with cyan app chrome and mobile section tabs, and brought sign-in/start/onboarding onto the same premium app surface. Preserved dashboard app-fit, OAuth, source freshness, Stripe, outbound email, and schema contracts.
...
- Files changed: `.foldera-contract.json`, `scripts/controller-autopilot.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `scripts/__tests__/preflight-contract.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Verified the frontend surface contract follow-up is pushed to `main`, GitHub check-runs on `f0c1b15` are green, Production E2E is green, and production `/api/health` serves `f0c1b15` from deployment `dpl_7vFTKqtZfEkA6QG2ri5JRYs3dpdQ`. Fixed the controller STOP path so it deletes stale `.foldera-contract.json` instead of leaving a finished contract as machine-readable executable work, and aligned preflight so this narrow controller STOP cleanup can be committed/pushed without weakening normal contract-required seams.
- Verification: `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts scripts/__tests__/preflight-contract.test.ts --reporter=verbose` passed (`32/32`). `npm run build` passed. `npm run lint` passed. `npm run controller:autopilot` returned `CONTROLLER RESULT: STOP` with only external money-loop blockers and removed `.foldera-contract.json`. `npm run health` returned `RESULT: 0 FAILING`.
...
## 2026-05-12 - Persisted artifact path blocks before persistence
- MODE: Persisted artifact path proof only; no frontend, no product code, no paid generation, no outbound email, no Stripe, no schema/destructive DB action.
- Files changed: `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`, `scripts/preflight-contract.ts`, `scripts/__tests__/preflight-contract.test.ts`.
- What changed: Recorded the exact current truth for the persisted-artifact path. The approve/skip/latest/detail/history plumbing is deterministic-green, and past production `write_document` artifact previews remain visible in history, but the current owner-day path has no selected safe artifact/current best move to persist. First broken rung: `candidate selected/current best move -> artifact/current move`. Also fixed the contractless STOP preflight allowlist so proof-mode source-truth receipts can include `CURRENT_STATE.md` without fabricating a generated contract.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run controller:autopilot` returned `CONTROLLER RESULT: STOP` with only external/passive blockers; `npm run winner:autopsy` returned `current_winner.verdict=no_safe_artifact_today`; read-only live DB proof found `0` owner `pending_approval` summary rows and recent owner `do_nothing` no-send actions; deterministic winner-truth daily-value returned `daily_utility_slate=null`; `npx vitest run lib/conviction/__tests__/execute-action.test.ts app/api/conviction/execute/__tests__/route.test.ts 'app/api/conviction/actions/[id]/__tests__/route.test.ts' app/api/conviction/history/__tests__/route.test.ts app/api/conviction/daily-value/__tests__/route.test.ts app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts lib/briefing/__tests__/daily-utility-slate.test.ts --reporter=verbose` passed (`50/50`); `node node_modules/vitest/vitest.mjs run scripts/__tests__/preflight-contract.test.ts --reporter=verbose` passed; `npm run build` passed.
- Unresolved issues: GitHub CLI is unavailable in this workspace, and the GitHub connector returned no workflow runs for current main commit `601afc85cc76cd2a6f11fa947db84daffc035b55`, so fresh CI proof is unavailable rather than red. Do not force golden proof data or paid/model-backed generation without explicit approval.

## 2026-05-12 - Candidate selection memory stops over-filtering current admin-deadline moves
- MODE: Product/briefing quality mode only; no frontend, no persistence plumbing, no paid generation, no golden artifact proof, no outbound email, no Stripe, no schema/destructive DB action.
- Files changed: `lib/briefing/discrepancy-card-frame.ts`, `lib/briefing/__tests__/discrepancy-card-frame.test.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Fixed the first broken rung between candidate pool and winner selection. The candidate pool was present and source sync was fresh enough, but learned noisy-pattern memory treated operational auto-suppressed proof rows as user-negative skips and hard-blocked broad `candidate:discrepancy` category keys. The memory layer now ignores operational auto-suppressed skip reasons for negative learning and no longer hard-blocks broad `candidate:*` keys, while keeping specific noisy patterns such as `generic:follow_up` blocked.
- Verification: `npm run health` passed before and after (`RESULT: 0 FAILING`); `npm run winner:autopsy` before returned `current_winner.verdict=no_safe_artifact_today`; diagnostic replay showed Tier 1 admin-deadline candidates blocked by `discrepancy_pattern_memory:noisy_pattern:candidate:discrepancy`; `node node_modules/vitest/vitest.mjs run lib/briefing/__tests__/discrepancy-card-frame.test.ts lib/briefing/__tests__/winner-selection.test.ts lib/briefing/__tests__/positive-winner-contract.test.ts --reporter=verbose` passed (`44/44`); `npm run winner:autopsy` after returned `current_winner.verdict=selected` for the WorkSourceWA account-activity deadline as Tier 1 `admin_deadline_decision_packet`; deterministic daily-value proof returned a non-null `daily_utility_slate.primary_move`; `npm run build` passed.
- Unresolved issues: The selected WorkSourceWA current move is not yet persisted as a finished artifact/action/history row. Next seam should prove deterministic/no-paid selected-move-to-artifact persistence or stop at the exact model-backed generation requirement.

## 2026-05-12 - Controller emits selected-move persistence seam after candidate selection fix
- MODE: Controller/source-truth selection only after `npm run controller:autopilot` returned a stale all-external STOP while current truth had a selected WorkSourceWA move that was not persisted.
- Files changed: `scripts/controller-autopilot.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Added a generated app-owner contract for the exact next rung, `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`, when backlog items are all passive/external but `CURRENT_STATE.md` records that the selected WorkSourceWA current move is not yet persisted as an artifact/action. The generated contract forbids frontend, golden artifact proof, paid generation, outbound email, Stripe, schema/destructive DB actions, and routes future work to no-paid selected-move-to-latest/history proof.
- Verification: `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts --reporter=verbose` passed (`20/20`), including the selected-move persistence generated-contract regression; `npm run health` passed with `RESULT: 0 FAILING`; `npm run build` passed.
- Unresolved issues: The generated contract must be executed in a later seam; this controller fix does not persist the selected WorkSourceWA artifact/action itself.

## 2026-05-13 - Active handoff line-cap failure moved into preflight
- MODE: CI/source-truth guard only; no product runtime, frontend, paid generation, outbound email, Stripe, schema, or destructive DB behavior changed.
- Files changed: `scripts/preflight-contract.ts`, `scripts/__tests__/preflight-contract.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
...
## 2026-05-13 - Selected WorkSourceWA move persists through no-paid artifact path
- MODE: Generated contract `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`; no frontend, paid generation, golden artifact proof, outbound email, Stripe, schema, or destructive DB action.
- Files changed: `app/api/conviction/generate/route.ts`, `app/api/conviction/latest/__tests__/selected-move-generate.test.ts`, `lib/conviction/artifact-generator.ts`, `lib/conviction/__tests__/artifact-generator.test.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Added a deterministic admin-deadline document renderer for selected WorkSourceWA `deadline_staleness` write-document moves and a `source=winner_truth` generate mode that persists the selected winner as a normal `pending_approval` action without signal extraction, `generateDirective()`, paid/model calls, or outbound email. The persisted payload includes the document artifact and the discrepancy card/quality frame used by latest/history surfaces.
- Verification: `node node_modules/vitest/vitest.mjs run lib/conviction/__tests__/artifact-generator.test.ts lib/conviction/__tests__/artifact-generator-contract.test.ts app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts app/api/conviction/history/__tests__/route.test.ts app/api/conviction/daily-value/__tests__/route.test.ts app/api/conviction/latest/__tests__/selected-move-generate.test.ts --reporter=verbose` passed (`41/41`); `npm run winner:autopsy` still selected the WorkSourceWA Tier 1 `admin_deadline_decision_packet`; `npm run health` passed with `RESULT: 0 FAILING`; `npm run build` passed; `tests/config/__tests__/docs-source-of-truth.test.ts` passed and `ACTIVE_HANDOFF.md` stayed under the 80-line cap.
...
## 2026-05-13 - CI action runtime bumped off Node 20 warnings
- MODE: CI dependency hardening only; product runtime unchanged.
- Files changed: `.github/workflows/ci.yml`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
...
## 2026-05-13 - Controller selects current selected-move persistence from live source truth
- MODE: Controller/source-truth selection only after `npm run controller:autopilot` returned an invalid all-external STOP while current no-paid truth still selected WorkSourceWA.
- Files changed: `scripts/controller-autopilot.ts`, `scripts/__tests__/controller-autopilot.test.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Expanded the generated selected-move persistence selector so current source-truth wording that says selected WorkSourceWA still needs no-paid execution proof emits `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT` instead of falling through to paid/external blockers. This keeps the controller tied to live closeability instead of an obsolete exact phrase.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; read-only Supabase latest owner actions showed newest row `40790ab9-d98e-43cf-8a3c-db78fb654d2d` is skipped `do_nothing` and no selected-move `pending_approval` exists; `npm run winner:autopsy` still selected the WorkSourceWA Tier 1 `admin_deadline_decision_packet`; `npm run build` passed; `node node_modules/vitest/vitest.mjs run scripts/__tests__/controller-autopilot.test.ts --reporter=verbose` passed (`21/21`); `npm run controller:autopilot` returned `GO` with `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`.
- Unresolved issues: Commit/push and hosted CI/deploy proof remain pending for this controller patch. The next generated contract must execute selected-move persistence without paid generation, outbound email, golden artifact proof, Stripe, schema, or destructive DB action.

## 2026-05-13 - Selected WorkSourceWA move persisted as pending approval
- MODE: Generated contract `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`; no paid generation, outbound email, golden artifact proof, Stripe, schema, destructive DB action, or frontend changes.
- Files changed: `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Executed the deployed no-paid selected-winner generate path and recorded closure. Production now has a real owner `pending_approval` `write_document` row for the WorkSourceWA account-activity closeout.
- Verification: Production `POST /api/conviction/generate?source=winner_truth` initially timed out client-side, then read-only DB proof showed it completed and persisted row `8aca653a-f0a1-46e9-9af4-323c5cee539b` with title `WorkSourceWA account activity closeout`, `status=pending_approval`, `action_type=write_document`, and `brief_origin=selected_move_generate`; `npm run health` now reports `Last generation write_document`; `https://www.foldera.ai/api/health` reports SHA `73aed8b0179e60448fc2baf099a093d6d5d2f84c`.
- Unresolved issues: Commit/push this closure receipt, rerun controller from clean `main`, and continue only if it emits a fresh no-paid contract.

## 2026-05-13 - Selected WorkSourceWA move enters latest readback gate
- MODE: Generated contract `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`; latest/history readback only, no paid generation, outbound email, Stripe, schema, destructive DB action, or frontend changes.
- Files changed: `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `lib/conviction/action-read-shapes.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Fixed the readback edge where the persisted selected-move artifact was visible in history but hidden from `/api/conviction/latest` because confidence `45` was below the normal send-confidence ranking threshold. The latest ranking query now includes `brief_origin` and allows `selected_move_generate` rows into the existing strict artifact/discrepancy validation path.
- Verification: `node node_modules/vitest/vitest.mjs run app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts app/api/conviction/history/__tests__/route.test.ts app/api/conviction/daily-value/__tests__/route.test.ts lib/conviction/__tests__/artifact-generator.test.ts lib/conviction/__tests__/artifact-generator-contract.test.ts --reporter=verbose` passed (`41/41`).
- Unresolved issues: Build/lint/preflight, commit/push, production SHA, and authenticated production `/api/conviction/latest` row proof are still pending.

## 2026-05-13 - Selected WorkSourceWA latest readback local proof
- MODE: Generated contract `GENERATED-SELECTED-MOVE-TO-PERSISTED-ARTIFACT`; latest/history readback only, no paid generation, outbound email, Stripe, schema, destructive DB action, or frontend changes.
- Files changed: `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `lib/conviction/action-read-shapes.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
...
## 2026-05-13 - Promise-chain rung 2 repeatable WorkSourceWA winner
- MODE: First broken promise-chain rung only; no UI, landing page, controller, paid generation, outbound email, Stripe, schema, destructive DB action, or artifact polish.
- Files changed: `PROMISE_CHAIN_AUDIT.md`, `lib/briefing/discrepancy-card-frame.ts`, `lib/briefing/__tests__/discrepancy-card-frame.test.ts`, `lib/signals/behavioral-graph.ts`, `lib/signals/__tests__/behavioral-graph.test.ts`, `ACTIVE_HANDOFF.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`.
- What changed: Wrote the promise-chain audit and fixed the first broken rung, `FINDS WHAT MATTERS`. Valid pending discrepancy cards with empty `blocked_by: []` no longer train noisy blocked pattern memory, so the WorkSourceWA selected deadline class does not self-veto after persistence. Small 90-day rolling-window graph aging is no longer a current blocker when no newer signal exists and 14/30-day counts remain stable.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run winner:autopsy` now selects the WorkSourceWA Tier 1 `admin_deadline_decision_packet` with `graph_drift=[]`, `action_needed=[]`, and `future_findings=[]`; focused discrepancy and graph tests passed (`14/14`); selected-move generation/artifact/latest-history/daily-value/execute proof passed (`46/46`); `npm run lint` passed; `npm run build` passed; read-only production DB proof found row `8aca653a-f0a1-46e9-9af4-323c5cee539b` still pending approval with source refs and `paid_flag=false`; Vercel deployment `dpl_GSE4Nuj5P8aoiTiRkUGt6n7QSpKa` became READY and production `/api/health` reported SHA `4b964abdc4acbb20604ef9c359f165db6286019f`.
- Unresolved issues: None for the first broken rung. Artifact/source-trail quality remains the next rung and was intentionally not touched.

## 2026-05-13 - Non-owner beta mock harness map and first path harness
- MODE: Non-owner beta harness only; no UI polish, no landing copy changes, no controller/meta, no Brandon owner data as proof, no fabricated production users, no paid generation, no outbound email, no Stripe, no schema changes.
- Files changed: `NON_OWNER_BETA_HARNESS_MAP.md`, `tests/e2e/non-owner-beta-harness.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
...
## 2026-05-13 - Visual gate status controller
- MODE: Gate controller only; no product UI polish, no paid generation, no outbound email, no Stripe, no schema, no fake users, and no owner-only beta proof.
- Files changed: `scripts/visual-gate-status.ts`, `scripts/__tests__/visual-gate-status.test.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/preflight-contract.test.ts`, `package.json`, `docs/QUALITY_GATES.md`, `tests/e2e/landing-dashboard-visual.spec.ts`, `tests/dashboard/live-artifact-pixel-lock.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added `npm run gate:visual` so Foldera can answer the current visual quality gate with one deterministic command. The controller reports release and quality gate context, requires QG_10 to pass first, checks current-move/source-trail/approval-control/responsive/screenshot proof, and labels proof as mock-only rather than beta proof. Refreshed the existing visual proof tests to assert the current dashboard surface instead of stale wrapper and width assumptions.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run build` passed; `npm run gate:status` still stops at `GATE_9_REAL_NON_OWNER_BETA` with `BLOCKED_EXTERNAL`; `npm run gate:quality` passed with `FIRST_FAILING_QUALITY_GATE: NONE`; `node node_modules/vitest/vitest.mjs run scripts/__tests__/visual-gate-status.test.ts scripts/__tests__/preflight-contract.test.ts --reporter=verbose` passed (`23/23`); `npm run gate:visual` passed with `FIRST_FAILING_VISUAL_GATE: NONE`; `npx playwright test tests/e2e/landing-dashboard-visual.spec.ts --grep "capture dashboard" --reporter=list` passed; `npx playwright test tests/dashboard/live-artifact-pixel-lock.spec.ts --reporter=list` passed (`3/3`).
- Unresolved issues: Release remains externally blocked on one real non-owner tester connecting Google or Microsoft. Visual proof is deterministic mock/browser proof only and does not claim beta readiness.

## 2026-05-13 - Decision trace quality gate controller
- MODE: Gate controller only; no UI polish, no frontend redesign, no paid generation, no outbound email, no Stripe, no schema, no fake users, no beta-readiness claim, and no hidden model reasoning exposed.
- Files changed: `scripts/decision-trace-gate-status.ts`, `scripts/__tests__/decision-trace-gate-status.test.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/preflight-contract.test.ts`, `package.json`, `docs/QUALITY_GATES.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added `QG_13_DECISION_TRACE_QUALITY` and `npm run gate:decision-trace`. The deterministic controller requires user-safe decision traces to show sources read, freshness, candidate list, rejected/blocked candidates, selected winner or no-safe-move, why-this-won, why-now, supporting source trail, proposed finished move, approval/save/skip state, and when Foldera would send nothing. Bad fixtures fail for magic AI explanations, unsupported source trails, hidden rejected candidates, missing why-now, internal scoring/debug terms, confidence percentages, and Brandon/private owner context leaks.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run build` passed; `npm run gate:status` still stops at `GATE_9_REAL_NON_OWNER_BETA` with `BLOCKED_EXTERNAL`; `npm run gate:quality` passed; `npm run gate:visual` passed; `npm run gate:decision-trace` passed with `FIRST_FAILING_DECISION_TRACE_GATE: NONE`, 8 bad trace fixtures rejected, 5 good trace fixtures accepted, and no proof missing; focused gate/preflight regression slice passed (`34/34`); `npm run preflight -- --stage=pre-commit` passed.
- Unresolved issues: Release remains externally blocked on one real non-owner tester connecting Google or Microsoft. Decision-trace proof is deterministic fixture proof only and does not claim beta readiness or live paid generation proof.

## 2026-05-13 - Release truth gate stop on real non-owner beta blocker
- MODE: Release truth/gate status only; no product code, no UI polish, no paid generation, no outbound email, no Stripe, no schema, no fake users, and no owner-only beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Fast-forwarded the workspace to current `origin/main` at `e861d2d3f8a4b5fa92c5d4c591e267bad8f39fce`, waited for production to serve that SHA, and recorded the current gate stack. The first failing release gate remains `GATE_9_REAL_NON_OWNER_BETA`.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` reported `GATE_9_REAL_NON_OWNER_BETA` as `BLOCKED_EXTERNAL`; `npm run gate:quality` passed with `FIRST_FAILING_QUALITY_GATE: NONE`; `npm run gate:visual` passed with `FIRST_FAILING_VISUAL_GATE: NONE`; production `/api/health` reported `revision.git_sha=e861d2d3f8a4b5fa92c5d4c591e267bad8f39fce` and deployment `dpl_aqTH5NrTsNjvtckm2bfMnVwT2pT4`.
- Unresolved issues: The stop reason is external: exactly one real non-owner account must connect Google or Microsoft. No fake rows, `OWNER_USER_ID`, `TEST_USER_ID`, mock harnesses, or owner data may count as beta proof.

## 2026-05-13 - Winner exposure risk framing hydrates real deadline risk
- MODE: Evidence/risk framing only; no proof packet, no new gate, no UI polish, no paid generation, no outbound email, no Stripe, no schema, no beta-readiness claim, and no discrepancy-card quality weakening.
...
## 2026-05-13 - Winner artifact pipe alignment fingerprints selected moves
- MODE: Winner -> artifact pipe alignment only; no new gate, proof packet, UI polish, paid generation, outbound email, Stripe, schema, beta-readiness claim, quality-bar weakening, or fake document content.
...
## 2026-05-13 - Document-collection requirements-needed packet
- MODE: Document artifact requirements only; no new gate, proof packet, UI polish, paid generation, outbound email, Stripe, schema, quality-bar weakening, or fake document content.
- Files changed: `app/api/conviction/generate/route.ts`, `lib/conviction/artifact-generator.ts`, `lib/conviction/__tests__/artifact-generator.test.ts`, `docs/DOCUMENT_ARTIFACT_REQUIREMENTS_REVIEW.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Traced the current document-collection winner to the exact commitment and source email rows. The source emails contain accepted-document requirements, pay, ownership/IP rules, rejection rules, Google-login context, and deadline, but not Brandon-owned candidate document bodies/files, specific document titles/topics, or a captured submission URL. Selected-move generation now hydrates linked source evidence, and document-production winners with missing required content produce a requirements-needed packet instead of a generic admin-deadline memo or fake `.docx` content.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run winner:autopsy` selected the document-collection deadline winner; read-only source query found current commitment `1d0e3ecb-899c-4ec1-96d0-748485678dfe`, source signal `1215e4fd-0d8b-4e47-9a67-0f855d2e8a2f`, and earlier detailed signal `9362af29-0b5d-47f3-a9f2-267bff2cd4d5`; focused artifact requirements test passed (`22/22`); `npm run gate:decision-trace` passed; `npm run build` passed; Vercel deployment `dpl_FL5Xq2BFyTuVEvtXjJ2r1UhBvCen` became `READY`; production `/api/health` reported `revision.git_sha=b3abc3a075ca77edf7fa2dbeceb63bc8073e223a`.
- Unresolved issues: The truthful output is now a requirements-needed packet. Finished `.docx` work remains blocked until owned source documents/content and submission destination exist.

## 2026-05-13 - Document-collection requirements packet persisted and latest-visible
- MODE: Requirements-packet persistence only; no new gate, proof packet, UI polish, paid generation, outbound email, Stripe, schema, quality-bar weakening, or fake document content.
- Files changed: `app/api/conviction/generate/route.ts`, `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `app/api/conviction/latest/__tests__/selected-move-generate.test.ts`, `lib/briefing/generator.ts`, `lib/conviction/__tests__/artifact-generator.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: The selected-move generate route now has enough runtime to finish winner-truth scoring, the persistence validator no longer misclassifies the document-collection requirements packet as a payment digest, and `/api/conviction/latest` displays the current selected-move requirements packet even though it is a blocker packet rather than finished `.docx` work.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run winner:autopsy` selected the document-collection winner; `npm run gate:decision-trace` passed; focused latest/generate/artifact tests passed (`39/39`); `npm run build` passed; authenticated production `POST /api/conviction/generate?source=winner_truth` persisted row `ce3a69f1-80e1-4ad8-8226-5c2f15cb0cba`; read-only DB proof found `REQUIREMENTS-NEEDED PACKET`, known requirements, missing ingredients, and no fake `.docx` content; production `/api/conviction/latest` and `/api/conviction/history?limit=2` show the new requirements packet; production `/api/health` reports SHA `f6b9d3bf772baef90db4c9106591915cedd52e2a`.
- Unresolved issues: Finished `.docx` work remains blocked until Brandon-owned candidate documents/source bodies and the actual submission destination are available. Real beta readiness remains externally blocked on one connected non-owner account.

## 2026-05-14 - Document-collection intake workflow local proof
- MODE: Narrow requirements-input workflow only; no new gate, no proof packet, no UI polish, no paid generation, no outbound email, no Stripe, no schema, no fake `.docx`, and no beta-readiness claim.
- Files changed: `docs/DOCUMENT_COLLECTION_INTAKE_WORKFLOW.md`, `lib/conviction/document-collection-intake.ts`, `lib/conviction/artifact-generator.ts`, `app/api/conviction/actions/[id]/document-collection-intake/route.ts`, `app/dashboard/page.tsx`, `app/dashboard/dashboard-page-model.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, focused tests, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: The document-collection requirements-needed packet now includes the exact intake ask and next action. `/dashboard` shows a scoped text/link intake panel only for that packet, and the new action route stores `submission_url` plus candidate document/source-body notes under `execution_result.document_collection_intake` without changing status or schema.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run winner:autopsy` still selected the document-collection winner; `npm run gate:decision-trace` passed; focused latest/generate/action-detail/intake/dashboard/generator tests passed (`48/48`); focused dashboard Playwright intake proof passed; `npm run build` passed; docs source-of-truth test passed; read-only DB proof confirmed the pre-deploy production packet has no fake `.docx` but still needs a post-deploy no-paid regeneration for the new exact wording.
- Unresolved issues: Commit/push, Vercel production SHA, no-paid selected-winner generation after deploy, and latest/history/detail/DB readback remain pending.

## 2026-05-14 - Document-collection intake workflow persistence and dashboard visibility
- MODE: Requirements-input workflow completion only; no new gate, no proof packet, no UI polish, no paid generation, no outbound email, no Stripe, no schema, no fake `.docx`, and no beta-readiness claim.
- Files changed: `app/dashboard/dashboard-page-model.tsx`, `app/dashboard/__tests__/dashboard-page-model.test.tsx`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Production no-paid selected-winner generation persisted updated row `d17931e4-ab3d-4d75-9775-61b6c58b22b4` with the exact requirements-needed intake ask. The dashboard visibility guard now allows this source-backed requirements-needed packet as a blocked/intake state even though it is intentionally not a finished artifact-quality packet.
- Verification: Production `/api/health` served commit `639beb97095f4b48fba6762513c350e4df94468d`; Vercel deployment `dpl_9h55iTkP5Jjf4YuKKx8uTPHcdnUU` was `READY`; authenticated production `POST /api/conviction/generate?source=winner_truth` persisted row `d17931e4-ab3d-4d75-9775-61b6c58b22b4`; `/api/conviction/latest` returned that row with the intake next action and no stale WorkSourceWA; `/api/conviction/history?limit=2` returned that row first; `/api/conviction/actions/d17931e4-ab3d-4d75-9775-61b6c58b22b4` and read-only DB proof showed known requirements, exact missing inputs, and no fake document body. The dashboard summary-only visibility test failed first, then the focused intake slice passed (`49/49`); `npm run build` passed; focused dashboard Playwright intake proof passed; `npm run health`, `npm run winner:autopsy`, and `npm run gate:decision-trace` passed.
- Unresolved issues: Dashboard visibility patch still needs commit/push/deploy proof. Finished `.docx` submission work remains blocked only by Brandon-owned source files/bodies, document topics/titles, and submission URL/upload destination.

## 2026-05-14 - GitHub CI final-gate enforcement
- MODE: CI final-gate enforcement only; no product work, no UI polish, no paid generation, no outbound email, no Stripe, no schema, and no feature continuation.
- Files changed: `CODEX_START.md`, `GPT.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `app/dashboard/page.tsx`, `components/dashboard/DocumentCollectionIntakePanel.tsx`.
- What changed: Added the rule that Foldera cannot be called done unless the exact current `origin/main` commit has green GitHub Actions and green Vercel proof. Root-caused the missed final gate to Vercel-only success being treated as enough while GitHub Actions was red. Mechanically extracted the document-collection intake panel and repeated workspace panel rendering so `app/dashboard/page.tsx` stays under the existing large-file split guard without weakening CI.
- Verification: GitHub Actions API found `CI` run `25863078234` for commit `6f0275ead9bc55e89075ab90467d8f3149c8d4d6` failed while Vercel deployment `dpl_GoEf99ATTkPbEVd8BEi8dyYoqePe` was green. Failing job was `unit`, failing step was `Vitest`, failing test was `tests/config/__tests__/large-file-splits.test.ts`, and the exact error was `expected 1061 to be less than or equal to 1000`. Focused failing unit test now passes locally after the mechanical split.
- Unresolved issues: Push the enforcement/fix commit, then verify GitHub CI and Vercel for the new current `main` commit before any done claim.

## 2026-05-14 - GitHub CI final-gate enforcement pushed and verified
- MODE: CI final-gate receipt only; no product work, no UI polish, no paid generation, no outbound email, no Stripe, no schema, and no feature continuation.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Recorded the post-push proof that the CI final-gate enforcement commit reached both GitHub Actions green and Vercel green for the same exact main commit.
- Verification: `origin/main` pointed at `efc214ba939f534e46acb2a6a5d282fb0d552956`; GitHub Actions for that SHA completed successfully for full `CI` #1020/run `25864465943`, fast `CI` #279/run `25864465931`, `Health Gate` #595/run `25864466031`, `semgrep` #1521/run `25864466010`, `Production E2E` #1175/run `25864610323`, and `Deploy to Vercel` #946/run `25864953249`; Vercel deployment `dpl_EWjQL11WWY6QDQAtfUP18ym75Gt6` was `READY`; production `/api/health` reported `revision.git_sha=efc214ba939f534e46acb2a6a5d282fb0d552956`.
- Unresolved issues: Any later commit must be re-checked against GitHub Actions and Vercel for that new exact `origin/main` SHA before a done claim.

## 2026-05-14 - Artifact readiness contract local implementation
- MODE: Artifact readiness contract only; no UI polish, no broad feature, no paid generation, no outbound email, no Stripe, no schema, no weakened quality bars, and no fake content.
- Files changed: `lib/conviction/artifact-readiness.ts`, `app/api/conviction/generate/route.ts`, `app/api/conviction/latest/route.ts`, `app/api/conviction/history/route.ts`, `lib/conviction/action-read-shapes.ts`, `app/dashboard/dashboard-page-model.tsx`, focused tests, `docs/ARTIFACT_READINESS_CONTRACT.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
...
## 2026-05-14 - Production Readback Proof for artifact readiness
- MODE: Production readback proof only; no broad product work, UI polish, paid generation, outbound email, Stripe, schema, or fake content.
- Files changed: `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `app/api/conviction/latest/__tests__/selected-move-generate.test.ts`, `app/dashboard/page.tsx`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Fixed production readback for the current selected-move requirements packet. `/api/conviction/latest` now avoids a full winner-truth recalculation when the persisted selected-move packet was generated after the latest source sync and no newer signals exist; stale snapshots still fall back to winner-truth and fail closed on timeout. The dashboard now labels document-collection requirements packets as `Inputs needed` / `Requirements packet` instead of overclaiming finished work.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run winner:autopsy` selected `Commitment due in 0d: Submit high-quality .docx documents for document collection`; `npm run gate:decision-trace` passed; focused artifact-readiness/latest/detail/history/dashboard tests passed (`37/37`); `npm run build` passed; Vercel deployment `dpl_CHXRSmNWKERdPrc4HdNbVAVhk9wB` was `READY` for runtime SHA `3933f24e31512bd34e329b26f69eece0db4759fb`; production `/api/health` served that SHA; authenticated production latest/detail/history/dashboard all returned row `d17931e4-ab3d-4d75-9775-61b6c58b22b4` with `REQUIREMENTS_NEEDED`, no stale WorkSourceWA, no fake `.docx`, and missing inputs `owned .docx/source files`, `document topics/titles`, and `submission URL`.
- Unresolved issues: This receipt-only update still needs its own push and exact-SHA GitHub/Vercel/production-health verification. Finished `.docx` work remains blocked by Brandon-owned source files/bodies, document topics/titles, and submission URL/upload destination.

## 2026-05-14 - Adversarial Phase 0 operating truth correction
- MODE: Operating truth only after adversarial audit Phase 0 found stale receipts; no product code, UI polish, paid generation, outbound email, Stripe, schema, fake users, fake source rows, or beta-readiness claim.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `docs/RELEASE_GATES.md`.
- What changed: Recorded the actual live truth for receipt/docs-only commit `da1678029876992ce4c38ebc8304815625675b05` and stopped the requested proof audit before product/readback attack. Cross-linked `docs/REAL_NON_OWNER_BETA_PROOF_CHECKLIST.md` from the GATE_9 release-gate section so the real non-owner proof checklist is discoverable from operating docs. The ignored local preflight contract was adjusted to reflect this user-pinned operating-truth scope.
- Verification: Phase 0 live truth showed local HEAD and `origin/main` at `da1678029876992ce4c38ebc8304815625675b05`; GitHub Actions for that SHA completed successfully (`CI #286`, `Health Gate #605`, `Deploy to Vercel #957`, `Production E2E #1185`); Vercel deployment `dpl_cYQfUXhZAQHtC1t4DUi8bPm8vkcA` was `READY`; production `/api/health` served the same SHA; `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed GATE_0 and stopped at `GATE_9_REAL_NON_OWNER_BETA`; docs source-of-truth test passed (`3/3`); `npm run build` passed.
- Unresolved issues: This correction itself is receipt-only and still needs commit/push plus external GitHub/Vercel/production-health proof for the final receipt commit. The adversarial product-readback audit did not proceed because Phase 0 found stale operating truth.

## 2026-05-14 - Receipt-only self-SHA wording correction
- MODE: Operating truth wording only; no product code, UI polish, paid generation, outbound email, Stripe, schema, fake users, fake source rows, or beta-readiness claim.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Reworded `ACTIVE_HANDOFF.md` so it does not become stale by embedding the SHA of its own receipt-only edit. The handoff now records the last externally verified receipt/docs SHA before the edit and requires the final self commit to be verified externally from `origin/main`, GitHub Actions, Vercel, and production `/api/health`.
- Verification: Prior receipt correction commit `c11bd5e4fd565d3b01813f0865630710aed18060` reached GitHub Actions success (`CI #287`, `Health Gate #606`, `Deploy to Vercel #958`, `Production E2E #1186`), Vercel deployment `dpl_FNfoD627pV5mbjvjfs6jbrkiad35` was `READY`, and production `/api/health` served the same SHA.
- Unresolved issues: This final wording commit must be pushed and verified externally. The adversarial product-readback audit remains stopped at Phase 0 because operating truth was stale.

## 2026-05-14 - Adversarial readback attack stopped on CI drift
- MODE: Adversarial readback attack; no product code, UI polish, paid generation, outbound email, Stripe, schema, fake users, fake source rows, fake documents, or beta-readiness claim.
- Files changed: `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added focused fallback tests proving `/api/conviction/latest` fails closed when a `selected_move_generate` artifact has no current winner match or the winner-truth check errors/times out. Updated operating truth after final proof found CI red before the external beta blocker.
- Verification: Phase 0 passed for `de2722eaeda0af3ba4585a57eb158ff12894bfff`; production latest/detail/history/dashboard readback agreed on row `d17931e4-ab3d-4d75-9775-61b6c58b22b4` as `REQUIREMENTS_NEEDED`; focused readiness/latest/detail/history/dashboard tests passed (`39/39`); focused dashboard render tests passed (`2/2`); `npm run health`, `npm run gate:status`, `npm run gate:quality`, `npm run gate:visual`, `npm run winner:autopsy`, `npm run gate:decision-trace`, and `npm run build` passed locally; read-only DB proof found zero connected non-owner/non-test token rows; pushed commit `2795975bfa5fc2142c4c9867800407742abcaa3c` reached Vercel READY (`dpl_GBbsh2PRiWvhxr8wZxghyzJjCtm3`) and production `/api/health` served that SHA.
- Unresolved issues: GitHub `CI #1025` failed in `unit` because `tests/config/__tests__/large-file-splits.test.ts` reports `app/dashboard/page.tsx` at `1008` lines, above the `1000` threshold. Stop condition: `NOT DONE - internal gate drift before GATE_9`.

## 2026-05-14 - Dashboard line split CI drift repair
- MODE: Dashboard line split only; no product work, UI polish, redesign, paid generation, outbound email, Stripe, schema, artifact generation/readback logic, beta proof, or broad refactor.
- Files changed: `app/dashboard/page.tsx`, `components/dashboard/DashboardArtifactBody.tsx`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Moved the already-isolated dashboard artifact body rendering into `components/dashboard/DashboardArtifactBody.tsx`, reducing `app/dashboard/page.tsx` from `1007` local lines to `971` local lines while preserving the same markup, data-testid values, markdown rendering, and Pro blur behavior.
- Verification: `npx vitest run tests/config/__tests__/large-file-splits.test.ts --reporter=verbose` passed; focused dashboard Playwright readback/render tests passed (`summary-only latest payload` and `document collection requirements packet`); `npx vitest run app/dashboard/__tests__/dashboard-page-model.test.tsx --reporter=verbose` passed; `npm run health` passed with `RESULT: 0 FAILING`; `npm run build` passed; GitHub Actions for `80f6be7163a50f6e6ddb3557c292bd984e9d682d` succeeded (`CI #1026`, `Health Gate #610`, `semgrep #1527`, `Production E2E #1190`, `Deploy to Vercel #962`); Vercel deployment `dpl_7fSh74PXguRbqopLzH6yt77L8Rnd` was `READY`; production `/api/health` served `80f6be7163a50f6e6ddb3557c292bd984e9d682d`.
- Unresolved issues: Real non-owner beta proof remains external. Finished `.docx` work remains blocked by Brandon-owned source files/bodies, document topics/titles, and submission URL/upload destination.

## 2026-05-14 - Receipt truth correction for dashboard split proof
- MODE: Receipt truth correction only; no product work, readback attack, quality attack, UI polish, paid generation, outbound email, Stripe, schema, fake users, fake source rows, fake documents, fake deadlines, fake emails, or beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Corrected operating receipts so the last externally verified receipt/proof SHA is `63a4bd7c6f94544af944a5ab5ddaaefc2f72d37a`, while preserving the receipt-only rule that this edit must not embed the SHA of its own containing commit.
- Verification: Before this edit, local HEAD and `origin/main` were `63a4bd7c6f94544af944a5ab5ddaaefc2f72d37a`; GitHub Actions for that SHA succeeded (`CI #290`, `Health Gate #611`, `Production E2E #1191`, `Deploy to Vercel #963`); Vercel deployment `dpl_Dp4Lkm4fSX3FZoNcnER9aPiTKKpA` was `READY`; production `/api/health` served `63a4bd7c6f94544af944a5ab5ddaaefc2f72d37a`.
- Unresolved issues: This correction is receipt-only; the commit containing this entry must be verified externally after push without editing the handoff again to embed its own future SHA.

## 2026-05-14 - Adversarial readback stale-selected-move repair
- MODE: Adversarial proof ledger; no UI polish, broad feature, paid generation, outbound email, Stripe, schema, fake users, fake source rows, fake documents, fake deadlines, fake emails, or beta proof.
- Files changed: `app/api/conviction/latest/route.ts`, `app/api/conviction/latest/__tests__/free-artifact-allowance.test.ts`, `scripts/health.ts`, `scripts/health-checks.ts`, `scripts/__tests__/health-checks.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added a focused failing test for time-driven winner drift after a selected-move artifact was persisted, then made `/api/conviction/latest` re-check selected-move rows generated before the current UTC day so stale packets fail closed as `NO_SAFE_ARTIFACT`. Added a focused health test and guard so source-backed requirements-needed blocker packets are not counted as stale actionable pending approvals.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; focused artifact/readback tests passed (`62/62`); focused health/latest tests passed (`24/24`); focused dashboard Playwright readback passed (`2/2`); `npm run gate:quality`, `npm run gate:visual`, and `npm run gate:decision-trace` passed deterministic gates; `npm run winner:autopsy` selected the Project Mosaic commitment winner; `npm run build` and `npm run lint` passed; read-only DB proof showed the stale >20h pending row was a requirements-needed selected-move blocker packet; production `/api/health` served `55342e1f808de111de9544702efa6f7bbf57132c`; authenticated production latest returned `NO_SAFE_ARTIFACT` with `stale_selected_move_artifact`, no row id, and no document-collection packet; production dashboard no longer rendered the stale document-collection title.
- Unresolved issues: This receipt update still needs commit/push plus external GitHub/Vercel/production-health proof for the receipt-only commit. Real non-owner beta proof remains external and must not be faked.

## 2026-05-15 - Frontend product truth no-safe fallback
- MODE: Frontend product truth audit; no backend artifact rewrite, no UI redesign, no paid generation, no outbound email, no Stripe, no schema, no fake production data, and no fake beta proof.
- Files changed: `app/dashboard/page.tsx`, `app/dashboard/dashboard-page-model.tsx`, `app/globals.css`, `components/foldera/RightPanel.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Fixed the screenshot-proven dashboard failure where a latest `NO_SAFE_ARTIFACT` state could stay on the loading placeholder if `/api/conviction/daily-value` stalled. The dashboard now builds a human-readable held-back slate from latest readiness truth and times out the secondary daily-value fallback. Also tightened the existing dashboard card/rail layout so finished, requirements-needed, and no-safe visual proofs do not show the main state as a broken placeholder or horizontally overflowing surface.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed through GATE_8 and stopped at external `GATE_9_REAL_NON_OWNER_BETA`; `npm run gate:quality`, `npm run gate:visual`, and `npm run gate:decision-trace` passed; `npm run winner:autopsy` selected Project Mosaic; `npm run build` passed; focused dashboard/authenticated Playwright passed (`59 passed`); deterministic screenshot proof for finished, requirements-needed, and no-safe states was captured under `%TEMP%\\foldera-frontend-audit-final-20260515-061903`.
- Unresolved issues: Final commit still needs push, GitHub CI, Vercel READY, production `/api/health`, and production dashboard screenshot verification. Real non-owner beta remains external after frontend product truth is visually proven.

## 2026-05-15 - Dashboard money-shot redline containment
- MODE: Frontend money-shot redline; no backend logic, artifact generation, winner/readback logic, schema, Stripe, paid generation, outbound email, fake production data, fake beta proof, new features, or broad redesign.
- Files changed: `app/dashboard/page.tsx`, `app/dashboard/dashboard-page-model.tsx`, `app/globals.css`, `components/foldera/DailyBriefCard.tsx`, `components/foldera/DailyUtilitySlateCard.tsx`, `components/foldera/RightPanel.tsx`, `tests/e2e/dashboard-navigation.spec.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Moved the desktop money-shot footer out of the scrollable artifact body, tightened the right source-trail/upload rail, surfaced source trail earlier on compact cards, shortened footer trust copy, compacted the no-safe slate, and humanized the `NO REAL PRESSURE` receipt reason into customer-facing held-back copy.
- Verification: Current local HEAD and `origin/main` started at `e44af5abf103831499f00d1cfb7dc30379d5c90e`; Vercel production was READY and production `/api/health` served the same SHA before edits. `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:visual`, `npm run gate:status`, `npm run gate:quality`, `npm run winner:autopsy`, `npm run gate:decision-trace`, and `npm run build` passed; focused dashboard/authenticated Playwright passed (`60 passed`). Before screenshots were captured under `%TEMP%\\foldera-dashboard-redline-before-2026-05-15T13-55-17-593Z` and `%TEMP%\\foldera-dashboard-prod-before-2026-05-15T13-56-15-015Z`; after screenshots were captured under `%TEMP%\\foldera-dashboard-redline-after-final2-2026-05-15T14-20-47-619Z` with final requirements verification under `%TEMP%\\foldera-dashboard-redline-after-final3-2026-05-15T14-26-38-568Z`.
- Unresolved issues: This receipt must be committed and pushed, then the final `origin/main` commit must be verified through GitHub CI, Vercel READY, production `/api/health`, and production dashboard screenshots. Real non-owner beta remains the external release blocker after this frontend proof is live.

## 2026-05-15 - Dashboard money-shot CI assertion repair
- MODE: Focused frontend verification repair only; no backend logic, artifact generation, winner/readback logic, schema, Stripe, paid generation, outbound email, fake production data, fake beta proof, new features, or broad redesign.
- Files changed: `tests/config/__tests__/dashboard-inbox-model.test.ts`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Aligned the dashboard model unit test with the customer-facing no-safe status label `Held back safely`, matching the money-shot frontend copy that replaced the old stale-source fallback label.
- Verification: Reproduced the CI unit failure locally (`dashboard-inbox-model.test.ts` expected `Needs fresher source`); focused model test then passed (`7/7`); full `npm run test:ci:unit` passed. The final commit still needs push plus external GitHub CI, Vercel READY, production `/api/health`, and production dashboard screenshot verification.
- Unresolved issues: Real non-owner beta remains the external release blocker after final frontend proof is live.

## 2026-05-15 - Dashboard frontend regression lock
- MODE: Frontend regression-prevention only; no redesign, backend artifact logic, schema, Stripe, paid generation, outbound email, fake production data, fake beta proof, or broad polish.
- Files changed: `tests/e2e/dashboard-money-shot-regression.spec.ts`, `tests/e2e/dashboard-money-shot-regression.spec.ts-snapshots/*`, `tests/config/__tests__/frontend-product-truth-gate.test.ts`, `scripts/frontend-product-truth-gate.ts`, `docs/FRONTEND_PRODUCT_TRUTH_GATE.md`, `CODEX_START.md`, `GPT.md`, `docs/QUALITY_GATES.md`, `package.json`, dashboard copy/control files, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added `npm run gate:frontend` with committed Playwright screenshot baselines for finished, requirements-needed, and no-safe states on desktop/mobile; added interaction matrix coverage for visible dashboard controls; added banned-copy source/rendered checks; disabled the notification bell with a reason; made the visible upgrade prompt real navigation; removed banned internal wording from dashboard UI copy.
- Verification: Screenshot matrix result: PASS for finished desktop/mobile, requirements-needed desktop/mobile, and no-safe desktop/mobile. Interaction matrix result: PASS for navigation, notification/status controls, Learn more, Upgrade to Pro, profile dropdown, Copy read, Copy draft, Open requirements packet, Skip, Save, Save packet, Approve, Sign out, source trail cards, upload card, account controls, and icon-only labels. Banned-copy audit result: PASS. `npx playwright test tests/e2e/dashboard-money-shot-regression.spec.ts --reporter=list` passed (`12/12`); `npx vitest run tests/config/__tests__/frontend-product-truth-gate.test.ts --reporter=verbose` passed (`2/2`); `npm run build` passed.
- Unresolved issues: Full required proof, push, GitHub CI, Vercel READY, production `/api/health`, and production dashboard screenshot verification still need to run for the final commit. Real non-owner beta remains external after frontend regression proof is live.

## 2026-05-15 - Frontend constitution hardening
- MODE: Frontend constitution and gate hardening only; no redesign, backend artifact logic, schema, Stripe, paid generation, outbound email, fake production data, fake users, fake beta proof, or public demo using Brandon private data.
- Files changed: `docs/FRONTEND_PRODUCT_TRUTH_GATE.md`, `scripts/frontend-product-truth-gate.ts`, `tests/config/__tests__/frontend-product-truth-gate.test.ts`, `CODEX_START.md`, `GPT.md`, `docs/QUALITY_GATES.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Strengthened the frontend constitution so dashboard/frontend work is not done without production current desktop/mobile screenshots, fixture screenshots for finished/requirements/no-safe desktop/mobile, interaction matrix proof, banned-copy proof, layout contract proof, and a rule that API-only or backend-only proof is not a frontend pass. The gate now checks the handoff and latest receipt for `gate:frontend`, screenshot matrix, interaction matrix, banned-copy audit, layout contract, and production current screenshots before allowing a frontend done claim.
- Verification so far: Phase 0 live truth passed for `origin/main` SHA `f7a9ab133146cb16f7379230226cf98976644988`; GitHub Actions were green, Vercel deployment `dpl_CbLga7N5NUk21dFHB7qvTZKVwvut` was READY, and production `/api/health` served the same SHA. Before screenshot matrix result: PASS. Production current screenshots were captured at `C:\Users\b-kap\AppData\Local\Temp\foldera-frontend-constitution-before-prod-1778860053375`; fixture baselines remain committed under `tests/e2e/dashboard-money-shot-regression.spec.ts-snapshots`. Interaction matrix result: PASS. Banned-copy audit result: PASS. Layout contract result: PASS. Focused frontend gate test was written red first on the missing production-current screenshot contract.
- Unresolved issues: Final proof, commit/push, GitHub CI, Vercel READY, production `/api/health`, and final production screenshot verification remain before closing this constitution slice.

## 2026-05-15 - Real-user dashboard surface repair
- MODE: Real-user dashboard surface repair; no backend artifact selection, artifact generation, schema, Stripe, paid generation, outbound email, fake production data, GATE_9, broad redesign, or public demo using Brandon private data.
- Files changed: `app/dashboard/page.tsx`, `components/dashboard/DashboardContextRail.tsx`, `components/dashboard/DashboardDesktopStage.tsx`, `components/dashboard/DashboardMobileLayout.tsx`, `components/dashboard/DashboardWorkspacePanels.tsx`, `components/foldera/RightPanel.tsx`, `docs/FRONTEND_PRODUCT_TRUTH_GATE.md`, `scripts/frontend-product-truth-gate.ts`, focused frontend tests, committed dashboard screenshots, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Removed the shell-like right-rail filler, converted the notification bell into a non-button disabled status, changed the upload card to an honest disabled "Uploads coming later" state, cleaned Recent Work rows into title/status/date/type/safe outcome receipts, added useful source/account state, capped wide desktop density, and hardened `gate:frontend` against filler copy, raw generated history text, fake active upload/notification controls, weak source/account panels, missing common viewport checks, and slow first dashboard content.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:visual` passed; `npm run gate:frontend` passed with 27 Playwright checks plus frontend gate tests and script proof; performance timing proof reported `first_non_loading_ms=248`, `main_content_ms=298`, and `current_action_ms=309`; focused dashboard/authenticated Playwright passed (`60/60`); `npm run build` passed; `npm run lint` passed. Screenshot matrix result: PASS for finished, requirements-needed, no-safe, and Today/Recent Work/Sources/Account desktop/mobile committed baselines.
- Unresolved issues: This repair receipt still needs commit/push plus GitHub CI, Vercel READY, and production `/api/health` proof for the final exact `origin/main` SHA. Real non-owner beta remains external only after this frontend proof is live.

## 2026-05-15 - GATE_9 real non-owner beta proof mode
- MODE: Verification-only GATE_9 proof; no product buildout, UI polish, backend artifact logic, schema, Stripe, paid generation, outbound email, fake users, fake rows, fake sources, fake artifacts, fake documents, fake deadlines, fake emails, or fake beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Recorded fresh live truth and the real non-owner DB exclusion proof for `GATE_9_REAL_NON_OWNER_BETA`.
- Verification: Local HEAD and `origin/main` were aligned at `18371dbf4a3e48e0998c92e4e22204e0b804c3c4`; GitHub Actions for that SHA were green (`CI` runs `25931486377` and `25931486534`, `Health Gate` `25931486401`, `semgrep` `25931486391`, `Production E2E` `25931613176`, `Deploy to Vercel` `25931494966` and `25931919864`); Vercel deployment `dpl_54fgcku2vTXqGELLWy9aarA73voU` was `READY`; production `/api/health` returned `revision.git_sha=18371dbf4a3e48e0998c92e4e22204e0b804c3c4`; `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:quality`, `npm run gate:visual`, `npm run gate:frontend`, `npm run winner:autopsy`, `npm run gate:decision-trace`, `npm run build`, and `npm run lint` passed. Initial `npm run gate:status` failed only because `ACTIVE_HANDOFF.md` was stale for the current production SHA; after this receipt update, `npm run gate:status` passed GATE_0 and stopped at external `GATE_9_REAL_NON_OWNER_BETA`. `npm run gate:frontend` was rerun after the receipt edit and passed again.
- Supabase proof: Read-only query against project `neydszeamsflpghtrhue` excluded `OWNER_USER_ID=e40b7cd8-4925-42f7-bc99-5022969f1d22` and `TEST_USER_ID=22222222-2222-2222-2222-222222222222`; qualifying non-owner Google/Microsoft token rows in any state were `0`, connected token rows were `0`, and refresh-capable rows were `0`. Existing connected Google/Microsoft token rows were owner-only; the reserved test row had no connected access row.
- Unresolved issues: `NOT DONE - GATE_9 still externally blocked.` One real non-owner user must sign in and connect Google or Microsoft through production OAuth before source status, dashboard state, source trail, save/skip/approve, and history can be honestly verified for beta proof.

## 2026-05-15 - GATE_9 receipt cockpit marker correction
- MODE: Receipt cockpit marker only; no product code, UI polish, backend artifact logic, schema, Stripe, paid generation, outbound email, fake users, fake rows, fake sources, fake artifacts, fake documents, fake deadlines, fake emails, or fake beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Added the required `Release gate status: BLOCKED_EXTERNAL` marker so `npm run gate:status` treats the GATE_9 receipt as current release truth instead of reopening the receipt-only self-SHA loop.
- Verification: Prior receipt commit `1b5352c91b0d8ed35a799e4f14fa4325cba70332` reached GitHub Actions green (`CI`, `Health Gate`, `Deploy to Vercel`, and `Production E2E`), Vercel deployment `dpl_78mpYfb8FrghGbTDPZNDEciek26r` was `READY`, and production `/api/health` served `revision.git_sha=1b5352c91b0d8ed35a799e4f14fa4325cba70332`.
- Unresolved issues: `NOT DONE - GATE_9 still externally blocked.` The final marker correction commit must be pushed and verified externally without embedding its own containing SHA.

## 2026-05-15 - Outcome Autopsy CWU gold-standard playbook
- MODE: Outcome Autopsy / Playbooks seam only; no ML prediction, fake probability scores, generic dashboard cards, unrelated pipeline behavior, paid generation, outbound email, Stripe, schema, fake rows, or fake beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `app/api/outcome-autopsy/latest/route.ts`, `app/dashboard/playbooks/page.tsx`, `lib/outcome-autopsy/outcome-autopsy.ts`, `lib/outcome-autopsy/__tests__/outcome-autopsy.test.ts`, `scripts/outcome-autopsy.ts`, `tests/e2e/outcome-autopsy.spec.ts`, `package.json`.
- What changed: Added a deterministic Outcome Autopsy artifact layer backed by stored TKG rows plus a user-provided CWU gold-standard seed. `/dashboard/playbooks` now renders the CWU Access Specialist offer as a reusable playbook with final outcome, timeline, strongest signals, high-signal artifacts, risks, decisive actions, evidence-vs-inference, repeat/avoid guidance, and future-role prioritization. Third-party student documentation is represented only as redacted/synthetic reasoning structure.
- Verification: Focused autopsy tests passed (`3/3`); real CWU stored-row proof passed and excluded polluted unrelated events (`onboarding@resend`, `Nothing cleared`, ESB, DSHS/OCM, raw sensitive medical terms, and probability language); `/dashboard/playbooks` Playwright proof passed on isolated port `3011`; `npm run lint` passed; clean `npm run build` passed after clearing generated `.next` state.
- Unresolved issues: Commit/push and external deploy/production SHA verification remain. Real non-owner beta proof remains externally blocked at `GATE_9_REAL_NON_OWNER_BETA`.

## 2026-05-15 - Owner-controlled Microsoft canary guard
- MODE: Owner-controlled Microsoft canary guard only; no schema, paid generation, outbound email, Stripe, fake users, fake token rows, fake source rows, fake artifacts, or real beta proof claim.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `docs/OWNER_CANARY_TEST_RUNBOOK.md`, `docs/RELEASE_GATES.md`, `docs/REAL_NON_OWNER_BETA_PROOF_CHECKLIST.md`, `lib/config/constants.ts`, `lib/cron/acceptance-gate.ts`, `lib/cron/__tests__/acceptance-gate.test.ts`, `lib/ops/beta-readiness.ts`, `scripts/release-gate-status.ts`, `scripts/__tests__/release-gate-status.test.ts`, `scripts/preflight-contract.ts`, `scripts/__tests__/preflight-contract.test.ts`.
- What changed: Added `OWNER_CANARY_USER_IDS` as the reserved identity list for Brandon-controlled Outlook/Hotmail connector canaries. `NON_OWNER_DEPTH`, release-gate status proof, and owner beta-readiness now exclude owner, synthetic test, and owner-canary users from real non-owner proof; the canary runbook records the exact safe Microsoft test path.
- Verification: Focused acceptance/release-gate tests passed (`11/11`); preflight contract regression passed; `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed through local gates and stayed externally blocked on a separate real non-owner tester while naming `OWNER_CANARY_USER_IDS` as forbidden beta proof; `npm run gate:frontend` passed (`27` Playwright checks plus gate script). Frontend receipt retained the required screenshot matrix, interaction matrix, banned-copy audit, layout contract, and production current screenshots markers from the deployed frontend proof; no new production screenshots are claimed for this connector-guard slice. `npm run build` passed; `npm run lint` passed.
- Unresolved issues: Canary proof is connector proof only. Real non-owner beta still requires a separate non-Brandon account not listed in `OWNER_USER_ID`, `TEST_USER_ID`, or `OWNER_CANARY_USER_IDS`.

## 2026-05-15 - Outcome Learning Engine CWU evidence packet
- MODE: Outcome Learning Engine seam only; no fake predictive analytics, no generic recommendation cards, no causality overclaim, no Stripe, no auth changes, no paid generation, no outbound email, no schema rewrite, no unrelated dashboard polish, and no fake beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `app/api/outcome-autopsy/latest/route.ts`, `app/dashboard/playbooks/page.tsx`, `lib/outcome-learning/outcome-learning-engine.ts`, `lib/outcome-learning/__tests__/outcome-learning-engine.test.ts`, `scripts/outcome-learning.ts`, `tests/e2e/outcome-autopsy.spec.ts`, `package.json`.
- What changed: Added the deterministic Outcome Learning layer on top of the existing CWU Outcome Autopsy. The API now returns `artifact` plus `learning`; Playbooks shows `What Foldera learned`, raw Evidence Packet, interpreted learning signals, recommendation feedback ledger, and pattern memory. Evidence packet separates raw artifact summaries from scored signals; redacted/synthetic student case material is marked `third_party_sensitive` and `redacted`. Pattern memory uses existing `tkg_pattern_metrics` rows with `outcome_learning:*` hashes rather than a disconnected prediction system.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed through GATE_8 and stopped at external `GATE_9_REAL_NON_OWNER_BETA`; `npm run gate:quality`, `npm run gate:visual`, and initial `npm run gate:frontend` passed. Focused learning/autopsy/large-file tests passed (`5/5`). `npm run outcome:learning -- "CWU Access Specialist"` rendered the real stored-row CWU learning snapshot. `npm run outcome:learning -- --persist-patterns "CWU Access Specialist"` inserted the deterministic pattern memory rows, and read-only Supabase readback confirmed 9 `outcome_learning:*` rows in `tkg_pattern_metrics`. `/dashboard/playbooks` Playwright proof passed on isolated port `3012`; adjacent dashboard/auth routes passed (`60/60`) on isolated port `3014`; `npm run lint` and `npm run build` passed.
- Unresolved issues: This local product slice still needs commit/push plus external GitHub CI, Vercel READY, and production `/api/health` proof for the final exact `origin/main` SHA. Real non-owner beta remains externally blocked.

## 2026-05-15 - Production risk snapshot and Supabase downgrade checklist
- MODE: Production risk snapshot only; no product buildout, UI polish, schema, paid generation, outbound email, or Stripe action.
- Files changed: `docs/PROD_RISK_SNAPSHOT.md`, `docs/SUPABASE_DOWNGRADE_CHECKLIST.md`, `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`.
- What changed: Recorded `origin/main`, production `/api/health`, Vercel READY deployment, Supabase org/project plan/status, advisors, DB/storage size, top tables, token rows, recent churn, ACTIVE_HANDOFF currency, exact downgrade stop conditions, tester blockers, and things not to touch.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed through `GATE_8` and stopped at external `GATE_9_REAL_NON_OWNER_BETA`; `npm run build` passed; `npm run lint` passed. `npm run gate:frontend` failed locally after build because `next start` returned HTTP 500 for local `/` and `/dashboard`, while production `/` returned `200` and unauthenticated `/dashboard` returned `307`. Vercel production deployment `dpl_QyQS8cyfsCjagxSHdKDWERefxVCA` was `READY` and production `/api/health` served `revision.git_sha=58f78bd374e1fb493ef08e4ab621ed4f7c66cc87`.
- Unresolved issues: Supabase downgrade is conditionally allowed only as a cost-control pause. Do not downgrade for a live tester window; stop if a real tester is scheduled/connected, non-owner token rows appear, 7-day logs/backups/support are needed, egress/billing is over Free allowance, DB size approaches `400 MB`, or production debugging is active. Real tester remains blocked on one real non-owner OAuth connection and clean `gate:frontend` proof.

## 2026-05-15 - Auth account switching and Google OAuth redirects
- MODE: Auth account-switching + Google OAuth proof; no feature buildout, UI polish, artifact logic, schema, paid generation, outbound email, Stripe, fake users, fake rows, fake tokens, or fake beta proof.
- Files changed: `ACTIVE_HANDOFF.md`, `SESSION_HISTORY.md`, `.env.example`, `.env.local.example`, `docs/GOOGLE_OAUTH_REDIRECTS.md`, `lib/auth/oauth-account-choice.ts`, `lib/auth/auth-options.ts`, `lib/auth/__tests__/auth-options.test.ts`, `app/login/login-inner.tsx`, `app/start/page.tsx`, Google/Microsoft connector routes/tests, `tests/config/__tests__/google-oauth-redirects.test.ts`, `tests/e2e/dashboard-navigation.spec.ts`, and `tests/e2e/public-routes.spec.ts`.
- What changed: Added account-choice OAuth prompts to Google sign-in (`consent select_account`), Microsoft sign-in (`select_account`), standalone Google connector, and standalone Microsoft connector. Login/start now make the different-account path explicit without changing dashboard artifact behavior. Dashboard Playwright sign-out proof now clears the mocked owner session and verifies `/dashboard` returns to login instead of Brandon's dashboard. Google redirect docs and env examples now list both NextAuth and standalone connector callback paths for `www.foldera.ai`, `foldera.ai`, and localhost. Connector path proof remains separate from real `GATE_9_REAL_NON_OWNER_BETA`.
- Verification: Vercel production deployment `dpl_QyQS8cyfsCjagxSHdKDWERefxVCA` was `READY`; production `/api/health` served `58f78bd374e1fb493ef08e4ab621ed4f7c66cc87`. `npm run health` passed with `RESULT: 0 FAILING`; `npm run gate:status` passed through `GATE_8` and stopped at external `GATE_9_REAL_NON_OWNER_BETA`. Focused Vitest passed (`9/9`) for provider prompts, connector authorize URLs, redirect docs, and GATE_9 separation. Focused Playwright passed for dashboard sign-out/account reset, public Google/Microsoft sign-in account-choice requests, and settings Google/Microsoft connector redirects. `npm run build` passed after clearing stale generated `.next` output from the first failed build attempt; `npm run lint` passed. `npm run gate:frontend` passed: browser screenshot matrix and interaction matrix passed; banned-copy audit and layout contract passed; production current screenshots are not newly claimed for this auth/config seam.
- Unresolved issues: Google Cloud Console redirect URI update remains manual/external. Real non-owner beta still requires exactly one non-Brandon account to sign in and connect Google or Microsoft through production OAuth; no mock, owner, test, canary, or fabricated row counts.
