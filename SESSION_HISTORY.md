...
## 2026-05-13 - Public demo boundary sanitized
- MODE: Demo-boundary audit and first public leak patch only; no backend logic, controller work, paid generation, outbound email, Stripe, schema, or broad frontend redesign.
- Files changed: `app/HomePageClient.tsx`, `components/demo/ApproveDialog.tsx`, `components/demo/DemoDashboardPage.tsx`, `components/demo/Sidebar.tsx`, `components/foldera/DashboardPreview.tsx`, `components/foldera/LandingPage.tsx`, `components/foldera/MobilePreview.tsx`, `components/foldera/ProductPreviewPanel.tsx`, `lib/cold-read.ts`, `lib/demo/demo-data.ts`, `lib/demo/demo-store.ts`, `tests/e2e/demo-route.spec.ts`, `SESSION_HISTORY.md`.
- What changed: Removed Brandon/Alex/private-owner-shaped copy from the unauthenticated demo and landing preview path. The public demo now uses a sanitized fictional Jordan/Riley customer-follow-up story with an unanswered work thread, calendar review hold, stale draft, board update, source trail, and approve/audit controls. Public cold-read job-search copy was replaced with an allowed hiring-packet framing.
- Verification: `npm run health` passed with `RESULT: 0 FAILING`; targeted public/demo grep found no forbidden private owner terms in `app/demo`, `components/demo`, `lib/demo`, landing preview components, `app/HomePageClient.tsx`, `lib/cold-read.ts`, or the demo route test aside from false-positive React `children` prop names; `npm run build` passed; `npm run lint` passed; `npx playwright test tests/e2e/demo-route.spec.ts tests/e2e/public-routes.spec.ts --reporter=list` passed (`54/54`).
- Unresolved issues: Production deploy and `/demo` production proof are pending until this boundary commit is pushed and deployed. Private owner proof fixtures and authenticated owner paths were intentionally preserved.
