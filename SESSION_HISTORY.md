...
## 2026-05-02 — Fail-closed command-center artifact gate restored
- MODE: Foldera Full Audit And Fix Plan P0 — restore fail-closed artifact gating.
...
- What changed: Reverted the safety-only soft-warning drift from `ee663b0` so artifact quality `passes` once again means safe to persist or send. Off-wedge, missing-grounding, generic coaching/prep homework, monitor-only check-ins, no concrete outcome, bad gold-set artifacts, and other command-center failures block before `pending_approval` and at daily/manual send recheck with `No safe artifact today.` / send suppression receipts instead of soft-warning delivery.
- Verification: `npm run health` passed (`RESULT: 0 FAILING`, Outlook warning only); focused P0 proof `npx vitest run lib/briefing/__tests__/artifact-quality-gate.test.ts lib/cron/__tests__/daily-brief.test.ts lib/briefing/__tests__/generator-runtime.test.ts lib/briefing/__tests__/pipeline-receipt.test.ts lib/briefing/__tests__/usefulness-gate.test.ts --reporter=basic` passed; `npm run test:ci:unit` passed; `npm run preflight` returned `3 pass`, `1 warn`, `0 FAIL`; `npm run scoreboard` showed the latest settings run blocked six bad ranked candidates and returned `generation_failed_sentinel`; `npm run lint` passed; `npm run build` passed; `npm run test:ci:e2e:smoke` passed (`40` tests); `npx playwright test tests/e2e/authenticated-routes.spec.ts --reporter=list` passed (`29` tests).
- Unresolved issues: No paid/model proof was run and no outbound email was sent. Production deploy/health proof is still pending until this corrective commit is pushed and Vercel serves the new build.
