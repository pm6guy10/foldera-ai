...
Last evidence: 2026-05-01 — one explicitly approved authenticated owner `POST https://www.foldera.ai/api/settings/run-brief?force=true&use_llm=true` attempt was made at `2026-05-01T18:18:37.464Z` against production build `c5da3ca` / deployment `dpl_BQGueFjnECfiUTojL2CwbBgTuYq7`. The client received `ECONNRESET`, and immediate follow-up checks initially found no owner `tkg_actions`, no `pipeline_runs`, no confirmed `api_usage`, and no `/api/conviction/latest` pending artifact after the proof start. Later proof review observed that the paid attempt ultimately produced and emailed a bad artifact, `Resend Relationship Status & Interview Decision Map`, which treated transactional sender `onboarding@resend.dev` silence as a possible employer/vendor/relationship obligation, asked whether Brandon had moved on, created reputational/professional risk pressure around unrelated job decisions, and demanded closure before external decisions were final. That artifact is a real BL-015 quality failure, not a successful proof and not merely a missing-artifact transport failure. Unpaid transport diagnostic follow-up on production build `3005890` / deployment `dpl_8qRsdXiNv9xAMaRVVq3vn7EKPUqP` returned HTTP 200 for authenticated `POST /api/settings/run-brief?transport_diagnostic=true&force=true`, with `paid_llm_requested=false`, `paid_llm_effective=false`, `live_generation_executed=false`, and `live_sync_executed=false`. The route persisted and read back `pipeline_runs` receipt `37679bb0-5567-40e2-a9b7-b79e02f825fd` with outcome `route_transport_diagnostic_returned`, invocation source `settings_run_brief_transport_diagnostic`, `route_entered=true`, `auth_resolved=true`, `lifecycle_started=false`, and `paid_generation_started=false`. 2026-05-01 local regression now blocks the Resend/onboarding decision-pressure class with `artifact_quality:transactional_sender_decision_pressure`, while the ESB Technician role-fit money-shot fixture still passes.
Next blocker: Transport/auth/DB receipt is proven and the exact bad Resend/onboarding artifact class is locally blocked, but BL-015 still requires a separately approved single owner Generate Now proof that returns or persists one fresh strict-rubric PASS `pending_approval` artifact visible in-product. Do not retry the paid path automatically; no paid test is the default unless Brandon explicitly approves that specific run.
...
Ending success state: Unless `ALLOW_APPROVAL_EMAIL_SEND === "true"`, approval cannot send Gmail, Outlook, or Resend email; write_document approval still saves the safe artifact but blocks the document-ready Resend email; `execution_result` records `email_send_disabled`; dashboard labels the primary approval action as `Approve` or `Save`, not `Approve & send`; Skip remains visible.
Problem: The approval path could send provider email for email artifacts or document-ready Resend email for write_document artifacts while Brandon does not want Foldera sending outbound email.
Protected contracts: No paid generation, no real email send, preserve safe handled approval/save behavior, preserve Skip, and do not touch auth, billing, generation, provider sync, or migrations.
Allowed files: `lib/conviction/execute-action.ts`, `lib/conviction/__tests__/execute-action.test.ts`, `app/dashboard/page.tsx`, `tests/e2e/authenticated-routes.spec.ts`, `FOLDERA_PRODUCTION_BACKLOG.md`, `CURRENT_STATE.md`, `SESSION_HISTORY.md`
...
Status: CLOSED
Last evidence: 2026-05-01 — local regression covers default-disabled send_message, legacy email artifact, and write_document document-ready Resend paths. Disabled approval returns executed/safe handled state with `email_send_disabled` in `execution_result` and no Gmail, Outlook, or Resend call. Dashboard proof expects `Save` for write_document and `Approve` for email artifacts, with no `Approve & send` and visible `Skip`. Validation passed: `npx vitest run lib/conviction/__tests__/execute-action.test.ts` (18 tests), `npx playwright test tests/e2e/authenticated-routes.spec.ts --grep "write_document journey"` (2 tests), `npm run health` (`RESULT: 0 FAILING`), `npm run preflight` (3 pass, 1 warn, 0 FAIL; local paid LLM unset), `npm run lint`, and `npm run build`.
Next blocker: None for this seam. Outbound approval sends require the explicit `ALLOW_APPROVAL_EMAIL_SEND === "true"` production flag.
