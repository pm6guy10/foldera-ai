...
export function DashboardSidebar({ activeLabel, userName, variant = 'default' }: DashboardSidebarProps) {
  const [accountOpen, setAccountOpen] = useState(false);
  const initial = userName.trim().charAt(0).toUpperCase() || 'B';

  if (variant === 'stage') {
    return (
      <aside className="foldera-dashboard-stage-sidebar absolute left-0 top-0 h-[1152px] w-[320px]">
        <div className="absolute left-[58px] top-[56px]">
          <FolderaLogo href="/dashboard" markSize="sm" />
        </div>

        <nav className="absolute left-[26px] top-[140px] w-[276px] space-y-2">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                href={item.href}
                className={`foldera-dashboard-nav-item ${isActive ? 'is-active' : ''} flex w-full items-center gap-3 rounded-[16px] px-5 text-[22px] leading-none transition-colors ${
                  isActive
                    ? 'h-[58px] border border-white/12 bg-white/[0.05] text-text-primary'
                    : 'h-[48px] border border-transparent text-text-secondary hover:bg-white/[0.02] hover:text-text-primary'
                }`}
              >
                <Icon className={`h-[18px] w-[18px] ${isActive ? 'text-text-primary' : 'text-text-muted'}`} aria-hidden="true" />
        ...
        <div className="foldera-subpanel foldera-dashboard-upgrade absolute left-[26px] top-[654px] h-[196px] w-[276px] px-5 py-5">
          <div className="flex items-start gap-3">
            <ArrowUpRight className="mt-0.5 h-4 w-4 shrink-0 text-accent" strokeWidth={2.25} aria-hidden />
            <div>
              <p className="text-[24px] font-semibold leading-none text-accent">Upgrade to Pro</p>
              <p className="mt-3 text-[22px] leading-[34px] text-text-muted">
        ...
        <div className="foldera-dashboard-account absolute left-[28px] top-[984px] h-[92px] w-[268px] rounded-[18px] border border-border bg-white/[0.02] px-3 py-3">
