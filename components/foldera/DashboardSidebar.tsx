  ...
  return (
    <aside className="foldera-panel hidden h-[calc(100vh-1.5rem)] min-h-[calc(100vh-1.5rem)] flex-col p-5 lg:sticky lg:top-3 lg:flex">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className={`flex min-h-[48px] items-center gap-3 rounded-2xl px-4 text-sm transition-colors ${
                isActive
                  ? 'border border-white/10 bg-white/[0.07] text-text-primary'
      ...
      <div className="mt-auto space-y-4 pt-6">
        ...
        <div className="relative rounded-[18px] border border-border bg-white/[0.02] px-3 py-3">
          ...
          {accountOpen ? (
            <div
              className="absolute bottom-full left-0 right-0 z-20 mb-2 rounded-[14px] border border-border bg-panel p-2 shadow-lg"
              role="menu"
            >
              <button
                type="button"
