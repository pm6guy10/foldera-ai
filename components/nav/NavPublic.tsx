  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between min-w-0">
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-h-[2.5rem]">
          {/* Fade these in after session resolves — avoids Sign in / Dashboard swap flicker */}
          <div
            className={`flex items-center gap-3 sm:gap-4 transition-opacity duration-200 ${
              sessionReady ? 'opacity-100' : 'opacity-0 pointer-events-none max-sm:w-0 max-sm:overflow-hidden max-sm:pointer-events-none'
            }`}
          >
            {!isLoggedIn && (
              <a
          ...
          {/* Hamburger stays interactive while session is loading (not logged in) */}
          {!isLoggedIn && (
            <button
              type="button"
              onClick={() => setMenuOpen((v) => !v)}
              className="sm:hidden flex flex-col items-center justify-center gap-[5px] w-10 h-10 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors shrink-0"
      ...
      {menuOpen && !isLoggedIn && (
        <div className="sm:hidden absolute top-full left-0 right-0 z-[60] max-h-[min(85dvh,calc(100dvh-4rem))] overflow-y-auto bg-[#07070c]/95 backdrop-blur-xl border-b border-white/10 px-6 py-5 flex flex-col gap-4 shadow-2xl">
          {navLinks.map((l) => (
            <a
              key={l.label}
              href={l.href}
  ...
  return (
    <nav className="relative z-10 w-full px-6 py-5 flex items-center justify-between max-w-6xl mx-auto border-b border-white/5">
      <a href="/" className="flex items-center gap-3 group">
        <FolderaMark
          size="sm"
          className="shadow-[0_0_24px_rgba(255,255,255,0.2)] transition-transform group-hover:scale-105"
        />
